    ...
    try:
        ...
        with connect() as db:
            ...
            if entry_id:
                ...
                # 引导带 entry_id 改(对齐 canon 的 on-conflict 幂等;别把 UniqueViolation 原文吐给 LLM)。
                if not new_row:
                    return (
                        f"失败: 剧本 #{sid} 已有同名世界书条目「{str(title).strip()}」。"
                        "要改它请带 entry_id(先用 list_worldbook_entries 拿 entry_id),不要重复新建。"
                    )
