...
def register_tavern_tools() -> None:
    ...
    if not registry.has("set_tavern_immersive"):
        registry.register(ToolSpec(
            ...
            description=(
                "开/关本对话的【沉浸式拟人模式】(持久开关,设置后逐回合生效、不会丢失)。\n"
                "⚠️ 这是用户偏好开关,默认【关闭】。**只在玩家明确提出时才调用,绝不主动 / 默认开启** —— "
                "不要因为『只是开始角色扮演 / 设定了角色 / 角色性格内向』就调用它。\n"
                "调用 enabled=true 的唯一时机:玩家明确说出类似『像真人一样聊天 / 用沉浸式 / 别写成小说 / "
                "别帮我说话和行动 / 第一人称对话』的要求(或所扮演角色卡里写明了要拟真互动的指令)。\n"
