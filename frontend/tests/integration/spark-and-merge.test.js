...
// 缺失嵌套字段：baseline 的子字段必须保留
{
  ...
  assertEq(
    baseline.permissions.pending_writes.length,
    0,
    'permissions.pending_writes 保留 baseline []',
  );
}
...
if (
  !/it\.data\.options\s*\|\|\s*it\.data\.choices/.test(gcjsx) ||
  !/\)\s*\|\|\s*\[\]\)\.map/.test(gcjsx)
) {
  console.error(
    'FAIL · ConfirmStrip 缺 (it.data.options || it.data.choices) || [] 兜底（task 46）',
  );
  process.exit(1);
}
console.log(
  'PASS · game-composer.jsx ConfirmStrip 兜底三连仍在（含 task 46 options/choices 兼容）',
);
...
function assertUnique(keys, label) {
  ...
  for (const k of keys) {
    if (seen.has(k)) {
      console.error(`FAIL · ${label}: 重复 key=${k}（all=${keys.join(',')}）`);
      process.exit(1);
    }
    seen.add(k);
  }
...
for (const name of ['HistoryDrawer', 'SearchDrawer']) {
  if (!new RegExp(`function ${name}\\(`).test(gajsx)) {
    console.error(`FAIL · ${name} 组件未定义`);
    process.exit(1);
  }
  if (!new RegExp(`\\b${name}\\b`).test(gajsx)) {
    console.error(`FAIL · ${name} 未挂到 window`);
    process.exit(1);
  }
}
...
if (!/showSearchDrawer, setShowSearchDrawer/.test(gchtml)) {
  console.error('FAIL · Game Console.html 缺 showSearchDrawer state');
  process.exit(1);
}
if (!/onOpenSearch=\{\(\) => setShowSearchDrawer\(true\)\}/.test(gchtml)) {
  console.error('FAIL · onOpenSearch 没绑 setShowSearchDrawer(true)');
  process.exit(1);
}
if (!/onOpenHistory=\{\(\) => setShowHistoryDrawer\(true\)\}/.test(gchtml)) {
  console.error('FAIL · onOpenHistory 没绑 setShowHistoryDrawer(true)');
  process.exit(1);
}
if (!/<HistoryDrawer open=\{showHistoryDrawer\}/.test(gchtml)) {
  console.error('FAIL · <HistoryDrawer> 未挂载');
  process.exit(1);
}
if (!/<SearchDrawer open=\{showSearchDrawer\}/.test(gchtml)) {
  console.error('FAIL · <SearchDrawer> 未挂载');
  process.exit(1);
}
...
for (const fn of ['HistoryDrawer', 'SearchDrawer']) {
  ...
  if (!m) {
    console.error(`FAIL · ${fn} 提取失败`);
    process.exit(1);
  }
  if (!/onClose/.test(m[0])) {
    console.error(`FAIL · ${fn} 缺 onClose`);
    process.exit(1);
  }
  if (!/Escape/.test(m[0])) {
    console.error(`FAIL · ${fn} 缺 Esc 关闭`);
    process.exit(1);
  }
}
...
if (/currentSaveId=\{game\._raw\?\.save_id \|\| 11\}/.test(gchtml)) {
  console.error('FAIL · Game Console 仍有 hardcoded 11 兜底（task 10）');
  process.exit(1);
}
if (!/currentSaveId=\{activeSave\?\.id \?\? null\}/.test(gchtml)) {
  console.error('FAIL · Game Console currentSaveId 未改为 activeSave?.id');
  process.exit(1);
}
if (!/reloadSaves/.test(gchtml)) {
  console.error('FAIL · Game Console 未拉真实 saves');
  process.exit(1);
}
...
if (!/HASH_ALIASES.*branches.*saves-branches/.test(platformHtml.replace(/\n/g, ' '))) {
  console.error('FAIL · Platform.html 缺 #branches → #saves-branches 别名');
  process.exit(1);
}
...
for (const f of ['src/game-app.jsx', 'Design Canvas.html', 'Overview.html', 'index.html']) {
  ...
  if (/Platform\.html#branches\b/.test(src)) {
    console.error(`FAIL · ${f} 仍有旧的 Platform.html#branches 链接（task 11）`);
    process.exit(1);
  }
}
...
if (!/realSaves\.length/.test(plSrc12)) {
  console.error('FAIL · ProfilePage 没用 realSaves.length 派生存档数 (task 12)');
  process.exit(1);
}
// 不应再渲染 stats.api_calls / 1000 之类的旧 mock 公式
if (/stats\.api_calls\s*\/\s*1000/.test(plSrc12)) {
  console.error('FAIL · ProfilePage 仍在用 stats.api_calls / 1000 渲染 mock 数字');
  process.exit(1);
}
// 不应再有 branchLabel 假分支表（只在 ProfilePage 函数体内查，brace-match 截取）
const profileBody = extractFunction(plSrc12, 'ProfilePage');
if (!profileBody) {
  console.error('FAIL · 未抽出 ProfilePage 函数体');
  process.exit(1);
}
if (/branchLabel\s*=/.test(profileBody)) {
  console.error('FAIL · ProfilePage 仍有 hard-coded branchLabel 赋值 (task 12)');
  process.exit(1);
}
if (/Math\.floor\(\(s\.id \* 137\)/.test(profileBody)) {
  console.error('FAIL · ProfilePage 仍有「第 X 章」mock 公式 (task 12)');
  process.exit(1);
}
...
if (!/platform\.stats\s*=\s*\{[\s\S]*scripts:\s*\(platform\.scripts/.test(dlSrc12)) {
  console.error('FAIL · data-loader stats 未从真实 platform.scripts 派生');
  process.exit(1);
}
...
if (!/function useReactiveUser\(/.test(plSrc13)) {
  console.error('FAIL · useReactiveUser hook 缺失 (task 13)');
  process.exit(1);
}
if (!/function publishUser\(/.test(plSrc13)) {
  console.error('FAIL · publishUser 函数缺失 (task 13)');
  process.exit(1);
}
if (!/window\.__publishUser/.test(plSrc13)) {
  console.error('FAIL · __publishUser 未 expose 到 window (task 13)');
  process.exit(1);
}
// PlatformShell 必须用 reactiveUser 不用 platform.user 渲染左侧名片
const shellBody = extractFunction(plSrc13, 'PlatformShell');
if (!shellBody) {
  console.error('FAIL · 提取 PlatformShell 失败');
  process.exit(1);
}
if (!/reactiveUser\.display_name/.test(shellBody)) {
  console.error('FAIL · PlatformShell 仍在用 platform.user.display_name 不响应保存 (task 13)');
  process.exit(1);
}
...
if (!/function GameToastStack\(/.test(gajsx14)) {
  console.error('FAIL · GameToastStack 组件未定义 (task 14)');
  process.exit(1);
}
if (!/window\.toast\s*=\s*fire/.test(gajsx14)) {
  console.error('FAIL · game-app.jsx 未在加载时安装 window.toast (task 14)');
  process.exit(1);
}
if (!/__GAME_TOAST_INSTALLED/.test(gajsx14)) {
  console.error('FAIL · 缺少重复安装防御 __GAME_TOAST_INSTALLED (task 14)');
  process.exit(1);
}
if (!/GameToastStack/.test(gchtml)) {
  console.error('FAIL · Game Console.html 未渲染 <GameToastStack/> (task 14)');
  process.exit(1);
}
...
if (/const __late = \(name\) =>/.test(gchtml)) {
  console.error('FAIL · 检测到 __late 包装器，必须移除（Chrome renderer crash）');
  process.exit(1);
}
for (const name of [
  'HistoryDrawer',
  ...
  if (!new RegExp(`const ${name} = window\\.${name} \\|\\| \\(\\(\\) => null\\)`).test(gchtml)) {
    console.error(`FAIL · ${name} 没用 const X = window.X || (() => null) 兜底 (task 21)`);
    process.exit(1);
  }
}
...
if (!/const \[mountStage, setMountStage\] = useState\(0\)/.test(gchtml)) {
  console.error('FAIL · Game Console.html 缺 mountStage 分阶段挂载 (task 21)');
  process.exit(1);
}
if (!/mountStage >= 1/.test(gchtml) || !/mountStage >= 2/.test(gchtml)) {
  console.error('FAIL · 缺 stage>=1 / stage>=2 gate (task 21)');
  process.exit(1);
}
...
if (!/readFileAsBase64/.test(plSrc16)) {
  console.error('FAIL · 缺 readFileAsBase64（task 16）');
  process.exit(1);
}
if (!/file:\s*\{\s*name:\s*selectedFile\.name,\s*base64\s*\}/.test(plSrc16)) {
  console.error('FAIL · POST /api/scripts/preview body 不是 {file:{name,base64}} 形状');
  process.exit(1);
}
// 失败路径必须 toast + 不再回退 fakeFile（previewError 标志）
if (!/previewError:\s*detail/.test(plSrc16)) {
  console.error('FAIL · 失败路径没有 previewError 字段（前端无法区分真实预算 vs 失败）');
  process.exit(1);
}
// 不能再把 isMockEstimate / fakeFile 混进真实文件的成功路径
const startEstBody =
  (plSrc16.match(/const startEstimate = async \(\) => \{[\s\S]*?\n  \};/) || [])[0] || '';
if (!startEstBody) {
  console.error('FAIL · 提取 startEstimate body 失败');
  process.exit(1);
}
// 真实文件成功路径不应再次 setEstimate({file: fakeFile, ...})（按缩进/上下文判断）
if (
  /setEstimate\(\{\s*file:\s*fakeFile/.test(
    startEstBody
      .replace(/\/\/[^\n]*/g, '')
      .split('if (!selectedFile)')
      .slice(1)
      .join('if (!selectedFile)')
      .split('setPreviewBusy(false);\n      return;')
      .slice(1)
      .join(''),
  )
) {
  // 上面的切片是：先去掉「无选文件」分支（合法 fakeFile），然后剩下的部分不应再有 setEstimate(fakeFile)
  console.error('FAIL · 选了真实文件后仍可能回退 fakeFile（task 16 静默假数据问题）');
  process.exit(1);
}
...
if (!/total_bytes:\s*totalBytes,\s*\n\s*total_chunks:\s*totalChunks/.test(plSrc17)) {
  console.error('FAIL · startImport 未发 total_bytes/total_chunks 给 /api/uploads/init (task 17)');
  process.exit(1);
}
if (!/chunk_index:\s*Number\(index\)/.test(apiSrc)) {
  console.error('FAIL · api-client.uploads.chunk 未改成 JSON {chunk_index, base64} (task 17)');
  process.exit(1);
}
// 失败路径必须 setJob(null) 而不是建假 job
const startImportBody =
  (plSrc17.match(/const startImport = async \(\) => \{[\s\S]*?\n  \};/) || [])[0] || '';
if (!startImportBody) {
  console.error('FAIL · 提取 startImport body 失败');
  process.exit(1);
}
if (!/setJob\(null\);/.test(startImportBody)) {
  console.error('FAIL · startImport catch 路径未 setJob(null)；仍可能造 fake job (task 17)');
  process.exit(1);
}
if (!/upload_id:\s*uploadId/.test(startImportBody)) {
  console.error('FAIL · scripts.importScript 未传 upload_id (task 17)');
  process.exit(1);
}
...
if (!/upload_id=str\(body\.get\("upload_id"\) or ""\)/.test(apiPy)) {
  console.error('FAIL · 后端 api_import_script 没透传 body.upload_id (task 17)');
  process.exit(1);
}
...
if (/HASH_ALIASES = \{[^}]*scripts[^}]*\}/.test(platformHtml17)) {
  console.error('FAIL · Platform.html HASH_ALIASES 仍把合法 #scripts 重映射 (task 18)');
  process.exit(1);
}
if (!/HASH_ALIASES = \{\s*"branches":\s*"saves-branches"\s*\}/.test(platformHtml17)) {
  console.error('FAIL · HASH_ALIASES 应只剩 branches→saves-branches (task 18)');
  process.exit(1);
}
...
if (!newGameBody) {
  console.error('FAIL · 抽 NewGameModal 失败');
  process.exit(1);
}
if (/setPickedCard\("uc1"\)/.test(newGameBody)) {
  console.error("FAIL · NewGameModal 仍用 mock 'uc1' 当默认 character (task 20)");
  process.exit(1);
}
if (!/window\.api\.scripts\.list\(\)/.test(newGameBody)) {
  console.error('FAIL · NewGameModal 没拉真实 /api/scripts (task 20)');
  process.exit(1);
}
if (!/\/api\/me\/personas/.test(newGameBody)) {
  console.error('FAIL · NewGameModal 没拉真实 personas (task 20)');
  process.exit(1);
}
if (!/submitErr/.test(newGameBody)) {
  console.error('FAIL · NewGameModal 没有 inline 错误显示 (task 20)');
  process.exit(1);
}
...
if (!chatBody) {
  console.error('FAIL · 抽 ChatArea 失败');
  process.exit(1);
}
if (!/HISTORY_WINDOW\s*=\s*80/.test(chatBody)) {
  console.error('FAIL · ChatArea 没有 HISTORY_WINDOW 窗口化（task 21）');
  process.exit(1);
}
if (!/history\.slice\(visibleStart\)/.test(chatBody)) {
  console.error('FAIL · ChatArea 仍渲染整段 history（task 21）');
  process.exit(1);
}
if (!/requestAnimationFrame/.test(chatBody)) {
  console.error('FAIL · ChatArea scroll 没用 rAF 延后（task 21）');
  process.exit(1);
}
...
if (
  !/requestAnimationFrame\(\(\) => \{\s*if \(cancelled\) return;\s*reloadState\(\);\s*reloadSaves\(\);/.test(
    gchtml21.replace(/\n/g, ' '),
  )
) {
  console.error('FAIL · Game Console.html 没把 reloadState/reloadSaves 延后到 rAF（task 21）');
  process.exit(1);
}
...
if (!/PICK_STATE_KEYS\s*=\s*\["player","world"/.test(gchtmlOOM)) {
  console.error('FAIL · Game Console 缺 PICK_STATE_KEYS 白名单（task 21 OOM）');
  process.exit(1);
}
// 不能再有 spread data 的 setGame
if (/setGame\(g => \(\{ \.\.\.g, \.\.\.data \}\)\)/.test(gchtmlOOM)) {
  console.error(
    'FAIL · Game Console 仍有 setGame({...g, ...data})；会把 tools/models/_raw 200KB+ 灌进 React state（task 21 OOM）',
  );
  process.exit(1);
}
if (/setGame\(g => \(\{ \.\.\.g, \.\.\.data\.state \}\)\)/.test(gchtmlOOM)) {
  console.error('FAIL · Game Console 仍有 setGame({...g, ...data.state})（同样问题）');
  process.exit(1);
}
...
if (/merged\._raw = data;/.test(dlSrcOOM)) {
  console.error('FAIL · data-loader 仍把整个 backend response 存进 _raw（task 21 OOM）');
  process.exit(1);
}
if (!/merged\._raw = \{\s*save_id:/.test(dlSrcOOM)) {
  console.error('FAIL · data-loader 应只把 {save_id, save_title, turn} 存进 _raw（task 21 OOM）');
  process.exit(1);
}
...
for (const [src, name] of [
  [plSrc24, 'platform-app.jsx'],
  [dlSrc24, 'data-loader.js'],
  [gchtml24, 'Game Console.html'],
]) {
  // 旧形态：(r?.scripts || []) 或 (saves && saves.saves) 等无 items 兜底
  // 匹配 `: (r?.scripts || [])`（不允许前面有 items）
  if (/:\s*\(\s*[rs]\?\.scripts\s*\|\|\s*\[\]\s*\)/.test(src)) {
    console.error(`FAIL · ${name} 仍有 ": (r?.scripts || [])" 旧形态（漏 .items）`);
    process.exit(1);
  }
  if (/:\s*\(\s*[rs]\?\.saves\s*\|\|\s*\[\]\s*\)/.test(src)) {
    console.error(`FAIL · ${name} 仍有 ": (r?.saves || [])" 旧形态（漏 .items）`);
    process.exit(1);
  }
}
// 所有 list-parse 应同时检查 .items
if (!/items \|\| r\?\.scripts/.test(plSrc24)) {
  console.error('FAIL · platform-app.jsx 缺 .items || .scripts 复合解析 (task 24)');
  process.exit(1);
}
if (!/items \|\| r\?\.saves/.test(plSrc24)) {
  console.error('FAIL · platform-app.jsx 缺 .items || .saves 复合解析 (task 24)');
  process.exit(1);
}
if (!/items \|\| scripts\?\.scripts/.test(dlSrc24)) {
  console.error('FAIL · data-loader.js 缺 scripts.items 解析 (task 24)');
  process.exit(1);
}
if (!/items \|\| saves\?\.saves/.test(dlSrc24)) {
  console.error('FAIL · data-loader.js 缺 saves.items 解析 (task 24)');
  process.exit(1);
}
...
if (!/function _renderVarValue\(/.test(gpSrc26)) {
  console.error('FAIL · game-panels.jsx 缺 _renderVarValue 兜底函数 (task 26)');
  process.exit(1);
}
// gp-var-val 必须走 _renderVarValue 而不是直接 {v}
if (!/<span className="gp-var-val">\{_renderVarValue\(v\)\}<\/span>/.test(gpSrc26)) {
  console.error('FAIL · gp-var-val 仍直接渲染对象 v (task 26)');
  process.exit(1);
}
...
if (!/_renderVarValue\(state[^)]*worldline[^)]*last_projection\)/.test(gpSrc26)) {
  console.error('FAIL · last_projection 仍直接放进 JSX (task 26)');
  process.exit(1);
}
...
try {
  ...
  if (!/ALL TESTS PASSED/.test(out)) {
    console.error(
      'FAIL · _test_paneltimeline.js 没有 ALL TESTS PASSED：\n' +
        out.split('\n').slice(-10).join('\n'),
    );
    process.exit(1);
  }
...
if (!/task 27[：:]/.test(uiSrc27)) {
  console.error('FAIL · ui.py 缺 task 27 注释锚（应在 /api/chat 里）');
  process.exit(1);
}
...
if (pre < 0 || persistIdx < 0 || preLlmIdx < 0 || persistIdx < pre || preLlmIdx < persistIdx) {
  console.error(
    'FAIL · ui.py /api/chat 没有按『apply_player_directives → _persist_runtime_checkpoint → updates(stage=pre_llm)』顺序排布 (task 27)',
  );
  process.exit(1);
}
...
if (!/task 28[：:]/.test(stSrc28)) {
  console.error('FAIL · state.py 缺 task 28 注释锚');
  process.exit(1);
}
...
if (timeIdx28 < 0 || assignIdx28 < 0 || timeIdx28 > assignIdx28) {
  console.error(
    'FAIL · apply_set_directive 顺序错：time targets 必须在 assignments 之前 (task 28)',
  );
  process.exit(1);
}
...
if (!/setNewCardBg\b/.test(plSrc29) || !/value=\{newCardBg\}/.test(plSrc29)) {
  console.error('FAIL · NewGameModal「设定」textarea 仍没绑 state (task 29)');
  process.exit(1);
}
// new_card payload 必须带 background
if (!/background:\s*newCardBg/.test(plSrc29)) {
  console.error('FAIL · new_card payload 没带 background (task 29)');
  process.exit(1);
}
// onCreate 必须透传 character_kind
if (!/character_kind:\s*vals\.character_kind/.test(plSrc29)) {
  console.error('FAIL · onCreate 没把 character_kind 透传给后端 (task 29)');
  process.exit(1);
}
...
if (!/task 29[：:]/.test(wsSrc29)) {
  console.error('FAIL · workspace.py 缺 task 29 注释锚');
  process.exit(1);
}
if (!/def create_save\([^)]*new_card[^)]*\)/.test(wsSrc29)) {
  console.error('FAIL · workspace.create_save 签名缺 new_card 参数 (task 29)');
  process.exit(1);
}
if (!/_build_initial_snapshot\(/.test(wsSrc29) || !/setup_player\(/.test(wsSrc29)) {
  console.error('FAIL · workspace 缺 _build_initial_snapshot/setup_player 调用 (task 29)');
  process.exit(1);
}
...
if (!/task 29[：:]/.test(apiSrc29)) {
  console.error('FAIL · api.py 缺 task 29 注释锚');
  process.exit(1);
}
// 简单子串检查：api_create_save 函数体内有 new_card=new_card 这串
if (!/new_card=new_card/.test(apiSrc29)) {
  console.error('FAIL · POST /api/saves 没把 new_card 传到 create_save (task 29)');
  process.exit(1);
}
...
// ---------------------------------------------------------------------------
const frSrc30 = fs.readFileSync(
  path.join(__dirname, '../rpg/platform_app/frontend_routes.py'),
  'utf8',
);
if (!/task 30[：:]/.test(frSrc30)) {
  console.error('FAIL · frontend_routes.py 缺 task 30 注释锚');
  process.exit(1);
}
if (!/_branches\.activate_save\(/.test(frSrc30)) {
  console.error('FAIL · api_save_activate 仍未调 branches.activate_save (task 30)');
  process.exit(1);
}
if (!/_invalidate_user_cache\(user\)/.test(frSrc30)) {
  console.error('FAIL · api_save_activate 仍未清 ui._invalidate_user_cache (task 30)');
  process.exit(1);
}
...
if (!/def activate_save\(user_id: int, save_id: int\)/.test(brSrc30)) {
  console.error('FAIL · branches.py 缺 activate_save 函数 (task 30)');
  process.exit(1);
}
if (!/runtime\.activate_state_snapshot\(/.test(brSrc30)) {
  console.error('FAIL · activate_save 没调 runtime.activate_state_snapshot (task 30)');
  process.exit(1);
}
// 前端 ContinueGameModal confirm 必须先 await activate 再 navigate
if (!/task 30[：:]/.test(plSrc29)) {
  console.error('FAIL · platform-app.jsx 缺 task 30 注释锚');
  process.exit(1);
}
if (
  !/\/api\/saves\/\$\{[^}]*\}\/activate/.test(plSrc29) &&
  !/window\.api\.saves\.activate\(targetId\)/.test(plSrc29)
) {
  console.error('FAIL · confirm() 没在跳 Game Console 前 POST /api/saves/{id}/activate (task 30)');
  process.exit(1);
}
...
if (/请求中断：上游 504。已保留你的上一条输入/.test(gaSrc31)) {
  console.error('FAIL · game-app.jsx 仍硬编码『请求中断：上游 504』作为错误显示文本 (task 31)');
  process.exit(1);
}
if (!/task 31[：:]/.test(gaSrc31)) {
  console.error('FAIL · game-app.jsx 缺 task 31 注释锚');
  process.exit(1);
}
...
if (!/task 32[：:]/.test(stSrc32)) {
  console.error('FAIL · state.py 缺 task 32 注释锚');
  process.exit(1);
}
...
if (/if "时间跳跃确认" in tag:\s*\n\s*return False/.test(helperBody)) {
  console.error(
    'FAIL · _gm_is_asking_for_time_confirm 仍在第一个『时间跳跃确认』tag 就立刻 return False (task 32)',
  );
  process.exit(1);
}
if (!/has_pending_signal/.test(helperBody) || !/has_explicit_confirm/.test(helperBody)) {
  console.error(
    'FAIL · _gm_is_asking_for_time_confirm 缺 has_pending_signal/has_explicit_confirm 双信号 (task 32)',
  );
  process.exit(1);
}
// apply_structured_updates 的 "时间跳跃确认" 分支也必须检查 value pending
if (!/_value_pending/.test(stSrc32)) {
  console.error('FAIL · apply_structured_updates 时间跳跃确认分支缺 _value_pending 防御 (task 32)');
  process.exit(1);
}
...
if (!/task 33[：:]/.test(gpSrc33)) {
  console.error('FAIL · game-panels.jsx 缺 task 33 注释锚');
  process.exit(1);
}
// PanelContext 必须把 chapter_refs 兜底成数组
if (
  !/Array\.isArray\(lastCtx\.chapter_refs\)\s*\?\s*lastCtx\.chapter_refs\s*:\s*\[\]/.test(gpSrc33)
) {
  console.error('FAIL · PanelContext 没把 chapter_refs 兜底成数组 (task 33)');
  process.exit(1);
}
// PanelTimeline 必须把 timeline / anchors 兜底
if (
  !/const timeline\s*=\s*\(state && state\.world && state\.world\.timeline\) \|\| \{\}/.test(
    gpSrc33,
  )
) {
  console.error('FAIL · PanelTimeline 没把 timeline 兜底 (task 33)');
  process.exit(1);
}
...
try {
  ...
  if (!/ALL TESTS PASSED \(8 panels\)/.test(out)) {
    console.error(
      'FAIL · _test_panel_all_tabs.js 没有 ALL TESTS PASSED：\n' +
        out.split('\n').slice(-15).join('\n'),
    );
    process.exit(1);
  }
...
if (!/task 34[：:]/.test(wsSrc34)) {
  console.error('FAIL · workspace.py 缺 task 34 注释锚');
  process.exit(1);
}
if (!/def _apply_script_opening\(/.test(wsSrc34)) {
  console.error('FAIL · workspace 缺 _apply_script_opening 函数 (task 34)');
  process.exit(1);
}
if (!/def _scrub_berlin_default\(/.test(wsSrc34)) {
  console.error('FAIL · workspace 缺 _scrub_berlin_default 函数 (task 34)');
  process.exit(1);
}
...
if (!/_apply_script_opening\(state,\s*user_id,\s*script_id\)/.test(buildBody)) {
  console.error('FAIL · _build_initial_snapshot 没调 _apply_script_opening (task 34)');
  process.exit(1);
}
// 三个 inline 元数据关键词（当前地点/当前目标/时间锚点）必须出现在 workspace 里
for (const kw of ['当前地点', '当前目标', '时间锚点']) {
  if (!wsSrc34.includes(kw)) {
    console.error('FAIL · workspace 缺 inline meta 关键词『' + kw + '』(task 34)');
    process.exit(1);
  }
}
...
if (!/task 41[：:]/.test(stSrc41)) {
  console.error('FAIL · state.py 缺 task 41 注释锚');
  process.exit(1);
}
// suggestions() 必须有 is_default_novel 判断 + fallback 拆分
if (!/is_default_novel/.test(stSrc41)) {
  console.error('FAIL · suggestions() 缺 is_default_novel 判断 (task 41)');
  process.exit(1);
}
if (!/fallback_generic/.test(stSrc41) || !/fallback_default_novel/.test(stSrc41)) {
  console.error('FAIL · suggestions() fallback 没拆通用/柏林专属 (task 41)');
  process.exit(1);
}
...
if (berlinFallbackIdx < 0) {
  console.error("FAIL · '要求一份柏林当前势力图' 字符串不见了 (task 41)");
  process.exit(1);
}
// 后端测试文件存在
const testFile = path.join(__dirname, '../rpg/tests/test_suggestions_no_berlin_leak.py');
if (!fs.existsSync(testFile)) {
  console.error('FAIL · 缺 test_suggestions_no_berlin_leak.py (task 41)');
  process.exit(1);
}
...
if (!/task 42[：:]/.test(retSrc42)) {
  console.error('FAIL · retrieval.py 缺 task 42 注释锚');
  process.exit(1);
}
// 必须有 _is_default_mumu_script + _strip_default_novel_leakage
if (!/def _is_default_mumu_script\(/.test(retSrc42)) {
  console.error('FAIL · retrieval.py 缺 _is_default_mumu_script (task 42)');
  process.exit(1);
}
if (!/def _strip_default_novel_leakage\(/.test(retSrc42)) {
  console.error('FAIL · retrieval.py 缺 _strip_default_novel_leakage (task 42)');
  process.exit(1);
}
// retrieve_context 签名必须加 script_id
if (!/def retrieve_context\([\s\S]{0,400}script_id/.test(retSrc42)) {
  console.error('FAIL · retrieve_context 签名缺 script_id (task 42)');
  process.exit(1);
}
// 非默认 script 必须跳过 SQLite 来源
if (!/if is_default:\s*\n\s*facts_text = load_chapter_facts/.test(retSrc42)) {
  console.error('FAIL · retrieve_context 没把 load_chapter_facts 包在 is_default 条件下 (task 42)');
  process.exit(1);
}
if (!/if is_default:\s*\n\s*char_names = detect_mentioned_characters/.test(retSrc42)) {
  console.error(
    'FAIL · retrieve_context 没把角色卡 / BM25 / summaries 包在 is_default 条件下 (task 42)',
  );
  process.exit(1);
}
...
const novelProviderSrc42 = fs.readFileSync(
  path.join(__dirname, '../rpg/context_providers/novel.py'),
  'utf8',
);
if (
  !/script_id=script_id/.test(caSrc42) ||
  !/ProviderServices\([\s\S]{0,500}script_id=script_id/.test(caSrc42)
) {
  console.error(
    'FAIL · context_agent 没把 script_id 注入 ProviderServices (task 42/provider 架构)',
  );
  process.exit(1);
}
if (
  !/NovelRetrievalProvider/.test(novelProviderSrc42) ||
  !/services\.retrieve_fn\([\s\S]{0,300}script_id=services\.script_id/.test(novelProviderSrc42)
) {
  console.error(
    'FAIL · NovelRetrievalProvider 没把 script_id 透传给 retrieve_fn (task 42/provider 架构)',
  );
  process.exit(1);
}
// 测试文件存在
if (!fs.existsSync(path.join(__dirname, '../rpg/tests/test_retrieval_no_default_leak.py'))) {
  console.error('FAIL · 缺 test_retrieval_no_default_leak.py (task 42)');
  process.exit(1);
}
...
if (!/script_id\s*=\s*_active_script_id\(api_user\)/.test(openingBody)) {
  console.error('FAIL · api_opening 没拿 script_id = _active_script_id(api_user) (task 43)');
  process.exit(1);
}
if (!/retrieve_context\([\s\S]{0,400}script_id=script_id/.test(openingBody)) {
  console.error('FAIL · api_opening retrieve_context 调用没传 script_id (task 43)');
  process.exit(1);
}
// query 必须按 script_id 动态构（非默认走 player/world/memory token）
if (!/if script_id:\s*\n[\s\S]{0,800}query_parts/.test(openingBody)) {
  console.error('FAIL · api_opening 没按 script_id 动态构 query (task 43)');
  process.exit(1);
}
// 测试文件存在
if (!fs.existsSync(path.join(__dirname, '../rpg/tests/test_opening_no_default_leak.py'))) {
  console.error('FAIL · 缺 test_opening_no_default_leak.py (task 43)');
  process.exit(1);
}
...
if (!/task 44[：:]/.test(ceSrc44)) {
  console.error('FAIL · context_engine.py 缺 task 44 注释锚');
  process.exit(1);
}
// pending 分支必须含禁止性指令
for (const kw of ['禁止把玩家请求的未来时间', '禁止输出标签', 'pending_confirmation']) {
  if (!ceSrc44.includes(kw)) {
    console.error('FAIL · context_engine 缺 pending 禁止指令『' + kw + '』(task 44)');
    process.exit(1);
  }
}
if (!fs.existsSync(path.join(__dirname, '../rpg/tests/test_pending_jump_forbids_narrative.py'))) {
  console.error('FAIL · 缺 test_pending_jump_forbids_narrative.py (task 44)');
  process.exit(1);
}
...
if (!/task 44[：:]/.test(gcHtml44)) {
  console.error('FAIL · Game Console.html 缺 task 44 注释锚');
  process.exit(1);
}
// 老 bug：if (data && data.state) setGame... 必须消失
if (/if \(data && data\.state\) setGame/.test(gcHtml44)) {
  console.error(
    'FAIL · Game Console.html SSE 回调仍误读 data.state（应读 data 或 data.status）(task 44)',
  );
  process.exit(1);
}
// on_done 必须从 data.status 提 payload
if (!/const payload = \(data && data\.status\) \|\| null/.test(gcHtml44)) {
  console.error('FAIL · on_done 没从 data.status 提 payload (task 44)');
  process.exit(1);
}
...
if (!/task 45[：:]/.test(gcHtml45)) {
  console.error('FAIL · Game Console.html 缺 task 45 注释锚');
  process.exit(1);
}
// 老 useState(structuredClone(window.MOCK_STATE)) 必须消失
if (/useState\(\(\)\s*=>\s*structuredClone\(window\.MOCK_STATE\)\)/.test(gcHtml45)) {
  console.error(
    'FAIL · Game Console.html 仍用 useState(structuredClone(window.MOCK_STATE)) 当初始值 (task 45)',
  );
  process.exit(1);
}
// 必须有 IS_ANON 判断 + EMPTY_STATE 兜底
if (!/IS_ANON/.test(gcHtml45) || !/EMPTY_STATE\s*=/.test(gcHtml45)) {
  console.error('FAIL · Game Console.html 缺 IS_ANON / EMPTY_STATE (task 45)');
  process.exit(1);
}
...
if (!/task 45[：:]/.test(dlSrc45) || !/injectDemoBanner/.test(dlSrc45)) {
  console.error('FAIL · data-loader.js 缺 injectDemoBanner (task 45)');
  process.exit(1);
}
...
if (!/task 45[：:]/.test(plSrc45) || !/function usePlatformData/.test(plSrc45)) {
  console.error('FAIL · platform-app.jsx 缺 usePlatformData hook (task 45)');
  process.exit(1);
}
...
if (/const allSaves = window\.MOCK_PLATFORM\.saves;/.test(plSrc45)) {
  console.error('FAIL · ContinuePicker 仍硬读 MOCK_PLATFORM.saves (task 45)');
  process.exit(1);
}
// BRANCH_DATA 不应再被 ContinuePicker 当 nodes 源
if (/const nodes = BRANCH_DATA\.nodes;/.test(plSrc45)) {
  console.error('FAIL · ContinuePicker 仍用 BRANCH_DATA 假节点 (task 45)');
  process.exit(1);
}
// publishUser 不应再 mutate MOCK_PLATFORM.user
if (/window\.MOCK_PLATFORM\.user\s*=\s*\{/.test(plSrc45)) {
  console.error('FAIL · publishUser 仍 mutate MOCK_PLATFORM.user (task 45)');
  process.exit(1);
}
...
if (!/task 45[：:]/.test(gaSrc45)) {
  console.error('FAIL · game-app.jsx 缺 task 45 注释锚');
  process.exit(1);
}
// 必须不再是 `<strong>{window.MOCK_NOVEL.script_title}</strong>` 直接读
if (/<strong>\{window\.MOCK_NOVEL\.script_title\}<\/strong>/.test(gaSrc45)) {
  console.error('FAIL · game-app.jsx 仍把 MOCK_NOVEL.script_title 当唯一品牌名 (task 45)');
  process.exit(1);
}
...
if (!/task 35[：:]/.test(stSrc35)) {
  console.error('FAIL · state.py 缺 task 35 注释锚');
  process.exit(1);
}
// apply_structured_updates 必须用 pending_jump.turn == state.turn 触发强制 asking
if (!/_player_pending_this_turn/.test(stSrc35)) {
  console.error('FAIL · apply_structured_updates 缺 _player_pending_this_turn 防御 (task 35)');
  process.exit(1);
}
...
if (!/task 36[：:]/.test(stSrc36)) {
  console.error('FAIL · state.py 缺 task 36 注释锚');
  process.exit(1);
}
if (!/def mark_user_locked\(/.test(stSrc36) || !/def _is_user_locked\(/.test(stSrc36)) {
  console.error('FAIL · state.py 缺 mark_user_locked / _is_user_locked (task 36)');
  process.exit(1);
}
// update_time 必须检查 user_locked 才决定是否覆盖 current_phase
if (!/_is_user_locked\("world\.timeline\.current_phase"\)/.test(stSrc36)) {
  console.error('FAIL · update_time 没检查 world.timeline.current_phase user lock (task 36)');
  process.exit(1);
}
// apply_state_write 在 force/user 源时必须 mark_user_locked
if (
  !/if force or str\(source or ""\)\.startswith\("user"\):\s*\n\s*self\.mark_user_locked\(path\)/.test(
    stSrc36,
  )
) {
  console.error('FAIL · apply_state_write 没在 force/user 源时 mark_user_locked (task 36)');
  process.exit(1);
}
...
if (!railFootMatch) {
  console.error('FAIL · 找不到 gc-rail-foot JSX 块 (task 37)');
  process.exit(1);
}
// 必须找到 a 标签，且其内不能再出现 verticalAlign+marginRight 在 Icon 上
const anchorMatch = railFootMatch[0].match(/<a [\s\S]*?<\/a>/);
if (!anchorMatch) {
  console.error('FAIL · gc-rail-foot 内找不到 <a> (task 37)');
  process.exit(1);
}
if (/verticalAlign/.test(anchorMatch[0]) || /marginRight/.test(anchorMatch[0])) {
  console.error(
    'FAIL · gc-rail-foot 的 Icon 仍带 verticalAlign/marginRight，会让 SVG 视觉外溢 (task 37)',
  );
  process.exit(1);
}
if (!/task 37[：:]/.test(gaSrc37)) {
  console.error('FAIL · game-app.jsx 缺 task 37 注释锚');
  process.exit(1);
}
...
if (!/task 38[：:]/.test(apiSrc38)) {
  console.error('FAIL · api.py 缺 task 38 注释锚');
  process.exit(1);
}
...
for (const kw of [
  'body.get\\("node_id"\\)',
  'body.get\\("save_id"\\)',
  'body.get\\("message_index"\\)',
]) {
  if (!new RegExp(kw).test(apiSrc38)) {
    console.error('FAIL · api.py /api/branches/continue 没读 ' + kw + ' (task 38)');
    process.exit(1);
  }
}
if (!/缺字段[:：]/.test(apiSrc38)) {
  console.error('FAIL · api.py 缺字段时应返清晰 400 error message (task 38)');
  process.exit(1);
}
...
if (!/def resolve_commit_id_by_message\(/.test(brSrc38)) {
  console.error('FAIL · branches.py 缺 resolve_commit_id_by_message (task 38)');
  process.exit(1);
}
...
if (!/task 38[：:]/.test(gaSrc38)) {
  console.error('FAIL · game-app.jsx 缺 task 38 注释锚');
  process.exit(1);
}
if (!/function MsgActions\([^)]*saveId[^)]*\)/.test(gaSrc38)) {
  console.error('FAIL · MsgActions 签名缺 saveId (task 38)');
  process.exit(1);
}
if (!/function NarrativeBlock\([^)]*msgIndex[\s\S]{0,80}saveId/.test(gaSrc38)) {
  console.error('FAIL · NarrativeBlock 签名缺 msgIndex/saveId (task 38)');
  process.exit(1);
}
// doFork 必须发 save_id+message_index 而不是只发 label
if (
  !/body\.save_id\s*=\s*saveId/.test(gaSrc38) ||
  !/body\.message_index\s*=\s*msgIndex/.test(gaSrc38)
) {
  console.error('FAIL · MsgActions.doFork 没发 save_id+message_index (task 38)');
  process.exit(1);
}
...
if (!/task 38[：:]/.test(gcHtml38)) {
  console.error('FAIL · Game Console.html 缺 task 38 注释锚');
  process.exit(1);
}
if (!/saveId=\{[^}]*activeSave[^}]*\}/.test(gcHtml38)) {
  console.error('FAIL · Game Console.html ChatArea 没传 saveId (task 38)');
  process.exit(1);
}
console.log(
  'PASS · task 38 /api/branches/continue 接 save_id+message_index + 前端 MsgActions 透传 saveId',
);
...
if (!/task 39[：:]/.test(gcompSrc39)) {
  console.error('FAIL · game-composer.jsx 缺 task 39 注释锚');
  process.exit(1);
}
// SLASH_COMMANDS 必须含 id: "set"
if (!/id:\s*"set"\s*,\s*trigger:\s*"\/set "/.test(gcompSrc39)) {
  console.error('FAIL · SLASH_COMMANDS 缺 /set 条目 (task 39)');
  process.exit(1);
}
// Game Console.html onSlashPick 必须把 trigger（带空格）写到 text
const gcHtml39 = gcHtml38; // 同文件
if (!/task 39[：:]/.test(gcHtml39)) {
  console.error('FAIL · Game Console.html 缺 task 39 注释锚');
  process.exit(1);
}
if (!/cmd\.trigger\.endsWith\(" "\)/.test(gcHtml39)) {
  console.error('FAIL · onSlashPick 没识别『trigger 带空格 = 立即写入 text』(task 39)');
  process.exit(1);
}
if (!/setText\(cmd\.trigger\)/.test(gcHtml39)) {
  console.error('FAIL · onSlashPick 没调 setText(cmd.trigger) (task 39)');
  process.exit(1);
}
