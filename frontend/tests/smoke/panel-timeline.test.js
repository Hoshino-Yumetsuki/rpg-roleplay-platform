...
try {
  html = ReactDOMServer.renderToString(
    React.createElement(global.PanelTimeline, { state: stateNoSave }),
  );
} catch (e) {
...
const stateWithSave = {
  _raw: { save_id: 42, save_title: '测试存档' },
  player: {},
  world: {},
  memory: {},
};
let html2;
try {
  html2 = ReactDOMServer.renderToString(
    React.createElement(global.PanelTimeline, { state: stateWithSave }),
  );
} catch (e) {
...
for (const [input, expected] of cases) {
  ...
  if (got !== expected)
    fail(
      `_renderVarValue(${JSON.stringify(input)}) → ${JSON.stringify(got)} ≠ ${JSON.stringify(expected)}`,
    );
}
