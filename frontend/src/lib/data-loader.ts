...
window.__guessKind = function () {
  return guessKind.apply(null, arguments);
};
...
async function hydratePlatform() {
  ...
  const branchSum = (platform.saves || []).reduce(
    (acc, s) => acc + (Number.isFinite(s.branch_count) ? s.branch_count : 0),
    0,
  );
  platform.stats = {
...
window.__normalizeScript = function (s) {
  return normalizeScript(s);
};
window.__normalizeSave = function (s) {
...
async function hydrateGameState() {
  ...
  try {
    ...
    if (!Array.isArray(merged.permissions.pending_questions))
      merged.permissions.pending_questions = [];
...
async function bootstrap() {
  ...
  window.RPG_AUTH = {
    ...
    online: true,
    user_id:
      authed && platform && platform.user && platform.user.id != null ? platform.user.id : null,
...
window.__refreshPlatform = async function () {
  ...
  try {
    const [{ platform, authed }, state] = await Promise.all([
      hydratePlatform(),
      hydrateGameState(),
    ]);
    window.MOCK_PLATFORM = platform;
    ...
    window.dispatchEvent(
      new CustomEvent('rpg-data-ready', { detail: { platform, state, authed } }),
    );
