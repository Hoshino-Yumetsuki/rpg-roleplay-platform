...
export function ModuleMatrixOverview({
  scriptId,
  status,
  loading,
  activeJobId,
  ...
  return (
    <div className={s.matrixRoot}>
      <div className={s.matrixHeader}>
        <h3 className={s.matrixTitle}>{t('modules.matrix.title', { defaultValue: '模块矩阵' })}</h3>
        <p className={s.matrixDesc}>
          {t('modules.matrix.desc', {
            defaultValue: '剧本所有派生数据的当前状态。每个模块可独立重做。',
          })}
        </p>
      </div>

      {loading && !status && <div className={s.matrixLoading}>{t('common.loading')}</div>}
