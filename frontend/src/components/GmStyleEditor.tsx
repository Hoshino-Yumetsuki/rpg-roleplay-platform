...
export default function GmStyleEditor({ scope = 'user', scriptId = null, canWrite = true }) {
  ...
  return (
    <CSSpaceBetween size="m">
        ...
        actions={
          canWrite ? (
            <CSSpaceBetween direction="horizontal" size="xs">
              {dirty && (
                <CSButton onClick={reset} disabled={saving}>
                  还原
                </CSButton>
              )}
              <CSButton variant="primary" onClick={save} loading={saving} disabled={!dirty}>
                保存
              </CSButton>
            </CSSpaceBetween>
          ) : undefined
      ...
      {err && (
        <CSAlert type="error" header="出错了">
          {err}
        </CSAlert>
      )}
      {okMsg && (
        <CSAlert type="success" dismissible onDismiss={() => setOkMsg('')}>
          {okMsg}
        </CSAlert>
      )}
