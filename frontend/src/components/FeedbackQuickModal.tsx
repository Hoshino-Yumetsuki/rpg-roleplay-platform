  ...
  return (
      ...
      footer={
        <CSBox float="right">
          <CSSpaceBetween direction="horizontal" size="xs">
            <CSButton variant="link" onClick={gotoFull}>
              查看全部反馈 / 历史
            </CSButton>
            <CSButton onClick={onClose}>关闭</CSButton>
            <CSButton variant="primary" onClick={submit} loading={busy} disabled={!canSubmit}>
              提交反馈
            </CSButton>
          </CSSpaceBetween>
        </CSBox>
    ...
    >
      <CSSpaceBetween size="m">
        {done ? (
          <CSAlert type="success" header="已收到你的反馈">
            感谢反馈!可在「查看全部反馈 / 历史」里跟进处理进度,或继续补充。
          </CSAlert>
        ) : (
          error && (
        ...
        <CSFormField
          label="问题 / 建议"
          description={`最多 ${MAX_FREE_TEXT} 字 · 复现步骤 / 期望 / 实际 越具体越好`}
        ...
        >
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--text-quiet, #968f85)', marginTop: 2 }}>
              想交流玩法 / 即时求助,欢迎加 QQ 群 {QQ_GROUP_NUMBER}。
            </div>
          </div>
          <CSButton variant="normal" iconName="external" href={QQ_JOIN_URL} target="_blank">
            加入群聊
          </CSButton>
        </div>
      </CSSpaceBetween>
