...
const IMPORT_JOB_TERMINAL_STATUSES = new Set([
  'done',
  'done_with_errors',
  'partial',
  'failed',
      ...
      >
        <header className="pl-modal-head">
          <div>
            <div className="pl-modal-eyebrow">
              {t('scripts.import.preview_eyebrow')} · {rule || t('scripts.import.rule_auto')}
            </div>
            <h2 className="pl-modal-title">
              {busy
                ? t('scripts.import.preview_splitting')
                : data?.title || t('scripts.import.unnamed')}
            </h2>
          </div>
            ...
            <div className="pl-validate-result" style={{ flex: '0 0 auto' }}>
              <div className="pl-validate-stat-row">
                ...
                <div className="pl-validate-stat">
                  ...
                  <span className="pl-stat-value" style={{ fontSize: 20 }}>
                    {(data.word_count / 10000).toFixed(1)}
                    <span style={{ fontSize: 12, color: 'var(--muted)', marginLeft: 3 }}>
                      {t('scripts.my.wan')}
                    </span>
                  </span>
                </div>
                <div className="pl-validate-stat">
                  ...
                  <span
                    ...
                    style={{
                      fontSize: 20,
                      color: data.confidence >= 0.85 ? 'var(--ok)' : 'var(--warn)',
                    }}
                  >
                ...
                <div className="pl-validate-stat">
                  ...
                  <span
                    ...
                    style={{
                      ...
                      color: data.problem_kind === 'ok' ? 'var(--ok)' : 'var(--warn)',
                    }}
                  >
                    {data.problem_label}
                  </span>
                ...
                >
                  {data.notes.map((n, i) => (
                    <li
                      key={i}
                      className="muted-2"
                      style={{ fontSize: 11.5, paddingLeft: 14, position: 'relative' }}
                    >
              ...
              <table className="pl-table" style={{ margin: 0 }}>
                <thead>
                  <tr>
                    ...
                    <th>{t('scripts.import.col_title')}</th>
                    <th>{t('scripts.import.col_volume')}</th>
                    <th style={{ textAlign: 'right' }}>{t('scripts.my.words')}</th>
                  </tr>
                </thead>
                <tbody>
                  {data.preview.map((p) => (
                    <tr
                      key={p.idx}
                      style={{ background: p.ok ? 'transparent' : 'var(--warn-soft)' }}
                    >
                      <td className="mono muted-2">{String(p.idx).padStart(3, '0')}</td>
                      <td>
                        <strong style={{ fontFamily: 'var(--font-serif)', fontSize: 14 }}>
                          {p.title}
                        </strong>
                        {!p.ok && (
                          <span className="pill warn" style={{ marginLeft: 8, fontSize: 10.5 }}>
                            <span className="dot warn" /> {p.hint}
                          </span>
                        )}
                      </td>
                      <td className="muted">{p.volume}</td>
                      <td className="mono" style={{ textAlign: 'right' }}>
                        {Number(p.words || 0).toLocaleString()}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <footer className="pl-modal-foot">
          <span className="muted-2" style={{ fontSize: 11.5 }}>
            ...
            {t('scripts.import.preview_footer', { count: data?.preview?.length || 0 })}
          </span>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {!busy && (
              <>
                <button
                  className="btn ghost"
                  onClick={() => onRetryRule?.('chapter_cn')}
                  data-tip={t('scripts.import.retry_tip')}
                >
...
function VersionHistoryDrawer({ script, currentUserId, onClose }) {
  ...
  const onRollback = async (commit) => {
    ...
    try {
      ...
      window.__apiToast?.(t('scripts.version.rollback_ok', { id: commit.id?.slice(0, 8) }), {
        kind: 'ok',
      });
      try {
      ...
      window.__apiToast?.(t('scripts.version.rollback_fail'), {
        kind: 'danger',
        detail: e?.message,
      });
    } finally {
      ...
      >
        ...
        >
          <CSBox variant="h3" padding="n">
            {t('scripts.version.drawer_title')} · {script.title}
          </CSBox>
          <CSButton variant="normal" iconName="close" onClick={onClose}>
            {t('common.close')}
          </CSButton>
        </div>
        <div style={{ flex: 1, padding: '12px 16px' }}>
          <CSTable
            ...
            columnDefinitions={[
              {
                id: 'commit',
                header: t('scripts.version.col_commit'),
                width: 110,
                cell: (c) => (
              ...
              {
                id: 'message',
                header: t('scripts.version.col_message'),
                cell: (c) => <CSBox fontSize="body-s">{c.message || '—'}</CSBox>,
              },
              {
                id: 'kind',
                header: t('scripts.version.col_kind'),
                width: 90,
                cell: (c) => (
              ...
              {
                id: 'date',
                header: t('scripts.version.col_date'),
                width: 130,
                cell: (c) => (
                  <CSBox fontSize="body-s" color="text-body-secondary">
              ...
              },
              {
                ...
                cell: (c) => (
                  ...
                  >
                    {t('scripts.version.rollback_btn')}
                  </CSButton>
                ),
              },
            ]}
            empty={
              <CSBox textAlign="center" padding={{ vertical: 'l' }} color="inherit">
                {t('scripts.version.empty')}
              </CSBox>
            }
          />
          {hasMore && (
            <div style={{ paddingTop: 12, textAlign: 'center' }}>
              <CSButton loading={loading} onClick={() => loadCommits(cursor)}>
...
function SharingModeSelector({ script, currentUserId, onChanged }) {
  ...
  const selectedCommitOpt =
    commitOptions.find((o) => o.value === pinCommitId) ||
    (pinCommitId ? { value: pinCommitId, label: pinCommitId.slice(0, 8) } : null);
      ...
      {mode === 'pinned-snapshot' && (
        <CSSpaceBetween direction="horizontal" size="xs" alignItems="flex-end">
          <CSFormField
            ...
            description={t('scripts.share.pin_commit_hint', {
              defaultValue:
                '选定版本作记录;当前 GM 检索按【目标剧本的最新内容】读取(精确版本回放为后续功能)。floating-latest 则始终跟随目标最新。',
            })}
            stretch
          >
            <CSSelect
          ...
          <CSButton
            loading={saving}
            disabled={!pinCommitId || saving}
            onClick={() => onSave('pinned-snapshot', pinCommitId)}
          >
...
function ScriptDetailPanel({
  ...
  useEffectPL(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          if (tab === 'world') setWb([]);
          else if (tab === 'npc') setNpc([]);
          else if (tab === 'timeline') setTl([]);
          else if (tab === 'params') setOv({});
        }
      } finally {
  ...
  const embedLabel = (() => {
    ...
    if (es.running)
      return t('scripts.my.embed_running', { pct: all ? Math.round((done / all) * 100) : 0 });
    return all > 0 && done >= all
      ? t('scripts.my.embed_done', { n: all })
      : t('scripts.my.embed_none');
  })();
  ...
  const embedWarning = (() => {
    ...
    if (
      (es.chunks?.done || 0) === 0 &&
      ((es.cards?.done || 0) > 0 || (es.worldbook?.done || 0) > 0)
    ) {
      return t('scripts.editor.embed_warn_chunks_missing', {
        defaultValue: '章节向量未建,RAG 退化到关键字',
      });
    }
    return null;
  })();
          ...
          <CSHeader
            ...
            actions={
              <CSSpaceBetween direction="horizontal" size="xs">
                <CSButton variant="primary" iconName="caret-right-filled" onClick={() => onPlay(s)}>
                  {t('scripts.my.play_game')}
                </CSButton>
                <CSButton iconName="file" onClick={() => onChapters(s)}>
                  {t('scripts.my.view_chapters')}
                </CSButton>
                <CSButton iconName="status-info" onClick={() => onReview(s)}>
                  {t('scripts.my.kb_review')}
                </CSButton>
                <CSButton iconName="settings" onClick={() => setHistoryOpen((v) => !v)}>
                  {t('scripts.version.history_btn')}
                </CSButton>
                <CSButtonDropdown
                  expandToViewport
                  items={[
                    {
                      id: 'embed',
                      text: es?.running ? t('scripts.my.embedding') : t('scripts.my.embed_start'),
                      iconName: 'search',
                      disabled: !!es?.running,
                    },
          ...
          <CSSpaceBetween size="s">
            <CSAlert
              ...
              action={
                forkConfirm ? (
                  <CSSpaceBetween direction="horizontal" size="xs">
                    <CSButton variant="primary" loading={forkBusy} onClick={doFork}>
                      {t('scripts.share.fork_btn')}
                    </CSButton>
                    <CSButton disabled={forkBusy} onClick={() => setForkConfirm(false)}>
                      {t('common.cancel', { defaultValue: '取消' })}
                    </CSButton>
                  </CSSpaceBetween>
                ) : (
                  <CSButton iconName="copy" onClick={() => setForkConfirm(true)}>
                    {t('scripts.share.fork_btn')}
                  </CSButton>
                )
              }
        ...
        <div data-detail-tabs>
          <CSTabs
            ...
            tabs={[
              {
                id: 'overview',
                label: t('scripts.editor.tab_overview'),
                content: (
                  <CSSpaceBetween size="l">
                    <CSKeyValuePairs
                      ...
                      items={[
                        {
                          label: t('scripts.my.chapters'),
                          value: (s.chapter_count || 0).toLocaleString(),
                        },
                        {
                          label: t('scripts.my.words'),
                          value: `${((s.word_count || 0) / 10000).toFixed(1)} ${t('scripts.my.wan')}`,
                        },
                        {
                          label: t('scripts.editor.split_mode'),
                          value: s.import_report?.mode_label || '—',
                        },
                        {
                          label: t('scripts.editor.split_confidence'),
                          value:
                        ...
                        {
                          label: t('scripts.editor.saves_count'),
                          value: t('scripts.editor.saves_n', { n: savesCount }),
                        },
                        {
                          label: t('scripts.editor.embed_index'),
                          value: (
                            <CSSpaceBetween direction="horizontal" size="xxs">
                              <span>{embedLabel}</span>
                              {embedWarning && (
                                <CSStatusIndicator type="warning">{embedWarning}</CSStatusIndicator>
                              )}
                            </CSSpaceBetween>
                          ),
                        },
                        {
                          label: t('scripts.my.share'),
                          value: s.is_public ? (
                            <CSStatusIndicator type="success">
                              {t('scripts.my.is_public')}
                            </CSStatusIndicator>
                          ) : (
                            <CSStatusIndicator type="stopped">
                              {t('scripts.editor.not_public')}
                            </CSStatusIndicator>
                          ),
                        },
                        {
                          label: t('scripts.editor.script_id'),
                          value: <span className="mono">{s.uid}</span>,
                        },
                      ]}
                    />
                    ...
                    <CSSpaceBetween size="s">
                      <CSHeader
                        variant="h3"
                        description={t('scripts.editor.embed_breakdown_desc', {
                          defaultValue: '向量索引按内容类型拆分,可选择性重嵌。',
                        })}
                      >
                        ...
                        {['chunks', 'cards', 'worldbook', 'canon'].map((kind) => {
                          ...
                          return (
                            <ModuleStatusCard
                              ...
                              totalCount={total}
                              activeJobId={
                                rb.activeJob ? rb.activeJob.job_id || rb.activeJob.id : null
                              }
                              title={t(`scripts.editor.embed_kind_${kind}`, { defaultValue: kind })}
                              description={t('scripts.editor.embed_kind_desc', {
                                defaultValue: 'pgvector embedding_vec 列',
                              })}
                              onRebuild={() =>
                                rb.openEstimate({
                                  module: 'embeddings',
                                  options: { include: [kind] },
                                })
                              }
                            />
                          );
              ...
              {
                id: 'params',
                label: t('scripts.editor.tab_params'),
                content: (
                  <CSSpaceBetween size="s">
                    <CSBox color="text-body-secondary" fontSize="body-s">
                      {t('scripts.editor.overrides_desc')}
                    </CSBox>
                    <pre
                      style={{
                        margin: 0,
                    ...
                    <CSButton iconName="edit" onClick={() => onEditOverrides(s)}>
                      {t('scripts.editor.edit_overrides')}
                    </CSButton>
                  </CSSpaceBetween>
                ),
              },
              {
                id: 'world',
                label: t('scripts.editor.tab_world'),
                content: (
                  <CSSpaceBetween size="l">
                    <ModuleStatusCard
                      ...
                      extraActions={
                        <CSSpaceBetween direction="horizontal" size="xxs">
                          <CSButton
                            iconName="add-plus"
                            onClick={() =>
                              rb.openEstimate({ module: 'worldbook', options: { source: 'canon' } })
                            }
                          >
                            {t('scripts.editor.wb_from_canon', {
                              defaultValue: '从知识库人物反推(免费)',
                            })}
                          </CSButton>
                          <CSButton
                            iconName="gen-ai"
                            variant="primary"
                            onClick={() =>
                              rb.openEstimate({ module: 'worldbook', options: { source: 'llm' } })
                            }
                          >
              ...
              {
                id: 'npc',
                label: t('scripts.editor.tab_npc'),
                content: (
                  <CSSpaceBetween size="l">
                    <ModuleStatusCard
                      {...rb.cardProps('cards')}
                      title={t('scripts.editor.tab_npc')}
                      description={t('scripts.editor.cards_desc', {
                        defaultValue: 'NPC 角色卡(可玩),与知识库人物条目不同',
                      })}
                    />
                    <CSCards
                      loading={loading && npc == null}
                      loadingText={t('scripts.editor.loading_npc')}
                      items={npc || []}
                      trackBy="id"
                        ...
                        sections: [
                          {
                            id: 'identity',
                            content: (c) => (
                              <CSBox color="text-label" fontSize="body-s" fontWeight="bold">
                          ...
                          },
                          {
                            id: 'meta',
                            content: (c) =>
                              c.first_revealed_chapter > 1 ||
                              c.importance != null ||
                              (Array.isArray(c.aliases) && c.aliases.length) ? (
                                <CSSpaceBetween direction="horizontal" size="xxs">
                                  {c.first_revealed_chapter > 1 && (
                                    <CSBadge color="blue">
                                      {t('scripts.editor.npc_chapter', {
                                        n: c.first_revealed_chapter,
                                      })}
                                    </CSBadge>
                                  )}
                                  {c.importance != null && (
                                    <CSBadge color="grey">
                                      {t('scripts.editor.npc_importance', { n: c.importance })}
                                    </CSBadge>
                                  )}
                                  {Array.isArray(c.aliases) &&
                                    c.aliases
                                      .slice(0, 3)
                                      .map((a) => <CSBadge key={a}>{a}</CSBadge>)}
                                </CSSpaceBetween>
                              ) : null,
                          },
                          {
              ...
              {
                id: 'canon-editor',
                label: t('scripts.editor.tab_canon', { defaultValue: '知识库人物' }),
                content: (
                  <CSSpaceBetween size="l">
                    <ModuleStatusCard
                      {...rb.cardProps('canon')}
                      description={t('scripts.editor.canon_desc', {
                        defaultValue:
                          'kb_canon_entities — LLM 抽出的人物/组织/地点等规范化条目;NPC 角色卡是另一码事。',
                      })}
                    />
                    <CSBox color="text-body-secondary" fontSize="body-s">
                      {t('scripts.editor.canon_editor_todo', {
                        defaultValue:
                          'kb_canon_entities 表格编辑器:见 /api/scripts/{id}/canon — 表格视图在另一 phase 落地。当前可用"重做"按钮重新抽取。',
                      })}
                    </CSBox>
                  </CSSpaceBetween>
                ),
              ...
              {
                id: 'timeline',
                label: t('scripts.editor.tab_timeline'),
                content: (
                  <CSSpaceBetween size="l">
                    <ModuleStatusCard
                      {...rb.cardProps('anchors')}
                      description={t('scripts.editor.anchors_desc', {
                        defaultValue:
                          'script_timeline_anchors,从 chapter_facts 的故事时间标签构建,零 LLM',
                      })}
                    />
                    {loading && tl == null ? (
                      ...
                      <CSBox textAlign="center" color="inherit" padding={{ vertical: 'l' }}>
                        {t('scripts.editor.timeline_empty')}
                      </CSBox>
                    ) : (
                      <CSSpaceBetween size="l">
                        {tl.map((p, i) => (
                          <div key={i}>
                            <CSBox variant="h4" padding="n">
                              {p.phase_label}{' '}
                              <CSBox
                                display="inline"
                                color="text-status-inactive"
                                fontSize="body-s"
                              >
                                {t('scripts.editor.chapter_range', {
                                  min: p.chapter_min,
                                  max: p.chapter_max,
                                })}
                              </CSBox>
                            ...
                            <CSSpaceBetween size="xxs">
                              {(p.anchors || []).map((a) => {
                                ...
                                const summary = String(a.sample_summary || '')
                                  .replace(/\s+/g, ' ')
                                  .trim();
                                return (
                                  <div
              ...
              {
                id: 'modules',
                label: t('scripts.editor.tab_modules', { defaultValue: '模块' }),
                content: (
              ...
              {
                id: 'extract',
                label: t('scripts.editor.tab_extract'),
                content: (
          ...
          <CardEditModal
            ...
            onSave={async (payload) => {
              try {
                ...
                window.__apiToast?.(
                  npcEdit.isNew ? t('scripts.toast.npc_added') : t('scripts.toast.npc_saved'),
                  { kind: 'ok' },
                );
                setNpcEdit(null);
                setNpc(null); // 触发 NPC 列表重新拉取
              } catch (e) {
                window.__apiToast?.(t('scripts.toast.save_fail'), {
                  kind: 'danger',
                  detail: e?.message,
                });
              }
...
function ScriptsLibraryView() {
  ...
  const onClone = async (s) => {
    ...
    try {
      ...
      setItems((arr) =>
        arr.map((x) => (x.id === s.id ? { ...x, clone_count: (x.clone_count || 0) + 1 } : x)),
      );
      try {
        window.dispatchEvent(new CustomEvent('rpg-scripts-updated'));
      } catch (_) {}
  ...
  return (
    <CSSpaceBetween size="l">
      <CSHeader
        ...
        actions={
          <CSButton iconName="refresh" onClick={() => reload(q)}>
            {t('common.refresh')}
          </CSButton>
        }
      >
        {t('scripts.public.title')}
      </CSHeader>
      ...
      <CSCards
        ...
        filter={
          <div style={{ minWidth: 320 }}>
            <CSTextFilter
              filteringText={q}
              filteringPlaceholder={t('scripts.public.search_placeholder')}
        ...
        empty={
          <CSBox textAlign="center" color="inherit" padding={{ vertical: 'l' }}>
            {loading
              ? t('common.loading')
              : q
                ? t('scripts.public.empty_search')
                : t('scripts.public.empty')}
          </CSBox>
        }
        cardDefinition={{
          header: (s) => (
            <CSSpaceBetween direction="horizontal" size="xs" alignItems="center">
              <CSBox key="t" variant="h3" padding="n">
                {s.title}
              </CSBox>
              {(s.mine || importedIds[s.id]) && (
                <CSBadge key="b" color="green">
                  {s.mine ? t('scripts.public.mine_badge') : t('scripts.public.imported_badge')}
                </CSBadge>
              )}
            </CSSpaceBetween>
          ),
          sections: [
            {
              ...
              content: (s) => (
                <CSBox fontSize="body-s" color="text-body-secondary">
                  {t('scripts.public.shared_by', {
                    author: s.author || s.author_username || t('scripts.public.anon'),
                  })}
                </CSBox>
              ),
            },
            {
              ...
              content: (s) => (
                <CSSpaceBetween direction="horizontal" size="xs">
                  <CSBadge key="ch">
                    {t('scripts.public.stat_chapters', {
                      n: (s.chapter_count || 0).toLocaleString(),
                    })}
                  </CSBadge>
                  <CSBadge key="wd">
                    {t('scripts.public.stat_words', {
                      n: ((s.word_count || 0) / 10000).toFixed(0),
                    })}
                  </CSBadge>
                  <CSBadge key="cl" color="grey">
                    {t('scripts.public.stat_clones', { n: s.clone_count || 0 })}
                  </CSBadge>
                </CSSpaceBetween>
              ),
            },
            {
              id: 'desc',
              content: (s) =>
                s.description ? <CSBox color="text-body-secondary">{s.description}</CSBox> : null,
            },
            {
              id: 'actions',
              content: (s) =>
                s.mine || importedIds[s.id] ? (
                  <CSButton disabled iconName="check">
                    {s.mine ? t('scripts.public.is_mine') : t('scripts.public.imported_badge')}
                  </CSButton>
                ) : (
                  <CSButton
                    variant="primary"
...
function ScriptsListView() {
  ...
  const triggerEmbed = React.useCallback(async (sid) => {
    try {
      ...
      if (j.ok === false) {
        ...
        const isCredsError =
          j.code === 'credentials_required' ||
          j.error_key === 'credentials_required' ||
          j.needs_credentials;
        if (isCredsError) {
          ...
          window.__apiToast?.(t('scripts.toast.embed_no_embedder'), {
            ...
            duration: 8000,
            action: {
              label: t('scripts.import.go_api_settings'),
              onClick: () => {
          ...
          window.__apiToast?.(t('scripts.toast.embed_fail'), {
            kind: 'danger',
            detail: j.error || t('scripts.toast.unknown_error'),
            duration: 5000,
          });
        }
        return;
      }
      window.toast?.(t('scripts.toast.embed_started'), {
        kind: 'ok',
        detail: t('scripts.toast.embed_started_detail'),
        duration: 3000,
      });
  ...
  useEffectPL(() => {
    const runningIds = Object.entries(embedStatus)
    ...
    const iv = setInterval(async () => {
      for (const sid of runningIds) {
        try {
          const r = await fetch(`${window.__API_BASE || ''}/api/scripts/${sid}/embed/status`, {
            credentials: 'include',
          });
          const j = await r.json();
          if (j.ok && j.status) {
  ...
  const reload = React.useCallback(async () => {
    try {
      ...
      Promise.all(
        normed.map(async (s) => {
          try {
            const sr = await fetch(`${window.__API_BASE || ''}/api/scripts/${s.id}/embed/status`, {
              credentials: 'include',
            });
  ...
  const onDelete = async (s) => {
    if (
      !(await window.__confirm({
        title: t('scripts.confirm.delete_title'),
        message: t('scripts.confirm.delete_msg', { title: s.title }),
        danger: true,
  ...
  const onImportPackFile = async (file) => {
    ...
    try {
      ...
      if (result && result.ok === false)
        throw new Error(result.error || result.detail || t('scripts.toast.import_fail'));
      const sid = result?.script_id;
      const warnings = result?.warnings;
      window.__apiToast?.(t('scripts.toast.pack_import_ok'), {
        kind: 'ok',
        detail: warnings?.length
          ? t('scripts.toast.pack_warnings', { msg: warnings.join('; ') })
          : sid
            ? `script #${sid}`
            : '',
      });
  ...
  const rowActions = (s) => {
    ...
    const embedText = running
      ? t('scripts.my.embed_running', { pct })
      : fullyDone
        ? t('scripts.my.embed_done', { n: totalAll })
    ...
    return [
      ...
      { id: 'review', text: t('scripts.my.action_review'), iconName: 'status-info' },
      {
        id: 'embed',
        text: embedText,
        iconName: fullyDone ? 'status-positive' : 'gen-ai',
        disabled: !!running,
      },
      {
        id: 'visibility',
        text: s.is_public ? t('scripts.my.action_unpublish') : t('scripts.my.action_publish'),
        iconName: s.is_public ? 'lock-private' : 'share',
      },
      {
        id: 'export',
        text: t('scripts.my.action_export'),
        iconName: 'download',
  ...
  const onToggleVisibility = async (s) => {
    ...
    if (next) {
      ...
      if ((s.review_status || 'unreviewed') !== 'reviewed') {
        window.__apiToast?.('分享前需先核对剧本设定', {
          kind: 'warn',
          detail:
            '已为你打开「设定核对」,确认 AI 提取的人物/世界观/时间线无误后点「确认设定无误」,再回来分享。',
          duration: 5500,
        });
        setReviewScript(s);
        return;
      ...
      if (
        !(await window.__confirm({
          title: t('scripts.confirm.publish_title'),
          message: t('scripts.confirm.publish_msg', { title: s.title }),
          confirmText: t('scripts.confirm.publish_btn'),
        }))
      )
        return;
    }
    try {
      ...
      window.__apiToast?.(next ? t('scripts.toast.published') : t('scripts.toast.unpublished'), {
        kind: 'ok',
        duration: 2000,
      });
      ...
      if (e?.payload?.error === 'REVIEW_REQUIRED') {
        window.__apiToast?.('分享前需先核对剧本设定', {
          kind: 'warn',
          detail: e?.payload?.message || '请先在「设定核对」确认设定无误。',
          duration: 5500,
        });
        setReviewScript(s);
        return;
  ...
  const pagedScripts = visibleScripts.slice(
    (scriptPage - 1) * SCRIPT_PAGE_SIZE,
    scriptPage * SCRIPT_PAGE_SIZE,
  );
  // 查询变化时重置到第 1 页
  React.useEffect(() => {
    setScriptPage(1);
  ...
  const isInternalPlaceholder = (s) =>
    s && typeof s.title === 'string' && s.title.startsWith('[内部]');
    ...
    isInternalPlaceholder(selected) ? (
      <CSContainer header={<CSHeader variant="h2">{selected.title}</CSHeader>}>
        <div style={{ padding: '36px 20px', textAlign: 'center' }}>
          ...
          <div
            style={{ fontSize: 13.5, color: 'var(--muted)', maxWidth: 480, margin: '0 auto 8px' }}
          >
            我们正在开发 D&amp;D 5E 规则模组容器，提供骰子裁定 / 法术 / 物品 / 角色升级 /
            战斗回合等结构化游玩。
          </div>
          <div style={{ fontSize: 12, color: 'var(--muted-2)', marginTop: 16 }}>
            预计公测后开放 · 如有建议请通过右上"提交反馈"告知我们
          </div>
        </div>
      </CSContainer>
    ) : (
  ...
  const tableEl = (
    <CSTable
      ...
      onSelectionChange={({ detail }) => {
        ...
        if (x) setSelectedId(x.id);
      }}
      onRowClick={({ detail }) => setSelectedId(detail.item.id)}
      empty={
        <CSBox textAlign="center" color="inherit" padding={{ vertical: 'l' }}>
          {query ? t('scripts.my.empty_search') : t('scripts.my.empty')}
        </CSBox>
      }
      pagination={
        scriptPageCount > 1 ? (
          <CSPagination
            currentPageIndex={scriptPage}
            pagesCount={scriptPageCount}
            onChange={({ detail }) => setScriptPage(detail.currentPageIndex)}
          />
        ) : undefined
      }
      columnDefinitions={[
        {
          id: 'title',
          header: t('scripts.my.col_script'),
          cell: (s) =>
            isInternalPlaceholder(s) ? (
              <div style={{ opacity: 0.55 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <CSBox fontWeight="bold" color="text-status-inactive">
                    {s.title}
                  </CSBox>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                  ...
                  {s.sharing_mode === 'floating-latest' && (
                    <CSBadge color="blue">{t('scripts.share.badge_floating')}</CSBadge>
                  )}
                  {s.sharing_mode === 'pinned-snapshot' && (
                    <CSBadge color="grey">
                      {t('scripts.share.badge_pinned', {
                        id: (s.current_pin_commit_id || '').slice(0, 7),
                      })}
                    </CSBadge>
                  )}
                  {s.sharing_mode === 'public' && (
                    <CSBadge color="green">{t('scripts.share.badge_public')}</CSBadge>
                  )}
                  {s.forked_from_script_id && <CSBadge color="severity-neutral">fork</CSBadge>}
                </div>
                <CSBox fontSize="body-s" color="text-body-secondary">
                  {s.uid} · {t('scripts.my.updated')} {s.updated_at}
                </CSBox>
              </div>
            ),
        },
        {
          id: 'chapters',
          header: t('scripts.my.chapters'),
          cell: (s) =>
            isInternalPlaceholder(s) ? (
              <CSBox color="text-status-inactive">—</CSBox>
            ) : (
              (s.chapter_count || 0).toLocaleString()
            ),
        },
        {
          id: 'words',
          header: t('scripts.my.words'),
          cell: (s) =>
            isInternalPlaceholder(s) ? (
              <CSBox color="text-status-inactive">—</CSBox>
            ) : (
              `${((s.word_count || 0) / 10000).toFixed(1)} ${t('scripts.my.wan')}`
            ),
        },
        {
          id: 'mode',
          header: t('scripts.my.split_mode'),
          cell: (s) =>
            isInternalPlaceholder(s) ? (
              <CSBox color="text-status-inactive">—</CSBox>
            ) : (
        ...
        {
          id: 'problem',
          header: t('scripts.my.problem'),
          cell: (s) => {
            if (isInternalPlaceholder(s))
              return <CSStatusIndicator type="pending">开发中</CSStatusIndicator>;
            const r = s.readiness || null;
            // phase_rebuild_panel: 没 readiness 字段就不撒谎"就绪",改返 unknown 占位 — 别让破壳数据冒充 ready
            if (!r) {
              if (
                s.import_report?.problem_label &&
                s.import_report.problem_label !== t('scripts.my.no_problem')
              ) {
                return (
        ...
        },
        {
          ...
          cell: (s) => {
            ...
            return n > 0 ? (
              <CSBadge color="green">{t('scripts.my.saves_count', { n })}</CSBadge>
            ) : (
              <CSBox color="text-status-inactive">—</CSBox>
            );
          },
        ...
        {
          id: 'public',
          header: t('scripts.my.share'),
          cell: (s) =>
            s.is_public ? (
              <CSStatusIndicator type="success">{t('scripts.my.is_public')}</CSStatusIndicator>
            ) : (
              <CSBox color="text-status-inactive">—</CSBox>
            ),
        },
        {
          ...
          cell: (s) =>
            isInternalPlaceholder(s) ? (
              <CSButton variant="inline-link" iconName="status-pending" disabled>
                {t('scripts.my.play')}
              </CSButton>
            ) : (
              <CSButton
  ...
  return (
    <CSSpaceBetween size="l">
      ...
      <input
        ...
        accept=".zip"
        style={{ display: 'none' }}
        onChange={(e) => onImportPackFile(e.target.files?.[0])}
      />
      <CSHeader
        variant="h1"
        ...
        actions={
          <CSSpaceBetween direction="horizontal" size="xs">
            <CSButton
              iconName="download"
              loading={importPackBusy}
              onClick={() => importPackRef.current?.click()}
            >
              {t('scripts.my.import_pack')}
            </CSButton>
            <CSButton variant="primary" iconName="upload" onClick={() => setImportOpen(true)}>
              {t('scripts.my.import_script')}
            </CSButton>
          </CSSpaceBetween>
        }
      >
      ...
      <div style={{ maxWidth: 360 }}>
        <CSTextFilter
          filteringText={query}
          filteringPlaceholder={t('scripts.my.search_placeholder')}
          onChange={({ detail }) => setQuery(detail.filteringText)}
        />
      </div>

      {selected ? <ResizableSplit storageKey="scripts" top={tableEl} bottom={detailEl} /> : tableEl}

      <ChaptersModal
        script={chaptersOpen}
        onClose={() => setChaptersOpen(null)}
        onChanged={reload}
      />
      {importOpen && (
        <div
          ...
          >
            <header className="pl-modal-head">
              <div>
                ...
                <h2 className="pl-modal-title">
                  {reviewScript.title || t('scripts.review.script_id', { id: reviewScript.id })}
                </h2>
              </div>
              <button
                className="iconbtn"
                onClick={() => setReviewScript(null)}
            ...
            <ScriptReview
              ...
              onReviewedChange={(sid, rs) => {
                ...
                setScripts((arr) =>
                  arr.map((x) => (x.id === sid ? { ...x, review_status: rs } : x)),
                );
                setReviewScript((cur) =>
                  cur && cur.id === sid ? { ...cur, review_status: rs } : cur,
                );
              }}
            />
          </div>
    ...
    <div className="pl-modal-backdrop" onClick={onClose}>
      ...
      >
        <header className="pl-modal-head">
          <div>
            <div className="pl-modal-eyebrow">
              {t('scripts.editor.overrides_eyebrow')} · {script.title}
            </div>
            <h2 className="pl-modal-title">
              {loading ? t('common.loading') : 'script_overrides JSONB'}
            </h2>
          </div>
          <button className="iconbtn" onClick={onClose} data-tip={t('common.close')}>
            <Icon name="close" size={14} />
        ...
        {err && (
          <div style={{ padding: '8px 16px', color: 'var(--danger)', fontSize: 13 }}>{err}</div>
        )}
        {!loading && (
          <div
            style={{
          ...
          >
            <div
              style={{ fontSize: 11.5, color: 'var(--muted-2)', marginBottom: 6, paddingTop: 12 }}
            >
              {t('scripts.editor.overrides_hint')}
              {!jsonValid && (
                <span style={{ color: 'var(--danger)', marginLeft: 8 }}>
                  {t('scripts.editor.json_invalid')}
                </span>
              )}
            </div>
            <textarea
        ...
        <footer className="pl-modal-foot" style={{ marginTop: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            >
              {saving ? (
                <>
                  <Icon name="spinner" size={12} className="spin" /> {t('scripts.editor.saving')}
                </>
              ) : (
                <>
...
function ChaptersModal({ script, onClose, onChanged }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setErr(e?.message || t('scripts.editor.fetch_fail'));
      } finally {
        setLoading(false);
      }
    })();
  ...
  const onRename = async () => {
    if (!cur) return;
    const newTitle = await window.__prompt({
      title: t('scripts.editor.rename_title'),
      label: t('scripts.editor.rename_label'),
      default: cur.title || '',
    });
  ...
  const onMergeNext = async () => {
    ...
    if (
      !(await window.__confirm({
        title: t('scripts.editor.merge_title'),
        message: t('scripts.editor.merge_msg', { a: activeIdx + 1, b: activeIdx + 2 }),
        confirmText: t('scripts.editor.merge_btn'),
      }))
    )
      return;
    try {
      const nextCh = chapters[activeIdx + 1];
      const nextIdx = nextCh
        ? (nextCh.chapter_index ?? nextCh.index ?? activeIdx + 1)
        : activeIdx + 1;
      await window.api.scripts.mergeChapter(script.id, {
        first_index: curIdx,
        second_index: nextIdx,
      });
  ...
  const onSplit = async () => {
    if (!cur) return;
    const pos = await window.__prompt({
      title: t('scripts.editor.split_title'),
      label: t('scripts.editor.split_label'),
      default: '',
    });
  ...
  const onResplit = async (vals) => {
    try {
      await window.api.scripts.resplit(script.id, {
        split_rule: vals.rule || 'auto',
        custom_pattern: vals.pattern || '',
      });
      ...
      window.__apiToast?.(t('scripts.toast.resplit_fail'), { kind: 'danger', detail: e?.message });
    }
  };
  return (
    <div className="pl-modal-backdrop" onClick={onClose}>
      ...
      >
        <header className="pl-modal-head">
          <div>
            <div className="pl-modal-eyebrow">
              {t('scripts.editor.chapters_eyebrow')} · {script.title}
            </div>
            <h2 className="pl-modal-title">
              {loading
                ? t('common.loading')
                : t('scripts.editor.chapters_title', {
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button
              className="btn ghost"
              onClick={() => setResplitOpen(true)}
              title={t('scripts.editor.resplit_tip')}
            >
              <Icon name="refresh" size={12} /> {t('scripts.editor.resplit_btn')}
            </button>
            <button className="iconbtn" onClick={onClose} data-tip={t('common.close')}>
              <Icon name="close" size={14} />
            </button>
        ...
        {err && (
          <div className="pl-model-empty" style={{ padding: '16px' }}>
            <span className="danger">{t('scripts.editor.load_fail_detail', { err })}</span>
          </div>
        )}
        {!err && chapters.length === 0 && !loading && (
          <div className="pl-model-empty" style={{ padding: '24px' }}>
            {t('scripts.editor.chapters_empty')}
          </div>
        )}
        {chapters.length > 0 && (
          <div
          ...
          >
            ...
            >
              {chapters.map((c, i) => (
                ...
                >
                  <span className="muted-2 mono" style={{ minWidth: 36, fontSize: 11 }}>
                ...
                <>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                    <strong style={{ fontSize: 15 }}>
                      {cur.title || t('scripts.editor.unnamed_chapter')}
                    </strong>
                    ...
                    <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
                      <button className="btn ghost" onClick={onRename}>
                        <Icon name="edit" size={12} /> {t('scripts.editor.rename_btn')}
                      </button>
                      <button className="btn ghost" onClick={onSplit}>
                        <Icon name="branch" size={12} /> {t('scripts.editor.split_chapter_btn')}
                      </button>
                      {activeIdx < chapters.length - 1 && (
                        <button className="btn ghost" onClick={onMergeNext}>
                          <Icon name="link" size={12} /> {t('scripts.editor.merge_next_btn')}
                        </button>
                      )}
                    </div>
                  </div>
                  ...
                  >
                    {activeLoading
                      ...
                      : (activeContent || cur.content_preview || '').slice(0, 8000) +
                        (activeContent && activeContent.length > 8000
                          ? t('scripts.editor.content_truncated')
                          : '')}
                  </pre>
                </>
              )}
        ...
        <footer className="pl-modal-foot">
          <span className="muted-2" style={{ fontSize: 11.5 }}>
            <Icon name="info" size={11} /> GET /api/scripts/{script.id}/chapters · POST /chapters/
            {`{idx}`} / merge / split / resplit
          </span>
          <button className="btn ghost" onClick={onClose}>
            {t('common.close')}
          </button>
      ...
      <PromptModal
        ...
        fields={[
          {
            key: 'rule',
            label: t('scripts.import.field_rule'),
            type: 'select',
            default: 'auto',
            options: [
          ...
          {
            key: 'pattern',
            label: t('scripts.import.field_custom_regex'),
            placeholder: t('scripts.import.field_custom_regex_placeholder'),
          },
        ]}
...
const IMPORT_STAGES = [
  {
    id: 'split',
    labelKey: 'scripts.import.stage_split',
    hintKey: 'scripts.import.stage_split_hint',
    tok_per_chap: 0,
  },
  {
    id: 'save',
    labelKey: 'scripts.import.stage_save',
    hintKey: 'scripts.import.stage_save_hint',
    tok_per_chap: 0,
  },
  {
    id: 'extract',
    labelKey: 'scripts.import.stage_extract',
    hintKey: 'scripts.import.stage_extract_hint',
    tok_per_chap: 120,
  },
  {
    id: 'card',
    labelKey: 'scripts.import.stage_card',
    hintKey: 'scripts.import.stage_card_hint',
    tok_per_chap: 60,
  },
  {
    id: 'world',
    labelKey: 'scripts.import.stage_world',
    hintKey: 'scripts.import.stage_world_hint',
    tok_per_chap: 90,
  },
  {
    id: 'timeline',
    labelKey: 'scripts.import.stage_timeline',
    hintKey: 'scripts.import.stage_timeline_hint',
    tok_per_chap: 40,
  },
];
...
function ScriptsImportView({ embedded = false, onClose } = {}) {
  ...
  const persistPendingPipeline = useCallbackPL((item) => {
    ...
    try {
      localStorage.setItem(PENDING_IMPORT_PIPELINE_KEY, JSON.stringify(payload));
    } catch {}
  }, []);
  ...
  const clearPendingPipeline = useCallbackPL(() => {
    ...
    try {
      localStorage.removeItem(PENDING_IMPORT_PIPELINE_KEY);
    } catch {}
  }, []);
  ...
  const onPickFile = (file) => {
    ...
    if (!/\.(txt|md)$/.test(name)) {
      window.__apiToast?.('仅支持 .txt / .md 剧本文件', {
        kind: 'danger',
        detail: '测试阶段已禁用其他文件类型上传',
        duration: 2800,
      });
      return;
    }
    if (file.size > 50 * 1024 * 1024) {
      window.__apiToast?.(t('scripts.import.file_too_large'), {
        kind: 'danger',
        detail: t('scripts.import.file_max_size'),
        duration: 2400,
      });
      return;
    }
  ...
  const buildRunningStages = (baseStages) => {
    const source =
      Array.isArray(baseStages) && baseStages.length
        ? baseStages
        : IMPORT_STAGES.map((s) => ({
            id: s.id,
            label: t(s.labelKey),
  ...
  const resumePendingPipeline = async () => {
    ...
    try {
      ...
      setJob({
        id: resp.job_id,
        file: pendingPipeline.file || {
          name: pendingPipeline.file_name || pendingPipeline.title || 'script',
        },
      ...
      if (isCredentialsRequiredError(e)) {
        ...
        const detail =
          ...
          t('scripts.toast.unknown_error');
        window.__apiToast?.(t('scripts.import.pipeline_resume_fail'), {
          kind: 'danger',
          detail,
          duration: 5000,
        });
  ...
  const startEstimate = async () => {
    ...
    try {
      ...
      if (uploadId) {
        try {
          await window.api.uploads.cancel(uploadId);
        } catch (_) {}
      }
      let detail =
        (e && (e.message || (e.payload && (e.payload.error || e.payload.detail)))) ||
      ...
      const isNetErr =
        (e && (e.code === 'network' || e.status === 0)) ||
        /Failed to fetch|NetworkError|网络异常/i.test(String(detail));
      if (isNetErr) {
        detail = `${detail} —— 若为自建/反向代理部署,请检查后端是否在运行,以及 nginx/caddy 的 client_max_body_size(建议 ≥ 50m)。`;
      }
      window.__apiToast?.(t('scripts.toast.preview_fail'), {
        kind: 'danger',
        detail,
        duration: 8000,
      });
    ...
    // 成功路径：用后端真实数字
    const chapters =
      Number(result.total_chapters) || (Array.isArray(result.preview) ? result.preview.length : 0);
    ...
    if (result.report && result.report.mode_label) {
      warnings.push(
        `切分模式：${result.report.mode_label}（置信 ${result.report.confidence ?? '—'}）`,
      );
    }
    setEstimate({
  ...
  const startImport = async () => {
    ...
    if (selectedFile) {
      ...
      setImportPercent(0);
      setImportProgress(
        uploadId ? t('scripts.import.import_reuse_upload') : t('scripts.import.upload_init'),
      );
      try {
        ...
        if (!uploadId) {
          uploadId = await uploadFileChunks(selectedFile, ({ stage, done, total, percent }) => {
            if (stage === 'init') {
              ...
              setImportPercent(Math.min(30, Math.round((percent || 0) * 0.3)));
        ...
        if (!importResp || importResp.ok === false) {
          throw new Error(
            (importResp && (importResp.error || importResp.detail)) || t('scripts.import.api_fail'),
          );
        }
        ...
        try {
          ...
          if (!pipelineResp || pipelineResp.ok === false || !pipelineResp.job_id) {
            throw new Error(
              (pipelineResp && (pipelineResp.error || pipelineResp.detail)) ||
                t('scripts.import.api_fail'),
            );
          }
          ...
          if (isCredentialsRequiredError(e)) {
            ...
            const modeLabel = (() => {
              const _r = SPLIT_RULES.find((r) => r.id === rule);
              return _r ? t(_r.labelKey) : rule;
            })();
            pipelinePaused = {
              script_id: sc.id,
              title: createdTitle,
        ...
        if (pipelinePaused) {
          ...
          window.toast &&
            window.toast(t('scripts.toast.import_ok'), {
              kind: 'ok',
    ...
    if (estimate && estimate.isMockEstimate) {
      window.__apiToast?.(t('scripts.toast.mock_warn'), {
        kind: 'warn',
        detail: t('scripts.toast.mock_warn_detail'),
        duration: 3000,
      });
      return;
    }
    ...
    try {
      ...
      setJob({
        ...
        file: pendingImport.file || {
          name: pendingImport.file_name || pendingImport.title || 'script',
        },
        title: sc.title || pendingImport.title || pendingImport.file_name || 'script',
        script_id: sc.id,
        mode: (() => {
          const _r = SPLIT_RULES.find((r) => r.id === (pendingImport.split_rule || 'auto'));
          return _r ? t(_r.labelKey) : pendingImport.split_rule || 'auto';
        })(),
        stages,
  ...
  const cancelJob = async () => {
    ...
    if (job.real) {
      try {
        await window.api.scripts.jobCancel(job.id);
      } catch (e) {}
    }
    setJob((j) => ({ ...j, status: 'cancelled', cancelled_at: Date.now() }));
    window.toast?.(t('scripts.toast.import_cancelled'), {
      kind: 'warn',
      detail: 'job ' + job.id,
      duration: 2400,
    });
  ...
  const onJobSseDone = useCallbackPL(() => {
    setJob((prev) => {
      ...
      const hasErr =
        errored.length > 0 || prev.status === 'failed' || prev.status === 'done_with_errors';
      const detail = errored.length
        ? errored
            .map(
              (s) =>
                (s.id || s.label || '?') + ': ' + (s.error || t('scripts.toast.unknown_error')),
            )
            .join('; ')
        : prev.error || '';
      if (prev.status === 'cancelled') {
        ...
        window.__apiToast?.(t('scripts.toast.import_fail'), {
          kind: 'danger',
          detail: detail || t('scripts.toast.unknown_error'),
          duration: 5000,
        });
      } else if (hasErr) {
        window.__apiToast?.(t('scripts.toast.import_partial'), {
          kind: 'warning',
          detail,
          duration: 6000,
        });
      } else {
        window.__apiToast?.(t('scripts.toast.import_ok'), {
          kind: 'ok',
          detail: t('scripts.toast.import_ok_detail', {
            id: prev.script_id || '?',
            title: prev.title || '',
          }),
          duration: 3000,
  ...
  const fileName =
    ...
    (estimate && estimate.file && estimate.file.name) ||
    null;
    ...
    <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <CSSpaceBetween size="l">
          ...
          {pendingImport && !jobRunning && (
            ...
            >
              {t('scripts.import.api_key_required_preimport_body', {
                title:
                  pendingImport.title || pendingImport.file_name || t('scripts.import.unnamed'),
          ...
          {pendingPipeline && !jobRunning && (
            ...
            >
              {t('scripts.import.api_key_required_body', {
                title:
                  pendingPipeline.title || pendingPipeline.file_name || t('scripts.import.unnamed'),
          ...
          <CSContainer
            header={
              <CSHeader variant="h2" description={t('scripts.import.basic_desc')}>
                {t('scripts.import.basic_title')}
              </CSHeader>
            }
          >
            <CSColumnLayout columns={2}>
              <CSFormField
                label={t('scripts.import.field_title')}
                description={t('scripts.import.field_title_desc')}
              >
                <CSInput
                  value={title}
                  onChange={({ detail }) => setTitle(detail.value)}
                  placeholder={t('scripts.import.field_title_desc')}
                />
              </CSFormField>
              <CSFormField label={t('scripts.import.field_rule')}>
                <CSSelect
                  selectedOption={{ value: ruleOpt.id, label: ruleLabel }}
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                <CSFormField
                  label={t('scripts.import.field_custom_regex')}
                  description={t('scripts.import.field_custom_regex_desc')}
                >
                  <CSInput
                    value={pattern}
          ...
          {embedderStatus &&
            embedderStatus.effective_source === 'none' &&
            !embedderStatus.preflight?.ok && (
              <CSAlert
                type="info"
                header={t('scripts.import.embedder_not_configured_title', {
                  defaultValue: '未配置 RAG / 向量嵌入模型（可选，但建议配置）',
                })}
                action={
                  <CSButton
                    iconName="settings"
          ...
          {embedderStatus && embedderStatus.preflight?.last_error_hint && (
            <CSAlert
              type="warning"
              header={t('scripts.import.embedder_error_title', {
                defaultValue: '向量嵌入配置可能有问题',
              })}
              action={
                <CSButton
                  iconName="settings"
          ...
          <CSContainer
            header={
              <CSHeader
                variant="h2"
                description="提取章节摘要 / NPC 角色卡 / 世界书所用的 LLM。仅在导入这一步生效;玩游戏时的主 GM 在另一处配置。这里改完会保存到你的「设置 → 模型管理 → 提取器」偏好(import-pipeline 后端读 prefs,不是当场传参)。"
              >
                提取模型
              </CSHeader>
            }
          ...
          <CSContainer
            header={
              <CSHeader variant="h2" description={t('scripts.import.pipeline_options_desc')}>
          ...
          >
            <CSColumnLayout columns={2}>
              <CSFormField
                label={t('scripts.import.enable_cards_label')}
                description={t('scripts.import.enable_cards_desc')}
              >
                <CSToggle
                  checked={enableCards}
                  onChange={({ detail }) => setEnableCards(detail.checked)}
                >
                  {enableCards ? t('common.enabled') : t('common.disabled')}
                </CSToggle>
              </CSFormField>
              <CSFormField
                label={t('scripts.import.enable_worldbook_label')}
                description={t('scripts.import.enable_worldbook_desc')}
              >
                <CSToggle
                  checked={enableWorldbook}
                  onChange={({ detail }) => setEnableWorldbook(detail.checked)}
                >
          ...
          <CSContainer
            header={
              <CSHeader variant="h2" description={t('scripts.import.file_desc')}>
                {t('scripts.import.file_title')}
              </CSHeader>
            }
          >
            <CSFileUpload
          ...
          {estimate && !job && <ImportEstimateView estimate={estimate} rule={rule} hideActions />}
        </CSSpaceBetween>
      </div>
              ...
              items={[
                ...
                ...(estimate
                  ? [
                      ...
                      {
                        label: t('scripts.my.words'),
                        value: `${(estimate.words / 10000).toFixed(1)} ${t('scripts.my.wan')}`,
                      },
                      {
                        label: t('scripts.import.est_cost'),
                        value: (
                          <CSBox color="text-status-info" fontWeight="bold">
                            ${estimate.cost.toFixed(2)}
                          </CSBox>
                        ),
                      },
                      {
                        label: t('scripts.import.est_time'),
                        value: t('scripts.import.est_time_val', {
                          min: Math.round(estimate.totalSec / 60),
                        }),
                      },
                    ]
                  : []),
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {!estimate && (
                <CSButton
                  ...
                  iconName="search"
                  loading={previewBusy}
                  disabled={!selectedFile || !!job || importBusy}
                  onClick={startEstimate}
                >
                  {previewBusy
                    ? t('scripts.import.calculating')
                    : t('scripts.import.preview_split')}
                </CSButton>
              )}
              {previewBusy && (
                <CSProgressBar
              ...
              {estimate && !job && (
                <>
                  ...
                  >
                    {importBusy
                      ? t('scripts.import.import_creating')
                      : t('scripts.import.confirm_import_bg')}
                  </CSButton>
                  <CSButton disabled={importBusy} onClick={() => setEstimate(null)}>
                    {t('scripts.import.re_estimate')}
                  </CSButton>
                </>
              )}
              {importBusy && (
    ...
    <CSContainer
      header={
        <CSHeader
          ...
          actions={
            <CSButton iconName="close" onClick={onCancel}>
              {t('scripts.import.cancel_import')}
            </CSButton>
          }
        >
      ...
      <CSSpaceBetween size="m">
        {isQueued ? (
          ...
          <div>
            ...
            <div
              className="pl-import-progress-bar"
          ...
          <CSProgressBar
            ...
            additionalInfo={
              currentStage
                ? `${currentStage}${stageTotal ? ` ${stageProgress}/${stageTotal}` : ''}`
                : ''
            }
            status="in-progress"
          />
        ...
        {!isQueued && (
          ...
          >
            {stages.map((s, i) => {
              ...
              else if (s.status === 'running' && s.id === currentStage && stageTotal)
                meta = `${stageProgress}/${stageTotal}`;
              const errDetail = s.status === 'error' || s.status === 'failed' ? s.error || '' : '';
              return (
                <div key={s.id || i}>
                  <CSStatusIndicator type={type}>
                    {String(i + 1).padStart(2, '0')} · {s.label || s.id}
                  </CSStatusIndicator>
                  <CSBox
                    fontSize="body-s"
                    color={errDetail ? 'text-status-error' : 'text-body-secondary'}
                  >
        ...
        {usage && (
          <CSBox fontSize="body-s" color="text-body-secondary">
            ...
            {usage.input_tokens != null
              ? ` · in ${Number(usage.input_tokens).toLocaleString()}`
              : ''}
            {usage.output_tokens != null
              ? ` · out ${Number(usage.output_tokens).toLocaleString()}`
              : ''}
...
function ImportJobResult({ job, onDismiss, onReuse }) {
  ...
  const headerKey = ok
    ? 'scripts.import.result_done'
    : failed
      ? 'scripts.toast.import_fail'
      : partial
        ? 'scripts.toast.import_partial'
      ...
      action={
        <CSSpaceBetween direction="horizontal" size="xs">
          {ok && (
            ...
            >
              {t('scripts.import.go_manage')}
            </CSButton>
          )}
          <CSButton onClick={onReuse}>
            {ok ? t('scripts.import.import_another') : t('scripts.import.retry')}
          </CSButton>
        </CSSpaceBetween>
      }
    >
      ...
      {(failed || partial) && (
        <CSSpaceBetween size="xxs">
          <CSBox>
            {job.error || (errored.length ? `${errored.length} stage(s) failed` : `job ${job.id}`)}
          </CSBox>
          {errored.length > 0 && (
  ...
  return (
    <CSContainer
      header={
        <CSHeader
          variant="h2"
          description={t('scripts.import.estimate_desc', {
            file: estimate.file.name,
            rule: ruleLabel,
          ...
          actions={
            hideActions ? undefined : (
              <CSSpaceBetween direction="horizontal" size="xs">
                ...
                <CSButton variant="primary" iconName="check" onClick={onConfirm}>
                  {t('scripts.import.confirm_import_bg')}
                </CSButton>
              </CSSpaceBetween>
            )
        ...
        >
          {t('scripts.import.estimate_title')}
        </CSHeader>
      }
    >
      <CSSpaceBetween size="l">
        <CSKeyValuePairs
          ...
          items={[
            ...
            {
              label: t('scripts.import.est_time'),
              value: t('scripts.import.est_time_val', { min: Math.round(estimate.totalSec / 60) }),
            },
          ]}
        />
        <CSTable
          ...
          columnDefinitions={[
            ...
            {
              id: 'label',
              header: t('scripts.import.stage_col'),
              cell: (s) => <CSBox fontWeight="bold">{s.label}</CSBox>,
            },
            { id: 'hint', header: t('scripts.import.hint_col'), cell: (s) => s.hint },
            { id: 'tok', header: t('scripts.import.est_tokens'), cell: (s) => fmtN(s.tokens_est) },
            {
              id: 'time',
              header: t('scripts.import.est_time'),
              cell: (s) =>
                s.time_est_sec < 60
                  ? s.time_est_sec + 's'
                  : Math.round(s.time_est_sec / 60) + 'min',
            },
          ]}
        />
...
function KbExtractPanel({ script, onDone }) {
  ...
  const _isZeroLlmScope =
    scope === 'worldbook_only' || scope === 'anchors_only' || scope === 'embed_only';
  ...
  const startStream = (jobId) => {
    ...
    esRef.current = window.api.scripts.streamImport(jobId, {
      on_message: (jb) => {
        if (jb && typeof jb === 'object') setJob({ ...jb, job_id: jb.job_id || jb.id || jobId });
      },
      on_done: () => {
        setPhase('done');
        window.__apiToast?.(t('scripts.review.extract_done'), {
          kind: 'ok',
          detail: t('scripts.review.extract_done_detail'),
          duration: 3200,
        });
        try {
          window.dispatchEvent(new CustomEvent('rpg-scripts-updated'));
  ...
  const doStart = async () => {
    ...
    try {
      ...
      if (jid) startStream(jid);
        ...
        setErr((r && r.error) || t('scripts.review.dispatch_fail'));
        setPhase('error');
      }
    } catch (e) {
      const p = (e && e.payload) || {};
      if (p.job_id) {
        startStream(p.job_id);
        return;
      } // 409 复用已在跑的任务
  ...
  const doCancel = async () => {
    ...
    if (!jid) return;
    try {
      await window.api.scripts.jobCancel(jid);
      window.__apiToast?.(t('scripts.review.cancel_requested'), { kind: 'warn', duration: 2400 });
    } catch (_) {}
  };
  ...
  if (apiId && !apiOptions.some((o) => o.value === apiId))
    apiOptions.unshift({ value: apiId, label: apiId + t('scripts.review.api_not_in_mgr') });
  ...
  if (model && !modelOptions.some((o) => o.value === model))
    modelOptions.unshift({ value: model, label: model + t('scripts.review.model_custom') });
  const onPickApi = (v) => {
    setApiId(v);
  ...
  return (
    <CSSpaceBetween size="l">
      <CSSpaceBetween direction="horizontal" size="xs">
        ...
        {phase === 'config' && !_isZeroLlmScope && (
          <CSButton
            onClick={doEstimate}
            loading={estimating}
            variant={_estimateStale ? 'primary' : 'normal'}
          >
            {t('scripts.review.estimate_cost')}
          </CSButton>
        )}
        {(phase === 'config' || phase === 'error') && (
          <CSButton
            variant={_isZeroLlmScope || !_estimateStale ? 'primary' : 'normal'}
            iconName="gen-ai"
            onClick={doStart}
            disabled={!_canStart}
          >
        ...
        {phase === 'running' && (
          <CSButton onClick={doCancel}>{t('scripts.review.cancel_job')}</CSButton>
        )}
      </CSSpaceBetween>
      ...
      {(phase === 'config' || phase === 'error') && (
        <CSSpaceBetween size="l">
          ...
          {scope === 'full' && (
            <CSFormField label={t('scripts.review.algorithm')}>
              <CSSegmentedControl
                ...
                options={[
                  { id: 'arc', text: t('scripts.review.algo_arc') },
                  { id: 'per_chapter', text: t('scripts.review.algo_per_chapter') },
                ]}
          ...
          {scope === 'full' && (
            <CSColumnLayout columns={2}>
              <CSFormField
                label={t('scripts.review.chapter_min')}
          ...
          {scope === 'full' && (
            <CSColumnLayout columns={2}>
              <CSFormField label="Provider" description={t('scripts.review.provider_desc')}>
                <CSSelect
                  selectedOption={
                    apiOptions.find((o) => o.value === apiId) ||
                    (apiId ? { value: apiId, label: apiId } : null)
                  }
                  options={apiOptions}
              ...
              {algorithm === 'arc' && (
                <CSFormField
                  label={t('scripts.review.target_arcs')}
                  description={t('scripts.review.target_arcs_desc')}
                >
                  <CSInput
                    type="number"
                    value={targetArcs}
                    onChange={({ detail }) => setTargetArcs(detail.value)}
                  />
                </CSFormField>
              )}
              <CSFormField label={t('scripts.review.concurrency')}>
                <CSInput
                  type="number"
                  value={concurrency}
                  onChange={({ detail }) => setConcurrency(detail.value)}
                />
              </CSFormField>
              <CSFormField
                label={t('scripts.review.author_era')}
                description={t('scripts.review.author_era_desc')}
              >
                <CSInput value={authorEra} onChange={({ detail }) => setAuthorEra(detail.value)} />
              </CSFormField>
              <CSFormField label={t('scripts.review.max_usd')}>
                <CSInput
                  type="number"
                  value={maxUsd}
                  onChange={({ detail }) => setMaxUsd(detail.value)}
                />
              </CSFormField>
            </CSColumnLayout>
          )}
                ...
                items={[
                  {
                    label: t('scripts.import.est_cost'),
                    value:
                      estimate.est_usd != null ? `$${Number(estimate.est_usd).toFixed(3)}` : '—',
                  },
                  {
                    label: t('scripts.review.arcs'),
                    value: estimate.arcs != null ? String(estimate.arcs) : '—',
                  },
                  {
                    label: t('scripts.review.input_tokens'),
                    value:
                      estimate.est_input_tokens != null
                        ? Number(estimate.est_input_tokens).toLocaleString()
                        : '—',
                  },
                  {
                    label: t('scripts.review.output_tokens'),
                    value:
                      estimate.est_output_tokens != null
                        ? Number(estimate.est_output_tokens).toLocaleString()
                        : '—',
                  },
                ]}
              />
              {estimate.note && (
                <CSBox fontSize="body-s" color="text-body-secondary" padding={{ top: 'xs' }}>
                  {estimate.note}
                </CSBox>
              )}
            </CSAlert>
          ...
          {estimate && estimate.ok === false && (
            <CSAlert type="warning">
              {estimate.error || estimate.note || t('scripts.review.cannot_estimate')}
            </CSAlert>
          )}
        </CSSpaceBetween>
      )}
      ...
      {(phase === 'running' || phase === 'done') && (
        <CSSpaceBetween size="m">
          <CSProgressBar
            value={overallTotal ? Math.round((overall / overallTotal) * 100) : 0}
            label={t('scripts.review.overall_progress')}
            additionalInfo={t('scripts.review.stage_info', { cur: overall, total: overallTotal })}
          ...
          <CSSpaceBetween size="xs">
            {stages.map((st) => (
              <div key={st.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <CSStatusIndicator type={_stageIndicator(st.status)}>
                  {st.label ||
                    (_EXTRACT_STAGE_LABEL_KEYS[st.id]
                      ? t(_EXTRACT_STAGE_LABEL_KEYS[st.id])
                      : st.id)}
                </CSStatusIndicator>
                {st.stage_total ? (
                  <CSBox fontSize="body-s" color="text-body-secondary">
                    {st.stage_progress || 0} / {st.stage_total}
                  </CSBox>
                ) : null}
              </div>
            ))}
            {stages.length === 0 && (
              <CSBox color="text-body-secondary" fontSize="body-s">
                {t('scripts.review.dispatching')}
              </CSBox>
            )}
          </CSSpaceBetween>
          {job && job.budget_estimate && job.budget_estimate.arcs ? (
            <CSBox fontSize="body-s" color="text-body-secondary">
              {t('scripts.review.split_arcs', { n: job.budget_estimate.arcs })}
            </CSBox>
          ) : null}
          {usage && (
            <CSAlert
              type={phase === 'done' ? 'success' : 'info'}
              header={t('scripts.review.usage')}
            >
              <CSKeyValuePairs
                columns={4}
                items={[
                  {
                    label: t('scripts.review.spent'),
                    value: usage.usd != null ? `$${Number(usage.usd).toFixed(3)}` : '—',
                  },
                  {
                    label: t('scripts.review.input_tokens'),
                    value:
                  ...
                  {
                    label: t('scripts.review.llm_calls'),
                    value: usage.llm_calls != null ? String(usage.llm_calls) : '—',
                  },
                ]}
              />
            </CSAlert>
          ...
          {phase === 'done' && (
            <CSAlert type="success">{t('scripts.review.extract_complete')}</CSAlert>
          )}
        </CSSpaceBetween>
      )}
    </CSSpaceBetween>
