...
function IdentityStep({
  ...
  const _srcOf = (id) =>
    !id ? 'none' : id._from === 'npc_card' ? 'npc' : id._from === 'ai' ? 'ai' : 'manual';
  ...
  const pickNpcIdentity = (card) => {
    ...
    const bg =
      card.background || card.persona || card.summary || card.description || card.bio || '';
    setIdentity({
      name: nm,
      role,
      background: bg,
  ...
  React.useEffect(() => {
    ...
    identity
      ? `${identity._from || ''}:${identity.npc_card_id || ''}:${identity.name || ''}`
      : null,
  ]);
    ...
    >
      <div style={{ display: 'grid', gap: 4, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          <CSBadge
            color={
              identity._from === 'ai' ? 'blue' : identity._from === 'npc_card' ? 'red' : 'grey'
            }
          >
            {identity._from === 'ai'
              ? t('saves.identity.badge_ai')
              : identity._from === 'npc_card'
                ? t('saves.identity.badge_npc')
      ...
      <CSButton iconName="close" variant="inline-link" onClick={() => chooseSource('none')}>
  ...
  return (
    <CSSpaceBetween size="m">
      ...
      <div key="origin-selector" style={{ ...panel, display: 'grid', gap: 12 }}>
        <div style={{ display: 'grid', gap: 4 }}>
          <span style={{ ...labelEyebrow }}>
            {t('saves.identity.step1')} · {t('saves.identity.origin_section_label')}
          </span>
      ...
      <div key="id-source" style={{ ...panel, display: 'grid', gap: 12 }}>
        <div style={{ display: 'grid', gap: 4 }}>
          <span style={{ ...labelEyebrow }}>
            {t('saves.identity.step2')} · {t('saves.identity.id_section_label')}
          </span>
          <span style={{ fontSize: 12, color: 'var(--muted, #968f85)', lineHeight: 1.55 }}>
            {t('saves.identity.id_section_hint', { name: pickedName })}
          </span>
        </div>
        <div
          style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}
          role="radiogroup"
          aria-label={t('saves.identity.id_section_label')}
        >
          {[
            ['none', 'src_none'],
            ['npc', 'src_npc'],
          ...
          (npcCards.length > 0 ? (
            <CSColumnLayout columns={2}>
              {npcCards.map((card, i) => {
                ...
                const isSel =
                  identity && identity._from === 'npc_card' && identity.npc_card_id === cid;
                const nm = card.name || card.title || '';
                ...
                return (
                  <button
                    key={cid}
                    type="button"
                    onClick={() => pickNpcIdentity(card)}
                    style={cardBtnStyle(isSel)}
                  >
                    <div
                ...
                {recs.map((rec, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {rec.background && (
                        <span
                          style={{ fontSize: 12, lineHeight: 1.6, color: 'var(--muted, #968f85)' }}
                        >
                          {rec.background}
                        </span>
                      )}
      ...
      {identity && playerOrigin !== 'body' && (
        <div key="known" style={{ ...panel, display: 'grid', gap: 10 }}>
          <div style={{ display: 'grid', gap: 4 }}>
            <span style={{ ...labelEyebrow }}>
              {t('saves.identity.step3')} · {t('saves.identity.identity_known_label')}
            </span>
            <span style={{ fontSize: 12, color: 'var(--muted, #968f85)', lineHeight: 1.55 }}>
          ...
          {identity._from === 'npc_card' && (
            <CSBox fontSize="body-s" color="text-body-secondary">
              {t('saves.identity.npc_known_hint')}
            </CSBox>
          )}
        </div>
      )}
      {identity && playerOrigin === 'body' && (
        <CSBox key="known-na" fontSize="body-s" color="text-status-inactive">
          {t('saves.identity.known_na_body')}
        </CSBox>
      )}
    </CSSpaceBetween>
  );
