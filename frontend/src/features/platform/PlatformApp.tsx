...
const getPLNav = (t) => [
  ...
  {
    id: 'cards-online',
    label: t('platform.nav.cards_online', { defaultValue: '在线角色卡库' }),
    icon: 'cards',
  },
...
const getPLTitles = (t) => ({
  ...
  'cards-online': [
    t('platform.nav.cards_online', { defaultValue: '在线角色卡库' }),
    t('platform.nav.cards_online_sub', { defaultValue: '浏览并导入他人公开分享的角色卡' }),
  ],
  ...
  'admin-registration': [
    t('platform.nav.admin_registration'),
    t('platform.nav.admin_registration_sub'),
  ],
  'admin-security': [t('platform.nav.admin_security'), t('platform.nav.admin_security_sub')],
  'admin-maintenance': [
    t('platform.nav.admin_maintenance'),
    t('platform.nav.admin_maintenance_sub'),
  ],
  'admin-dmca-takedowns': [
    t('platform.nav.admin_dmca_takedowns'),
    t('platform.nav.admin_dmca_takedowns_sub'),
  ],
  'admin-dmca-strikes': [
    t('platform.nav.admin_dmca_strikes'),
    t('platform.nav.admin_dmca_strikes_sub'),
  ],
  'admin-csam-reports': [
    t('platform.nav.admin_csam_reports'),
    t('platform.nav.admin_csam_reports_sub'),
  ],
  'admin-aup-actions': [
    t('platform.nav.admin_aup_actions'),
    t('platform.nav.admin_aup_actions_sub'),
  ],
  'admin-feedback': [t('platform.nav.admin_feedback'), t('platform.nav.admin_feedback_sub')],
  'admin-achievements': [
    t('platform.nav.admin_achievements'),
    t('platform.nav.admin_achievements_sub'),
  ],
...
function ResizableSplit({ top, bottom, storageKey, initialTop = 240, minTop = 96 }) {
  ...
  const onDown = (e) => {
    ...
    const onUp = () => {
      ...
      if (storageKey) {
        try {
          window.localStorage?.setItem('platform.pl-split-' + storageKey, String(latest));
        } catch (_) {}
      }
    };
  ...
  return (
    <div className="pl-vsplit">
      <div
        className="pl-vsplit-top"
        style={{ maxHeight: topH, overflow: 'auto', borderRadius: 12 }}
      >
        {top}
      </div>
      <div
        className="pl-vsplit-handle"
        onMouseDown={onDown}
      ...
      >
        <div
          className="pl-vsplit-grip"
          style={{
            width: 56,
            height: 5,
            borderRadius: 3,
            background: 'var(--line-strong, #4a4540)',
          }}
        />
      </div>
    ...
    <div className="pl-modal-backdrop" onClick={onClose}>
      ...
      >
        <header className="pl-modal-head">
          ...
          <button className="iconbtn" onClick={onClose} data-tip="关闭">
        ...
        <footer className="pl-modal-foot">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            >
              {danger ? <Icon name="trash" size={12} /> : <Icon name="check" size={12} />}{' '}
              {submitLabel}
            </button>
          </div>
        </footer>
    ...
    <div className="pl-modal-backdrop" onClick={onClose}>
      ...
      >
        <header className="pl-modal-head">
          <div>
            <div
              className="pl-modal-eyebrow"
              style={{ color: danger ? 'var(--danger)' : 'var(--muted-2)' }}
            >
              {danger ? '高危操作' : '确认'}
            </div>
            <h2 className="pl-modal-title">{title}</h2>
          </div>
          <button className="iconbtn" onClick={onClose} data-tip="关闭" disabled={busy}>
            <Icon name="close" size={14} />
          </button>
        </header>
    ...
    <div className="pl-modal-backdrop" onClick={busy ? undefined : handleClose}>
      ...
      >
        ...
        <div style={{ padding: '4px 0 16px' }}>
          ...
          {showCoBuilder && (
            <div
              style={{ borderTop: '1px solid var(--line, #3a322b)', paddingTop: 14, marginTop: 4 }}
            >
              <label
...
async function flushAchievementToasts(items) {
  const fresh = (items || []).filter(
    (a) => a && a.unlocked && a.seen === false && !__achvToasted.has(a.id),
  );
  ...
  try {
    await window.api.account.achievementsSeen();
  } catch (_) {}
}
    ...
    <div className="pl-toast-stack" aria-live="polite">
      {items.map((t) => (
        <div key={t.id} className={`pl-toast pl-toast-${t.kind}`}>
          <span
            className={`pl-toast-icon dot ${t.kind === 'ok' ? 'ok' : t.kind === 'warn' ? 'warn' : t.kind === 'danger' ? 'danger' : 'info'}`}
          />
      ...
      footer={
        <CSBox float="right">
          <CSSpaceBetween direction="horizontal" size="xs">
            <CSButton variant="link" onClick={() => close(cancelVal)}>
              取消
            </CSButton>
            <CSButton variant="primary" onClick={() => close(okVal)}>
              {dlg.confirmText}
            </CSButton>
          </CSSpaceBetween>
        </CSBox>
    ...
    (field, val) => {
      ...
      timerRef.current = setTimeout(async () => {
        ...
        if (Object.keys(batch).length === 0) {
          // 兼容旧调用：仅 toast，不打后端
          window.toast?.(`${label}已保存`, {
            kind: 'ok',
            detail: scope ? `${scope} · ${field}` : field,
            duration: 2400,
          });
          return;
        }
        try {
          ...
          window.toast?.(`${label}保存失败`, {
            kind: 'danger',
            detail: e?.message || '网络错误',
            duration: 3000,
          });
        }
      }, 250);
...
function publishUser(patch) {
  ...
  try {
    window.dispatchEvent(new CustomEvent(USER_EVENT, { detail: next }));
  } catch (_) {}
}
...
function UnifiedSearch({ open, onClose, setPage }) {
  ...
  const pages = [
    {
      id: 'profile',
      label: tSearch('platform.nav.profile'),
      kind: 'page',
      icon: 'home',
    ...
    {
      id: 'me',
      label: tSearch('platform.nav.me'),
      kind: 'page',
      icon: 'user',
      keywords: 'me profile account',
    },
    {
      id: 'me-edit',
      label: tSearch('platform.nav.me_edit_full'),
      kind: 'page',
      icon: 'edit',
      keywords: 'edit profile avatar',
    },
    {
      id: 'me-settings',
      label: tSearch('platform.nav.me_settings_full'),
      kind: 'page',
      icon: 'settings',
      keywords: 'privacy security 2fa',
    },
    {
      id: 'scripts',
      label: tSearch('platform.nav.scripts'),
      kind: 'page',
      icon: 'book',
      keywords: 'scripts import',
    },
    {
      id: 'cards',
      label: tSearch('platform.nav.cards'),
      kind: 'page',
      icon: 'cards',
      keywords: 'characters card user npc',
    },
    {
      id: 'cards-npc',
      label: tSearch('platform.nav.cards_npc_full'),
      kind: 'page',
      icon: 'cards',
      keywords: 'npc characters',
    },
    {
      id: 'saves',
      label: tSearch('platform.nav.saves'),
      kind: 'page',
      icon: 'play',
      keywords: 'saves continue',
    },
    {
      id: 'saves-branches',
      label: tSearch('platform.nav.saves_branches_full'),
      kind: 'page',
      icon: 'branch',
      keywords: 'branches tree fork',
    },
    ...
    {
      id: 'settings',
      label: tSearch('platform.nav.settings'),
      kind: 'page',
      icon: 'settings',
    ...
    {
      id: 'usage',
      label: tSearch('platform.nav.usage'),
      kind: 'page',
      icon: 'usage',
    ...
    {
      id: 'plugins',
      label: tSearch('platform.nav.plugins'),
      kind: 'page',
      icon: 'plug',
  ...
  const settingsItems = [
    {
      id: 'preferences',
      label: tSearch('platform.nav.settings_preferences'),
      parent: settingsLabel,
      hash: 'settings',
      keywords: 'language font density theme',
    },
    {
      id: 'models',
      label: tSearch('platform.nav.settings_models'),
      parent: settingsLabel,
      hash: 'settings',
      keywords: 'openai anthropic models api',
    },
    {
      id: 'memory',
      label: tSearch('platform.nav.settings_memory'),
      parent: settingsLabel,
      hash: 'settings',
      keywords: 'memory recall context',
    },
    {
      id: 'permissions',
      label: tSearch('platform.nav.settings_permissions'),
      parent: settingsLabel,
      hash: 'settings',
      keywords: 'permission write structured',
    },
    {
      id: 'danger',
      label: tSearch('platform.nav.settings_danger'),
      parent: settingsLabel,
      hash: 'settings',
      keywords: 'danger reset delete',
    },
    ...
    ...(isAdmin
      ? [
          {
            id: 'deploy',
            label: tSearch('platform.nav.admin_deploy'),
            parent: adminLabel,
            hash: 'admin-deploy',
            keywords: 'host port cors upload deploy admin',
          },
          {
            id: 'admin-users',
            label: tSearch('platform.nav.admin_users'),
            parent: adminLabel,
            hash: 'admin-users',
            keywords: 'users ban role deactivate admin',
          },
          {
            id: 'admin-usage',
            label: tSearch('platform.nav.admin_usage'),
            parent: adminLabel,
            hash: 'admin-usage',
            keywords: 'global usage token cost admin',
          },
          {
            id: 'admin-audit',
            label: tSearch('platform.nav.admin_audit'),
            parent: adminLabel,
            hash: 'admin-audit',
            keywords: 'audit log admin action',
          },
          {
            id: 'admin-health',
            label: tSearch('platform.nav.admin_health'),
            parent: adminLabel,
            hash: 'admin-health',
            keywords: 'health db memory disk process',
          },
          {
            id: 'admin-logs',
            label: tSearch('platform.nav.admin_logs'),
            parent: adminLabel,
            hash: 'admin-logs',
            keywords: 'logs system stderr stdout',
          },
          {
            id: 'admin-reg',
            label: tSearch('platform.nav.admin_registration'),
            parent: adminLabel,
            hash: 'admin-registration',
            keywords: 'registration invite code signup',
          },
          {
            id: 'admin-sec',
            label: tSearch('platform.nav.admin_security'),
            parent: adminLabel,
            hash: 'admin-security',
            keywords: 'ip blocklist rate limit password policy',
          },
          {
            id: 'admin-maint',
            label: tSearch('platform.nav.admin_maintenance'),
            parent: adminLabel,
            hash: 'admin-maintenance',
            keywords: 'maintenance mode announcement restart',
          },
          {
            id: 'admin-dmca-td',
            label: tSearch('platform.nav.admin_dmca_takedowns'),
            parent: adminLabel,
            hash: 'admin-dmca-takedowns',
            keywords: 'dmca takedown notice copyright admin',
          },
          {
            id: 'admin-dmca-sk',
            label: tSearch('platform.nav.admin_dmca_strikes'),
            parent: adminLabel,
            hash: 'admin-dmca-strikes',
            keywords: 'dmca strike repeat offender admin',
          },
          {
            id: 'admin-csam',
            label: tSearch('platform.nav.admin_csam_reports'),
            parent: adminLabel,
            hash: 'admin-csam-reports',
            keywords: 'csam report child abuse admin',
          },
          {
            id: 'admin-aup',
            label: tSearch('platform.nav.admin_aup_actions'),
            parent: adminLabel,
            hash: 'admin-aup-actions',
            keywords: 'aup suspend ban terminate policy admin',
          },
          {
            id: 'admin-feedback',
            label: tSearch('platform.nav.admin_feedback'),
            parent: adminLabel,
            hash: 'admin-feedback',
            keywords: 'feedback review user report admin',
          },
          {
            id: 'admin-achv',
            label: tSearch('platform.nav.admin_achievements'),
            parent: adminLabel,
            hash: 'admin-achievements',
            keywords: 'achievement badge milestone catalog 成就 徽章 admin',
          },
        ]
      : []),
  ];
  ...
  const lib = (() => {
    ...
    return recent.slice(0, 8).map((f, i) => ({
      ...
      kind: 'library',
      icon: f.kind === 'folder' ? 'folder' : f.kind === 'image' ? 'image' : 'file',
      hash: 'library',
    }));
  })();
  ...
  React.useEffect(() => {
    ...
    const onKey = (e) => {
      if (e.key === 'Escape') {
        ...
        e.preventDefault();
        if (flatList[cursor]) pick(flatList[cursor]);
      }
    };
        ...
        <div className="pl-search-body">
          {!q && (
            <div className="pl-search-empty">
              ...
              >
                {tSearch('platform.search.suggestions')}
              </div>
              {pages.slice(0, 6).map((p, i) => (
                <button
                  key={p.id}
                  className={`pl-search-row ${i === 0 ? 'active' : ''}`}
                  onClick={() => pick(p)}
                >
                  ...
                  <span className="pl-search-meta muted-2">
                    {tSearch('platform.search.category_pages')}
                  </span>
                </button>
              ))}
            </div>
                ...
                <div key={kind} className="pl-search-group">
                  ...
                  {items.map((it) => {
                    ...
                    return (
                      <button
                        key={it.id}
                        className={`pl-search-row ${active ? 'active' : ''}`}
                        onClick={() => pick(it)}
                      ...
                      >
                        ...
                        <span className="pl-search-label">
                          ...
                          {it.sub && (
                            <span className="muted-2" style={{ marginLeft: 8, fontSize: 11.5 }}>
...
function AchievementWall({ items }) {
  const groups = (() => {
    ...
    return [...m.keys()]
      .sort(
        (x, y) =>
          (ACHV_CAT_ORDER.indexOf(x) < 0 ? 99 : ACHV_CAT_ORDER.indexOf(x)) -
          (ACHV_CAT_ORDER.indexOf(y) < 0 ? 99 : ACHV_CAT_ORDER.indexOf(y)),
      )
      .map((k) => [k, m.get(k)]);
  })();
  return (
        ...
        <CSSpaceBetween size="xs" key={cat}>
          ...
          <CSColumnLayout columns={4} variant="text-grid">
            {list.map((a) => (
              <div
                key={a.id}
                className={`pl-achv ${a.unlocked ? 'unlocked' : 'locked'}${a.tier ? ' tier-' + a.tier : ''}`}
              >
                <div className="pl-achv-mark">
                  {a.icon ? (
                    <span style={{ fontSize: 16 }}>{a.icon}</span>
                  ) : (
                    <Icon name={a.unlocked ? 'check' : 'lock'} size={a.unlocked ? 16 : 14} />
                  )}
                </div>
                <div className="pl-achv-body">
                  ...
                  {a.unlocked ? (
                    ...
                    <div className="pl-achv-progress">
                      <div className="pl-achv-bar">
                        <div className="pl-achv-fill" style={{ width: (a.pct || 0) + '%' }} />
                      </div>
                      <span className="muted-2 mono" style={{ fontSize: 10.5 }}>
                        {a.target != null
...
function AchvShareModal({ user, items, unlockedCount, total, onClose }) {
  ...
  const copy = async () => {
    try {
      ...
      window.toast('链接已复制', { kind: 'ok' });
    } catch (_) {
      window.toast('复制失败，请手动复制', { kind: 'warn' });
    }
  };
  return (
    <CSModal
        ...
        <CSBox float="right">
          <CSSpaceBetween direction="horizontal" size="xs">
            ...
            <CSButton
              onClick={() => {
                onClose();
                plNavigate('wall', { search: '?u=' + encodeURIComponent(username) });
              }}
            >
              查看我的公开墙
            </CSButton>
            <CSButton variant="primary" iconName="copy" onClick={copy}>
              复制链接
            </CSButton>
          </CSSpaceBetween>
        </CSBox>
    ...
    >
      <CSSpaceBetween size="m">
        ...
        <CSBox color="text-body-secondary" fontSize="body-s">
          ...
          <div className="mono" style={{ marginTop: 4, wordBreak: 'break-all', fontSize: 12 }}>
            {wallUrl}
          </div>
        </CSBox>
      </CSSpaceBetween>
...
export function PublicAchievementsPage() {
  ...
  useEffectPL(() => {
    ...
    (async () => {
      ...
      try {
        const r = await window.api.account.publicWall(username);
  ...
  if (err) {
    ...
    return (
      <CSContainer>
        <CSBox textAlign="center" color="text-body-secondary" padding="xxl">
  ...
  if (!data)
    return (
      <CSContainer>
        <CSBox textAlign="center" padding="xxl">
          加载中…
        </CSBox>
      </CSContainer>
    );
    ...
    <CSSpaceBetween size="l">
      <CSContainer>
        <CSSpaceBetween direction="horizontal" size="m">
          <div className="pl-achv-share-avatar lg">
            {(data.display_name || data.username || '?').slice(0, 1)}
          </div>
          <div>
            <CSBox variant="h2">{data.display_name || data.username}</CSBox>
            <CSBox color="text-body-secondary" fontSize="body-s">
              @{data.username} · 解锁 {data.unlocked_count} / {data.total} 成就
            </CSBox>
          </div>
        </CSSpaceBetween>
      </CSContainer>
  ...
  useEffectPL(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setMeActivity((r && r.activity) || []);
      } catch (_) {
        if (!cancelled) setMeActivity([]);
      }
    })();
    return () => {
      cancelled = true;
              ...
              <CSSpaceBetween size="xs">
                <CSBox variant="h2">
                  ...
                  <span className="pill" style={{ marginLeft: 8 }}>
                    <span className="dot ok pulse" /> 在线
                  </span>
                ...
                <CSBox color="text-body-secondary" fontSize="body-s">
                  <span>
                    <Icon name="user" size={11} /> @{user.username}
                  </span>
                  <span className="mono" style={{ marginLeft: 12 }}>
                    uid {user.uid}
                  </span>
                  <span style={{ marginLeft: 12 }}>
                    <Icon name="history" size={11} /> 注册于 {regAt} · 上次登录 {lastLoginAgo}
                  </span>
                </CSBox>
        ...
        <CSColumnLayout columns={5} variant="text-grid">
          <div>
            ...
            <CSBox fontSize="display-l" fontWeight="bold">
              {playHoursLabel}
              {playMinutesTotal != null && (
                <span style={{ fontSize: 14, color: 'var(--muted)', marginLeft: 4 }}>h</span>
              )}
            </CSBox>
            <CSBox color="text-body-secondary" fontSize="body-s">
              {playMinutesWeek != null ? `本周 +${(playMinutesWeek / 60).toFixed(1)}h` : '暂无统计'}
            </CSBox>
          </div>
          <div>
            <CSBox variant="awsui-key-label">回合数</CSBox>
            <CSBox fontSize="display-l" fontWeight="bold">
              {totalRounds != null ? totalRounds.toLocaleString() : '—'}
            </CSBox>
            <CSBox color="text-body-secondary" fontSize="body-s">
              分布在 {saves.length} 个存档
            </CSBox>
          </div>
          <div>
            ...
            <CSBox color="text-body-secondary" fontSize="body-s">
              {maxDepth ? `最深 ${maxDepth} 层` : '—'}
            </CSBox>
          </div>
          <div>
            <CSBox variant="awsui-key-label">导入剧本</CSBox>
            <CSBox fontSize="display-l" fontWeight="bold">
              {importedScripts != null ? importedScripts : '—'}
            </CSBox>
            <CSBox color="text-body-secondary" fontSize="body-s">
              {importedWords ? `共 ${fmtCN(importedWords)}字` : '—'}
            </CSBox>
          </div>
          <div>
            ...
            <CSBox fontSize="display-l" fontWeight="bold">
              ...
              <span style={{ fontSize: 14, color: 'var(--muted)', marginLeft: 4 }}>天</span>
            </CSBox>
            <CSBox color="text-body-secondary" fontSize="body-s">
              {longestStreak ? `最长 ${longestStreak} 天` : '—'}
            </CSBox>
          </div>
        </CSColumnLayout>
      </CSContainer>
        ...
        header={
          <CSHeader
            ...
            actions={
              !IS_ANON &&
              unlockedCount > 0 && (
                <CSButton iconName="share" onClick={() => setShareOpen(true)}>
                  分享成就
                </CSButton>
              )
            }
          ...
          <CSHeader
            ...
            actions={
              <CSSpaceBetween direction="horizontal" size="xs">
                ...
                <CSButton
                  variant={filter === '回合' ? 'primary' : 'normal'}
                  onClick={() => setFilter('回合')}
                >
                  回合
                </CSButton>
                <CSButton
                  variant={filter === '分支' ? 'primary' : 'normal'}
                  onClick={() => setFilter('分支')}
                >
                  分支
                </CSButton>
                <CSButton
                  variant={filter === '剧本' ? 'primary' : 'normal'}
                  onClick={() => setFilter('剧本')}
                >
                  剧本
                </CSButton>
              </CSSpaceBetween>
          ...
          {filteredActivity.map((a, i) => (
            <li key={i}>
              <div className="pl-activity-rail">
                <span className="pl-activity-dot">
              ...
              <div className="pl-activity-body">
                ...
                {a.sub ? (
                  <div className="pl-activity-sub muted-2" style={{ fontSize: 12, marginTop: 2 }}>
                    {a.sub}
                  </div>
                ) : null}
                <div className="pl-activity-meta">
                  ...
                  <span className="muted-2 mono" style={{ fontSize: 11 }}>
                    {/^\d{4}-\d{2}-\d{2}T/.test(a.ts || '') ? fmtAgo(a.ts) : a.ts}
                  </span>
                </div>
              </div>
            </li>
...
function MeEditProfile() {
  ...
  const u = (k, v) => {
    setForm((f) => ({ ...f, [k]: v }));
    try {
      window.__capMarkDirty && window.__capMarkDirty('settings.profile');
    } catch (_) {}
  };
  ...
  useEffectPL(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled)
          window.__apiToast?.('加载资料失败,请检查网络后重试', {
            kind: 'danger',
            detail: e?.message,
            duration: 3000,
          });
  ...
  const onSave = async () => {
    ...
    try {
      ...
      try {
        window.__capClearDirty && window.__capClearDirty('settings.profile');
      } catch (_) {}
  ...
  return (
    <CSSpaceBetween size="l">
        ...
        <CSSpaceBetween size="m">
          <div className="pl-me-avatar-row">
            ...
            <div className="pl-me-avatar-actions">
              <CSBox color="text-body-secondary" fontSize="body-s">
                支持 PNG / JPG / WEBP，建议 512×512。最大 2 MB。
              </CSBox>
              <CSSpaceBetween direction="horizontal" size="xs">
                <CSButton iconName="upload" onClick={() => setUploadOpen(true)}>
                  上传新头像
                </CSButton>
                <CSButton iconName="remove" onClick={() => setResetAvatarOpen(true)}>
                  使用默认
                </CSButton>
              </CSSpaceBetween>
            </div>
      ...
      {/* 基本资料 */}
      <CSContainer
        header={<CSHeader variant="h2">基本资料</CSHeader>}
        data-cap-anchor="settings.profile"
      >
        <CSSpaceBetween size="l">
          <div className="pl-form-grid-2">
            <Field label="显示名" hint="出现在游戏和评论里">
              <CSInput
                ...
                onChange={({ detail }) => u('display_name', detail.value)}
              />
            </Field>
            <Field label="代词">
              <CSSelect
                selectedOption={
                  [
                    ...
                    { value: '不公开', label: '不公开' },
                  ].find((o) => o.value === form.pronouns) || null
                }
                options={[
                  { value: '她/她', label: '她/她' },
                  { value: '他/他', label: '他/他' },
            ...
            <Field label="用户名" hint="登录用，6 个月可改一次" required>
              <CSInput
                value={form.username}
                onChange={({ detail }) => u('username', detail.value)}
              />
            </Field>
            <Field label="真实姓名" hint="仅自己可见">
              <CSInput
                value={form.real_name}
                onChange={({ detail }) => u('real_name', detail.value)}
              />
            </Field>
            <Field label="性别">
              <CSSpaceBetween direction="horizontal" size="xs">
                {[
                  ...
                  <CSButton
                    key={o.v}
                    variant={form.gender === o.v ? 'primary' : 'normal'}
                    onClick={() => u('gender', o.v)}
                  >
                    {o.l}
                  </CSButton>
                ))}
            ...
            <Field label="生日">
              <CSInput
                type="date"
                value={form.birthday}
                onChange={({ detail }) => u('birthday', detail.value)}
              />
            </Field>
            <Field label="所在地">
              <CSInput
                value={form.location}
                onChange={({ detail }) => u('location', detail.value)}
                placeholder="例：上海"
              />
            </Field>
            <Field label="个人网站">
              <CSInput
                value={form.website}
                onChange={({ detail }) => u('website', detail.value)}
                placeholder="https://..."
              />
            </Field>
          </div>
          <Field label="简介" hint="280 字以内">
            ...
            />
            <CSBox color="text-body-secondary" fontSize="body-s" textAlign="right">
              {form.bio.length} / 280
            </CSBox>
          </Field>
        </CSSpaceBetween>
      ...
      <CSContainer header={<CSHeader variant="h2">联系方式</CSHeader>}>
        <div className="pl-form-grid-2">
          <Field label="邮箱" hint="用于通知与找回密码">
            <CSInput
              value={form.email}
              onChange={({ detail }) => u('email', detail.value)}
          ...
          <Field label="手机" hint="选填，仅自己可见">
            <CSInput
              value={form.phone}
              onChange={({ detail }) => u('phone', detail.value)}
              placeholder="选填"
            />
          </Field>
        </div>
            ...
            <CSSelect
              selectedOption={
                [
                  ...
                  { value: 'ja', label: '日本語' },
                ].find((o) => o.value === form.language) || null
              }
              options={[
                { value: 'zh-CN', label: '简体中文' },
                { value: 'zh-TW', label: '繁體中文' },
          ...
          <Field label="时区">
            <CSSelect
              selectedOption={
                ...
                ].find((o) => o.value === form.timezone) || null
              }
              options={[
                { value: 'Asia/Shanghai', label: 'UTC+8 · 上海' },
                { value: 'Asia/Tokyo', label: 'UTC+9 · 东京' },
      ...
      <ConfirmModal
        ...
        confirmLabel="恢复默认"
        onClose={() => setResetAvatarOpen(false)}
...
function MeUserSettings() {
  ...
  const onSavePreference = async (key, value) => {
    try {
      await window.api.account.preferences({ [key]: value });
    } catch (_) {}
  };
  // Persist preference changes on each toggle:
  // 值为 null 时说明后端偏好尚未加载完成，跳过（避免 mount 时以默认值覆盖已存设置）
  useEffectPL(() => {
    if (twofa !== null && prefLoaded) onSavePreference('two_fa', twofa);
  }, [twofa, prefLoaded]);
  useEffectPL(() => {
    if (emailNotif !== null && prefLoaded) onSavePreference('email_notif', emailNotif);
  }, [emailNotif, prefLoaded]);
  useEffectPL(() => {
    if (publicProfile !== null && prefLoaded) onSavePreference('public_profile', publicProfile);
  }, [publicProfile, prefLoaded]);
  useEffectPL(() => {
    if (searchable !== null && prefLoaded) onSavePreference('searchable', searchable);
  }, [searchable, prefLoaded]);
  useEffectPL(() => {
    if (shareUsage !== null && prefLoaded) onSavePreference('share_usage', shareUsage);
  }, [shareUsage, prefLoaded]);
  useEffectPL(() => {
    if (shareCrash !== null && prefLoaded) onSavePreference('share_crash', shareCrash);
  }, [shareCrash, prefLoaded]);
  useEffectPL(() => {
    if (adsTrack !== null && prefLoaded) onSavePreference('ads_track', adsTrack);
  }, [adsTrack, prefLoaded]);
  ...
  return (
    <CSSpaceBetween size="l" data-cap-anchor="me.settings">
      ...
      <CSContainer header={<CSHeader variant="h2">账号 · 安全</CSHeader>}>
        <CSSpaceBetween size="l">
          <SettingRow
            ...
            desc={
              hasPassword
                ? '建议每 90 天更换一次，至少 12 位字符 + 大小写 + 数字。'
                : '当前账号通过邮箱链接登录，尚未设置密码；可直接设置一组新密码。'
            }
            control={
              <CSButton iconName="lock-private" onClick={() => setPwOpen(true)}>
              ...
              <CSSpaceBetween direction="horizontal" size="xs">
                {twofa && (
                  <span className="pill ok">
                    <span className="dot ok" /> Authenticator
                  </span>
                )}
          ...
          {(() => {
            ...
            const okIn30d = loginHistory.filter((h) => {
              ...
              try {
                return new Date(h.at).getTime() >= cutoff;
              } catch {
                return false;
              }
            }).length;
            ...
            return (
              <>
                <SettingRow
                  ...
                  desc={sessDesc}
                  control={
                    <CSButton iconName="visibility-on" onClick={() => setSessionsOpen(true)}>
                      查看会话
                    </CSButton>
                  }
                />
                <SettingRow
                  ...
                  desc={histDesc}
                  control={
                    <CSButton iconName="status-info" onClick={() => setHistoryOpen(true)}>
                      查看日志
                    </CSButton>
                  }
                />
      ...
      <CSContainer header={<CSHeader variant="h2">数据所有权</CSHeader>}>
        <CSSpaceBetween size="l">
          <SettingRow
            ...
            control={
              <CSButton iconName="download" onClick={() => setExportOpen(true)}>
                申请导出
              </CSButton>
            }
          />
          <SettingRow
            ...
            control={
              <CSButton variant="normal" onClick={() => setConfirmDeact(true)}>
                停用账号
              </CSButton>
            }
          />
          <SettingRow
            ...
            control={
              <CSButton variant="normal" iconName="remove" onClick={() => setConfirmDelete(true)}>
                删除账号
              </CSButton>
            }
          />
      ...
      <ConfirmModal
        ...
        body={
          <>
            这会<strong>立刻</strong>删除你的账号、剧本、存档、库资产，<strong>无法恢复</strong>
            。删除后无法用同一邮箱再注册（30 天冷冻期）。
          </>
        }
        danger
        confirmLabel="确认删除"
        ...
        <div className="pl-modal-backdrop" onClick={() => setSessionsOpen(false)}>
          ...
          >
            <header className="pl-modal-head">
              <div>
                ...
                <h2 className="pl-modal-title">
                  {sessions.length === 0 ? '暂无活跃会话' : `${sessions.length} 个登录中`}
                </h2>
              </div>
              <button className="iconbtn" onClick={() => setSessionsOpen(false)} data-tip="关闭">
                <Icon name="close" size={14} />
              </button>
            </header>
                    ...
                    <div>
                      ...
                      {s.current && (
                        <span className="pill ok" style={{ marginLeft: 6 }}>
                          <span className="dot ok pulse" /> 当前
                        </span>
                      )}
                    </div>
                    <span className="muted-2 mono" style={{ fontSize: 11 }}>
            ...
            <footer className="pl-modal-foot">
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="btn ghost" onClick={() => setSessionsOpen(false)}>
                  关闭
                </button>
                <button className="btn danger" onClick={onRevokeAll} disabled={busyRevokeAll}>
                  <Icon name="close" size={12} /> 全部下线（保留当前）
                </button>
              </div>
            </footer>
          </div>
        ...
        <div className="pl-modal-backdrop" onClick={() => setHistoryOpen(false)}>
          ...
          >
            <header className="pl-modal-head">
              ...
              <button className="iconbtn" onClick={() => setHistoryOpen(false)} data-tip="关闭">
                <Icon name="close" size={14} />
              </button>
            </header>
            <ul className="pl-session-list">
              {loginHistory.length === 0 ? (
                <li className="muted" style={{ padding: 16, textAlign: 'center' }}>
                  暂无记录
                </li>
              ) : (
                loginHistory.map((r, i) => (
                  <li key={i} className="pl-history-row">
                    <span className="mono muted-2" style={{ fontSize: 11, width: 92 }}>
                      {r.ts}
                    </span>
                    ...
                    {r.result === 'ok' ? (
                      <span className="pill ok" style={{ fontSize: 10.5 }}>
                        <span className="dot ok" /> 成功
                      </span>
                    ) : (
                      <span className="pill danger" style={{ fontSize: 10.5 }}>
                        <span className="dot danger" /> 已拦截
                      </span>
                    )}
                  </li>
                ))
        ...
        <div className="pl-modal-backdrop" onClick={() => setPolicyOpen(false)}>
          ...
          >
            <header className="pl-modal-head">
              ...
              <button className="iconbtn" onClick={() => setPolicyOpen(false)} data-tip="关闭">
                <Icon name="close" size={14} />
              </button>
            </header>
            <div
            ...
            >
              <p>
                <strong>1. 我们收集什么</strong>
                ：账号信息（用户名、邮箱、可选手机）、设备指纹（用于会话）、用量遥测（仅在你开启时）。
              </p>
              <p>
                <strong>2. 我们 不 收集什么</strong>
                ：剧本正文、玩家变量、私聊、长期记忆、世界书条目——这些数据加密存储在你的工作区，团队
                无 任何访问。
              </p>
              <p>
                <strong>3. 与第三方</strong>：不向第三方分享剧本内容。模型 API
                调用按你配置直接发往对应厂商（OpenAI / Anthropic 等），团队 不 代理也 不 留存。
              </p>
              <p>
                <strong>4. 数据所有权</strong>
                ：你可以随时通过『导出我的数据』申请完整归档；可随时『停用账号』（90
                天保留）或『永久删除』（立刻执行）。
              </p>
              <p>
                <strong>5. 合规</strong>：本平台符合 GDPR · 中国《个人信息保护法》· 加州 CCPA。
              </p>
            </div>
            <footer className="pl-modal-foot">
              <a
              ...
              <button className="btn primary" onClick={() => setPolicyOpen(false)}>
                我已阅读
              </button>
            </footer>
          </div>
...
function ProfilePage() {
  ...
  const branchAgg =
    realSaves.reduce((a, s) => a + (Number(s && s.branch_count) || 0), 0) ||
    (stats?.branches ?? null);
  ...
  return (
    <CSSpaceBetween size="l">
      ...
      <div
        style={{
          background:
            'linear-gradient(135deg, var(--panel-2,#282623) 0%, var(--panel,#211f1d) 100%)',
        ...
        <CSSpaceBetween direction="horizontal" size="xs">
          {lastSave ? (
            <CSButton
              ...
              onClick={() => window.__openContinue?.(lastSave)}
            >
              继续《{lastSave.title || lastScript?.title || '上次存档'}》
            </CSButton>
          ) : (
            <CSButton variant="primary" iconName="add-plus" onClick={() => plNavigate('scripts')}>
              浏览剧本
            </CSButton>
          )}
          <CSButton iconName="folder" onClick={() => plNavigate('scripts')}>
            剧本库
          </CSButton>
          <CSButton iconName="user-profile" onClick={() => plNavigate('cards')}>
            用户角色卡
          </CSButton>
          <CSButton iconName="settings" onClick={() => plNavigate('settings')}>
            设置
          </CSButton>
        </CSSpaceBetween>
      </div>
      ...
      <CSContainer header={<CSHeader variant="h2">平台资源</CSHeader>}>
        <div className="pl-resources-grid">
          <div>
            ...
            <CSBox color="text-body-secondary" fontSize="body-s">
              {wordTotal > 0 ? `共 ${wordWan} 万字` : '未导入剧本'}
            </CSBox>
          </div>
          <div>
            ...
            <CSBox color="text-body-secondary" fontSize="body-s">
              {realSaves[0]?.updated_at ? `最近：${realSaves[0].updated_at}` : '尚未创建存档'}
            </CSBox>
          </div>
          <div>
            ...
            <CSBox color="text-body-secondary" fontSize="body-s">
              {realSaves.length ? `来自 ${realSaves.length} 个存档` : '—'}
            </CSBox>
          </div>
          <div>
            ...
            <CSBox color="text-body-secondary" fontSize="body-s">
              ...
              <a
                ...
                style={{ borderBottom: '1px dotted var(--muted-2)' }}
              >
                用量页
              </a>
            </CSBox>
          ...
          items={[
            {
              ...
              value: (
                <CSBox>
                  ...
                  <CSStatusIndicator
                    type={database.ok ? 'success' : 'error'}
                    style={{ marginLeft: 8 }}
                  >
      ...
      <CSContainer
        header={
          <CSHeader
            variant="h2"
            actions={
              <CSButton onClick={() => plNavigate('saves')} iconName="caret-right-filled">
                全部存档
              </CSButton>
            }
          >
            最近游玩{' '}
            <span className="muted-2" style={{ fontWeight: 'normal' }}>
              按上次操作时间
            </span>
          </CSHeader>
        }
        ...
        {realSaves.length === 0 ? (
          <CSBox textAlign="center" color="text-body-secondary" padding="l">
            <CSSpaceBetween size="s">
              ...
              <CSButton onClick={() => plNavigate('scripts')} iconName="file">
                去剧本页
              </CSButton>
            </CSSpaceBetween>
          </CSBox>
          ...
          <CSTable
                  ...
                  <span className="muted">
                    {s.current && (
                      <span className="pill accent" style={{ marginRight: 6 }}>
                        <span className="dot accent pulse" /> 在玩
                      </span>
                    )}
                    {s.updated_at || '—'}
                  </span>
            ...
            trackBy="id"
            empty={
              <CSBox color="text-body-secondary" textAlign="center">
                暂无存档
              </CSBox>
            }
          />
...
function ModulesPage() {
  ...
  return (
    <CSSpaceBetween size="l">
      {errorMsg && (
        <CSAlert type="error" dismissible={false}>
          {errorMsg}
        </CSAlert>
      )}
      <CSContainer
            ...
            columnDefinitions={[
              {
                ...
                cell: (m) => (
                  <div className="pl-title-cell">
                    ...
                    {m.tagline ? (
                      <span className="muted-2" style={{ fontStyle: 'italic', marginTop: 3 }}>
                        {m.tagline}
                      </span>
                    ) : null}
                  </div>
                ...
                cell: (m) => {
                  ...
                  return (
                    <CSStatusIndicator type="success">
                      {ruleset.public_label || '5E compatible'}
                    </CSStatusIndicator>
                  );
                },
              },
      ...
      <CSContainer>
        <CSBox color="text-body-secondary" fontSize="body-s">
          ...
          检定、战斗、HP/AC 计算由确定性 RulesEngine 完成；GM 直写 HP/AC/initiative 会被 State Gate
          拒绝。
        </CSBox>
      </CSContainer>
    </CSSpaceBetween>
  ...
  return (
    <div style={{ padding: 32, textAlign: 'center', color: 'var(--muted, #968f85)' }}>
      ...
      <p style={{ fontSize: 13, lineHeight: 1.7, maxWidth: 520, margin: '0 auto' }}>
        测试阶段我们只允许上传剧本文件 (.txt / .md) 用于「导入剧本」。 其他附件 /
        资产上传暂时关闭,等接入文件类型白名单 + 病毒扫描后再开放。
      </p>
  ...
  return (
    <CSSpaceBetween size="l">
          ...
          <CSHeader
            ...
            description={
              <CSSpaceBetween size="xs" direction="horizontal">
                <CSButton variant="inline-link" onClick={() => setPath('')}>
                  库
                </CSButton>
            ...
            actions={
              <CSSpaceBetween size="xs" direction="horizontal">
                ...
                <CSButton iconName="add-plus" onClick={() => setMkdirOpen(true)}>
                  新建文件夹
                </CSButton>
                <CSButton
                  variant="primary"
              ...
              },
              {
                ...
                cell: (r) => (
                  <span
                    title={r.name}
                    onClick={() => {
                      if (r.kind === 'folder') setPath(r.path || r.name);
                    }}
                    style={{
                      cursor: r.kind === 'folder' ? 'pointer' : 'default',
                      color: r.kind === 'folder' ? 'var(--color-text-link-default)' : undefined,
                    }}
                  >
                    {r.name}
                  </span>
              ...
              },
              {
                ...
                cell: (r) => (
                  <span className="mono muted">
                    {r.kind === 'folder' ? `${r.items || 0} 项` : fmtBytes(r.size)}
                  </span>
                ),
              },
              {
      ...
      <PromptModal
        ...
        fields={[{ key: 'name', label: '文件夹名', required: true, placeholder: '例：人物谱' }]}
        submitLabel="创建"
...
function UsagePage() {
  ...
  // 切换时间范围时重置到第 0 页
  const handleSetRange = useCallbackPL((r) => {
    setRange(r);
    setRecentPage(0);
  }, []);
    ...
    <CSSpaceBetween size="l">
      ...
      <CSContainer
        header={
          ...
          >
            ...
            >
              最近 {USAGE_RANGES.find((r) => r.id === range)?.label}
            </span>
          </CSHeader>
        }
      >
        <CSColumnLayout columns={5} variant="text-grid">
          <div>
            ...
            <CSBox color="text-body-secondary" fontSize="body-s">
              {totalTurns ? `日均 ${Math.round(totalTurns / days)}` : '—'}
            </CSBox>
          </div>
          <div>
            ...
            <CSBox color="text-body-secondary" fontSize="body-s">
              {totalTokOut
                ? `输出 ${fmtN(totalTokOut)} · 比 1 : ${(totalTokIn / Math.max(1, totalTokOut)).toFixed(1)}`
                : '输出 —'}
            </CSBox>
          </div>
          <div>
          ...
          <div>
            ...
            <CSBox fontSize="display-l" fontWeight="bold">
              {totalCachedIn ? fmtN(totalCachedIn) : '—'}
            </CSBox>
            <CSBox color="text-body-secondary" fontSize="body-s">
          ...
          <div>
            ...
            <CSBox fontSize="display-l" fontWeight="bold">
              {totalCachedIn > 0 && totalTokIn > 0
                ? `$${(totalCost > 0 ? (totalCachedIn / totalTokIn) * totalCost * 0.75 : 0).toFixed(
                    3,
                  )}`
                : '—'}
            </CSBox>
          ...
          return (
            <CSContainer
              header={
                <CSHeader
                  variant="h2"
                  description="基于过去 7 天平均日消耗（固定窗口，不随时间范围切换器变化）"
                >
                  消费预测
                </CSHeader>
              }
      ...
      {byScenario &&
        ...
        (() => {
          ...
          const scenarioTotalTurns =
            scenarioKeys.reduce((s, k) => s + Number(byScenario[k]?.turns || 0), 0) || 1;
          return (
            <CSContainer
              header={
                <CSHeader variant="h2" description="按场景统计本窗口请求分布">
                  按场景拆分
                </CSHeader>
              }
            >
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: 6,
                        background:
                          'var(--color-background-container-content, rgba(255,255,255,0.03))',
                        border:
                          '1px solid var(--color-border-divider-default, rgba(255,255,255,0.08))',
                      }}
                    >
                      ...
                      <div
                        style={{
                          fontSize: 12,
                          color: 'var(--color-text-label, #aaa)',
                          marginBottom: 6,
                          fontWeight: 600,
                          textTransform: 'uppercase',
                      ...
                      <div style={{ fontSize: 11, color: 'var(--color-text-body-secondary)' }}>
                        ${cost.toFixed(3)}
                      </div>
                      <div
                        style={{
                          ...
                          height: 3,
                          borderRadius: 999,
                          background: 'var(--color-background-control-default, #333)',
                          overflow: 'hidden',
                        }}
                      >
                        <div
                          style={{
                            width: pct + '%',
                            height: '100%',
                            background: 'var(--color-text-accent, #d4a45e)',
                            borderRadius: 999,
                          }}
                        />
                      </div>
      ...
      <CSContainer
        header={
          <CSHeader variant="h2" description="每日聚合">
            趋势
          </CSHeader>
        }
      >
        {bucketSeries.length === 0 ? (
          ...
          <CSSpaceBetween size="m">
            <CSColumnLayout columns={2} variant="text-grid">
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  ...
                  <span
                    className="mono"
                    style={{ fontSize: 12, color: 'var(--color-text-body-secondary)' }}
                  >
                    {fmtN(reqSeriesVals.reduce((a, x) => a + x, 0))}
                  </span>
                </div>
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  ...
                  <span
                    ...
                    style={{ fontSize: 12, color: 'var(--color-text-body-secondary)' }}
                  >
                    ${costSeriesVals.reduce((a, x) => a + x, 0).toFixed(2)}
                  </span>
                </div>
            ...
            {/* U2: token 趋势 */}
            <div>
              <CSBox variant="awsui-key-label" style={{ marginBottom: 6 }}>
                Token 趋势
              </CSBox>
              <CSColumnLayout columns={2} variant="text-grid">
                <div>
                  ...
                  >
                    <span style={{ fontSize: 12, color: 'var(--color-text-body-secondary)' }}>
                      输入 Token
                    </span>
                    <span
                      className="mono"
                      style={{ fontSize: 12, color: 'var(--color-text-body-secondary)' }}
                    >
                      {fmtN(tokInSeriesVals.reduce((a, x) => a + x, 0))}
                    </span>
                  </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{ fontSize: 12, color: 'var(--color-text-body-secondary)' }}
                    >
                      {fmtN(tokOutSeriesVals.reduce((a, x) => a + x, 0))}
                    </span>
                  </div>
          ...
          columnDefinitions={[
            {
              ...
              header: 'API',
              cell: (r) => (
                <strong style={{ fontFamily: 'var(--font-serif)', fontSize: 13.5 }}>{r.id}</strong>
              ),
            },
            {
              id: 'requests',
            ...
            },
            {
              ...
              cell: (r) => (
                <span className="mono">
                  <span className="muted">{fmtN(r.tokens_in)}</span>{' '}
                  <span className="muted-2">/</span> {fmtN(r.tokens_out)}
                </span>
              ),
            },
            {
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div
                    style={{
                      ...
                      height: 4,
                      borderRadius: 999,
                      background: 'var(--color-background-control-default)',
                      overflow: 'hidden',
                    }}
                  >
                    <div
                      style={{
                        width: (totalTurns ? (r.requests / totalTurns) * 100 : 0) + '%',
                        height: '100%',
                  ...
                  <span className="muted-2 mono" style={{ fontSize: 11 }}>
                    {totalTurns ? Math.round((r.requests / totalTurns) * 100) : 0}%
                  </span>
                </div>
              ),
            },
              ...
              <CSButton
                ...
                onClick={() => {
                  const sorted = [...byModel].sort(
                    (a, b) => Number(b.turns || 0) - Number(a.turns || 0),
                  );
                  const headers = [
                    '排名',
                    '模型',
          ...
          columnDefinitions={[
            {
              ...
              width: 40,
              cell: (m) => (
                <span className="mono muted-2">{String((m._rank ?? 0) + 1).padStart(2, '0')}</span>
              ),
            },
            {
              id: 'model',
            ...
            },
            {
              ...
              cell: (m) => (
                <span className="mono">
                  <span className="muted">{fmtN(Number(m.input_tokens || 0))}</span>{' '}
                  <span className="muted-2">/</span> {fmtN(Number(m.output_tokens || 0))}
                </span>
              ),
            },
            {
              ...
              cell: (m) => (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  >
                    <div
                      style={{
                        width: (totalTurns ? (Number(m.turns || 0) / totalTurns) * 100 : 0) + '%',
                        height: '100%',
                        background: 'var(--color-text-accent)',
                      }}
                    />
                  ...
                  <span className="muted-2 mono" style={{ fontSize: 11 }}>
                    {totalTurns ? Math.round((Number(m.turns || 0) / totalTurns) * 100) : 0}%
                  </span>
                </div>
              ),
            },
        ...
        <CSTable
          columnDefinitions={[
            ...
            },
            {
              ...
              cell: (r) => (
                <span className="mono">
                  <span className="muted">{fmtN(Number(r.input_tokens || 0))}</span>{' '}
                  <span className="muted-2">/</span> {fmtN(Number(r.output_tokens || 0))}
                </span>
              ),
            },
            {
              ...
              cell: (r) => (
                <span className="mono muted">
                  {r.cached_input_tokens ? fmtN(Number(r.cached_input_tokens)) : '—'}
                </span>
              ),
            },
            {
                    ...
                    >
                      <div
                        style={{
                          width: pct + '%',
                          height: '100%',
                          background: danger ? '#d54' : 'var(--color-text-accent)',
                          borderRadius: 999,
                        }}
                      />
                    </div>
          ...
          items={recent}
          trackBy={(r) =>
            `${r.at || ''}/${r.api_id || ''}/${r.model || ''}/${r.input_tokens || ''}`
          }
          empty={
            <CSBox textAlign="center" color="text-body-secondary" padding="l">
              {loading ? '加载中…' : '暂无最近调用'}
      ...
      try {
        ...
        if (kind === 'plugins') {
          list = (t.plugins || []).map((p) => ({
            ...
            tag: p.kind || 'plugin',
            on: p.enabled !== false,
            status: p.enabled === false ? '未启用' : '已启用',
            _raw: p,
          }));
        } else if (kind === 'mcp') {
          ...
          let running = [];
          try {
            const rt = await window.api.mcp.runtime();
            running = (rt && (rt.running || [])) || [];
          } catch (_) {}
          const runSet = new Set(running.map((r) => r.id || r.server_id || r.name));
          list = servers.map((s) => {
            const isOn = !!s.enabled;
            const isRunning =
              isOn && (runSet.has(s.id) || runSet.has(s.server_id) || runSet.has(s.name));
            return {
          ...
          list = (t.skills || []).map((s) => ({
            ...
            on: s.enabled !== false,
            status: s.enabled !== false ? '已部署' : '未启用',
            _raw: s,
          }));
        }
        setItems(list);
  ...
  const onValidateAll = async () => {
    ...
    if (kind === 'mcp') {
      const results = await Promise.all(
        items
          .filter((it) => it.on)
          .map((it) => window.api.mcp.validate({ id: it.id, server_id: it.id }).catch(() => null)),
      );
      const ok = results.filter((r) => r && r.ok !== false).length;
      const fail = results.length - ok;
      window.__apiToast?.(`校验完成 · ${ok} ok / ${fail} fail`, {
        kind: fail ? 'warn' : 'ok',
        duration: 2400,
      });
    }
        ...
        header={
          <CSHeader
            ...
            counter={
              loading
                ? undefined
                : `(${items.length} 项 · ${items.filter((i) => i.on).length} 已启用)`
            }
            actions={
              <CSSpaceBetween size="xs" direction="horizontal">
                <CSButton iconName="refresh" onClick={onValidateAll} loading={loading}>
                  校验
                </CSButton>
                <CSButton variant="primary" iconName="add-plus" onClick={() => setAddOpen(true)}>
        ...
        onConfirm={async (vals) => {
          ...
          try {
            if (kind === 'mcp') {
              ...
              try {
                await window.api.mcp.validate({ name: vals.name });
              } catch (_) {}
            } else if (kind === 'skills') {
              ...
              window.__apiToast?.('插件由平台预置 · 暂不支持自定义新增', {
                kind: 'warn',
                duration: 2400,
              });
            }
  ...
  const handleToggle = async (next) => {
    ...
    if (kind === 'mcp') {
      try {
        ...
        if (next) {
          try {
            await window.api.mcp.start({ id, server_id: id });
          } catch (_) {}
        } else {
          try {
            await window.api.mcp.stop({ id, server_id: id });
          } catch (_) {}
        }
        onChanged && onChanged();
      } catch (e) {
      ...
      window.__apiToast?.('Skill 默认全部启用 · 暂不支持单独停用', {
        kind: 'warn',
        duration: 2400,
      });
      setV(true);
    } else if (kind === 'plugins') {
      window.__apiToast?.('插件状态由平台管理 · 暂不支持手动切换', {
        kind: 'warn',
        duration: 2400,
      });
      setV(true);
    ...
    kind === 'mcp'
      ? [
          ...
          },
          {
            ...
            required: true,
            mono: true,
            default: tag === 'stdio' ? 'uvx my-mcp' : 'https://localhost:7300',
          },
          {
            key: 'env',
            label: '环境变量 / Headers',
    ...
    <div className="pl-cap">
      <div className="pl-cap-head">
        <div className="pl-cap-icon">
          <Icon
            name={kind === 'mcp' ? 'diamond' : kind === 'skills' ? 'spark' : 'plug'}
            size={16}
          />
        </div>
      ...
      <div className="pl-cap-foot">
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          <button className="iconbtn" data-tip="编辑" onClick={() => setEditOpen(true)}>
            <Icon name="edit" size={12} />
          </button>
          <button className="iconbtn" data-tip="查看日志" onClick={() => setLogOpen(true)}>
            <Icon name="debug" size={12} />
          </button>
        </div>
      </div>
      <PromptModal
        ...
        <div className="pl-modal-backdrop" onClick={() => setLogOpen(false)}>
          ...
          >
            <header className="pl-modal-head">
              ...
              <button className="iconbtn" onClick={() => setLogOpen(false)} data-tip="关闭">
                <Icon name="close" size={14} />
              </button>
            </header>
            <pre
            ...
            >
              {logBusy ? '加载中…' : logText || '（暂无内容）'}
            </pre>
            <footer className="pl-modal-foot">
              <span className="muted-2" style={{ fontSize: 11.5 }}>
                <Icon name="info" size={11} />{' '}
                {kind === 'mcp'
                  ? 'GET /api/mcp/runtime · admin 可见 stderr'
                  : '本类型暂无运行时日志'}
              </span>
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="btn ghost" onClick={loadLog} disabled={logBusy}>
      ...
      <section className="pl-sec">
        <div className="pl-sec-head">
          <h2>
            稳定接口{' '}
            <span className="muted-2">
              v1 · {filtered.length} 条 · {Object.keys(groups).length} 组
            </span>
          </h2>
        ...
        {Object.entries(groups).map(([group, items]) => (
          <div key={group} style={{ display: 'grid', gap: 8 }}>
            <div className="pl-stat-label" style={{ padding: '8px 4px 0' }}>
              {group}
            </div>
            <div className="pl-api">
              <div className="pl-api-row head">
                <div>METHOD</div>
                <div>路径</div>
                <div>说明</div>
                <div></div>
              ...
              {items.map((r, i) => (
                <div key={i} className="pl-api-row">
                  <div>
                    <span className={`pl-api-method ${r.m}`}>{r.m}</span>
                  </div>
...
function AuthPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      window.__apiToast?.(mode === 'register' ? '注册成功' : '登录成功', {
        kind: 'ok',
        duration: 1400,
      });
      location.replace(__nextOrDefault());
    ...
    <div className="pl-auth-wrap">
      <div className="pl-auth">
        ...
        <form className="pl-auth-form" onSubmit={submit}>
          ...
          {err && (
            <div
              className="pl-auth-error"
              style={{ color: 'var(--danger,#c0392b)', fontSize: 12.5, padding: '4px 0' }}
            >
              {err}
            </div>
          )}
          {notice && (
            <div
              ...
              style={{
                ...
                fontSize: 12.5,
                padding: '4px 0',
                borderLeft: '2px solid var(--accent, #b65b41)',
                paddingLeft: 8,
              }}
            >
              {notice}
...
const getCSModules = (t) => [
  {
    id: 'scripts',
    label: t('platform.nav.scripts'),
    group: t('platform.nav.group_create'),
  ...
  {
    id: 'account',
    label: t('platform.nav.account'),
    group: t('platform.nav.group_system'),
    pages: [
      'me',
      'me-edit',
      'me-settings',
    ...
    sub: [
      ...
      {
        text: t('platform.nav.settings_account', { defaultValue: '账号与数据迁移' }),
  ...
  {
    id: 'admin',
    label: t('platform.nav.admin'),
    group: t('platform.nav.group_admin'),
    adminOnly: true,
    pages: [
      'admin-deploy',
      'admin-users',
  ...
  {
    id: 'extensions',
    label: t('platform.nav.extensions'),
    group: t('platform.nav.group_system'),
    pages: ['plugins', 'mcp', 'skills', 'apis', 'usage'],
    sub: [
...
function AdminGuard({ children }) {
  ...
  if (!role) {
    return (
      <div style={{ padding: '48px 20px', textAlign: 'center', color: 'var(--muted,#8c857a)' }}>
        正在校验权限…
      </div>
    );
  }
  if (role !== 'admin') {
    return (
      <div style={{ maxWidth: 560, margin: '64px auto', textAlign: 'center' }}>
        ...
        <div
          style={{ fontSize: 17, fontWeight: 600, color: 'var(--text,#ebe7df)', marginBottom: 8 }}
        >
          需要管理员权限
        </div>
        <div style={{ fontSize: 13.5, lineHeight: 1.7, color: 'var(--text-quiet,#a8a195)' }}>
          系统管理(部署配置等站点级设置)仅平台管理员可访问。
          <br />
          当前账号角色为 <strong style={{ color: 'var(--text,#ebe7df)' }}>{role}</strong>
          。如需权限请联系管理员。
        </div>
        <div style={{ marginTop: 22 }}>
          <a
            ...
            style={{ color: 'var(--accent,#c96442)', textDecoration: 'none', fontSize: 13.5 }}
          >
            ← 返回主页
          </a>
        </div>
...
function PlatformShellCS({ page, setPage, children, assistant, assistantOpen, onToggleAssistant }) {
  ...
  const chromeApi = React.useMemo(
    () => ({ set: (c) => setChromeState(c || {}), clear: () => setChromeState({}) }),
    [],
  );
  void chrome;
  useEffectPL(() => {
  ...
  useEffectPL(() => {
    const check = () => {
      window.__checkAchievements && window.__checkAchievements();
    };
    check();
  ...
  useEffectPL(() => {
    ...
    (async () => {
      try {
        ...
        const newReviewed = (data.items || []).filter(
          (it) => it.review_decision && it.id > lastSeen,
        );
  ...
  useEffectPL(() => {
    ...
    window.__openContinue = async (save, nodeId) => {
      ...
      if (!targetSaveId) {
        window.__apiToast?.('没有可进入的存档', { kind: 'warn', duration: 2400 });
        return;
      }
      // 用户手势内先开空白标签,绕过弹窗拦截
      ...
      try {
        if (gameWin && gameWin.document) {
          ...
          gameWin.document
            .write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>正在加载存档…</title>
      ...
      try {
        ...
        try {
          if (gameWin && gameWin.document) {
            gameWin.document.open();
            gameWin.document
              .write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>无法打开存档</title>
    ...
    <>
        ...
        >
          {!standalone && (
            <button
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.background = '#282623';
              }}
              onMouseLeave={(e) => {
          ...
          <CSButtonDropdown
            ...
            onItemClick={({ detail }) => {
              const m = csModules.find((x) => x.id === detail.id);
              if (m) {
                setPage(m.pages[0]);
              }
            }}
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center' }}>
          ...
          >
            <button
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.background = 'rgba(196,155,78,0.10)';
                e.currentTarget.style.borderColor = 'rgba(196,155,78,0.6)';
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.background = 'transparent';
                e.currentTarget.style.borderColor = 'rgba(196,155,78,0.35)';
              }}
            >
            ...
            <button
              ...
              style={{
                ...
                padding: '3px 10px',
                borderRadius: 6,
                border: '1px solid rgba(140,140,180,0.30)',
                background: 'transparent',
                color: '#c8c2b7',
                cursor: 'pointer',
                fontSize: 12.5,
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.background = 'rgba(140,140,180,0.10)';
                e.currentTarget.style.borderColor = 'rgba(140,140,180,0.55)';
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.background = 'transparent';
                e.currentTarget.style.borderColor = 'rgba(140,140,180,0.30)';
              }}
            >
              ...
              utilities={[
                {
                  type: 'button',
                  iconName: 'search',
                  title: t('platform.menu.search_title'),
                  ariaLabel: t('platform.menu.search_title'),
                ...
                },
                {
                  ...
                  ariaLabel: t('platform.nav.settings'),
                  disableUtilityCollapse: true,
                  onClick: () => {
                    setPage('settings');
                  },
                },
                {
                  ...
                  title: helpSlugForPage
                    ? `${t('platform.menu.help_current')} (${helpSlugForPage})`
                    : t('platform.menu.help'),
                  ariaLabel: t('platform.menu.help'),
                  disableUtilityCollapse: true,
                  onClick: () => {
                    if (window.__openHelp) window.__openHelp(helpSlugForPage || 'intro');
                  },
                },
                {
                  ...
                  ariaLabel: t('platform.menu.refresh_aria'),
                  disableUtilityCollapse: true,
                  onClick: _csRefresh,
                },
                {
      ...
      <CSAppLayout
        ...
        navigation={
          <CSSideNavigation
            header={{
              text: _csActiveModule(page, csModules).label,
              href: '#' + _csActiveModule(page, csModules).pages[0],
            }}
            activeHref={'#' + page}
            onFollow={(e) => {
              e.preventDefault();
            ...
            items={_csActiveModule(page, csModules).sub.map((s) => ({
              type: 'link',
              text: s.text,
              href: s.href,
            }))}
        ...
        content={<ShellChromeCtx.Provider value={chromeApi}>{children}</ShellChromeCtx.Provider>}
      />
