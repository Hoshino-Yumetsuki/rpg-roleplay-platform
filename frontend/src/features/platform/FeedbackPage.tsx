    ...
    >
      <div
        style={{
          ...
          fontWeight: 700,
          lineHeight: 1.1,
          color: color || 'var(--text, #e8e3d9)',
        }}
      >
        {value}
      </div>
      <div style={{ fontSize: 12.5, color: 'var(--text-quiet, #968f85)', marginTop: 4 }}>
        {label}
      </div>
    </div>
  );
...
export function FeedbackPage() {
  ...
  const loadHistory = React.useCallback(async () => {
    ...
    try {
      ...
      setHistoryError(e?.message || '读取反馈记录失败');
    } finally {
  ...
  const toggleExcerpt = (idx) =>
    setSelectedExcerpts((p) => (p.includes(idx) ? p.filter((i) => i !== idx) : [...p, idx]));
  const canSubmit =
    consent && freeText.trim().length > 0 && freeText.length <= MAX_FREE_TEXT && !busy;
  ...
  async function withdraw(id) {
    ...
    try {
      ...
      window.__apiToast?.('撤回失败', { kind: 'danger', detail: e?.message });
    }
  }
  ...
  const counts = React.useMemo(() => {
    ...
    for (const it of history) {
      if (!it.review_decision) pending++;
      else if (it.review_decision === 'ok') ok++;
    }
    ...
    <div style={{ maxWidth: 960, margin: '0 auto', paddingBottom: 32 }}>
      <CSSpaceBetween size="l">
        <CSHeader
          ...
          actions={
            <CSButton iconName="refresh" loading={historyLoading} onClick={loadHistory}>
              刷新
            </CSButton>
          }
        >
        ...
        <CSAlert type="warning" header="内容限制">
          反馈渠道不得包含性、露骨、NSFW 等成人材料(无论是否年满
          18)。违反将永久封号并加入禁注表。详见{' '}
        ...
        <CSContainer
          header={
            <CSHeader variant="h2" description="带运行环境信息能让排查更快(仅管理员可见)。">
              提交反馈
            </CSHeader>
          }
        >
          <CSSpaceBetween size="m">
            ...
            {includeRuntime && runtimePreview && (
              <CSBox fontSize="body-s" color="text-body-secondary">
                <div>
                  页面 <code>{runtimePreview.hash || runtimePreview.url || '—'}</code> · 剧本{' '}
                  {String(runtimePreview.active?.script_id ?? '—')} / 存档{' '}
                  {String(runtimePreview.active?.save_id ?? '—')}
                </div>
                <div>
                  错误 {runtimePreview.errors?.length || 0} 条 · 失败 API{' '}
                  {runtimePreview.api_failures?.length || 0} 条 · {runtimePreview.viewport}
                </div>
              </CSBox>
            )}
            <CSCheckbox
                ...
                <CSSpaceBetween size="xs">
                  {recentTurns.map((t) => (
                    ...
                    >
                      ...
                      <CSBox color="text-body-secondary" fontSize="body-s" display="inline">
                        {t.plaintext.slice(0, 70)}
                        {t.plaintext.length > 70 ? '…' : ''}
                      </CSBox>
                    </CSCheckbox>
                  ))}
                </CSSpaceBetween>
        ...
        <CSContainer
          header={
            <CSHeader variant="h2" counter={history.length ? `(${history.length})` : undefined}>
              我的反馈记录
            </CSHeader>
          }
        >
          <CSSpaceBetween size="m">
            ...
            {historyError ? (
              <CSAlert type="error" header="读取失败">
                {historyError}
              </CSAlert>
              ...
              <CSBox textAlign="center" color="text-body-secondary" padding={{ vertical: 'l' }}>
                该筛选下暂无反馈。
              </CSBox>
            ) : (
                ...
                {filtered.map((it) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <CSBadge color={statusColor(it.review_decision)}>
                        {statusLabel(it.review_decision)}
                      </CSBadge>
                      <CSBox fontSize="body-s" color="text-body-secondary">
                        提交 {fmtTime(it.created_at)}
                    ...
                    {it.admin_reply && (
                      <div
                        style={{
                          ...
                          padding: '10px 13px',
                          borderRadius: 8,
                          background: 'var(--accent-soft, rgba(201,100,66,.12))',
                          borderLeft: '3px solid var(--accent, #c96442)',
                          fontSize: 13,
                          lineHeight: 1.6,
                        }}
                      >
                        <strong>官方回复</strong>
                        {it.replied_at ? (
                          <span style={{ color: 'var(--text-quiet,#968f85)', fontWeight: 400 }}>
                            {' '}
        ...
        <CSColumnLayout columns={2} variant="text-grid">
          ...
          <CSContainer header={<CSHeader variant="h3">提示</CSHeader>}>
            <CSBox fontSize="body-s" color="text-body-secondary">
              · 描述越具体越好:复现步骤、期望/实际、截图链接。
              <br />· bug 类请保留「附带运行环境信息」勾选,便于定位。
              <br />· 已处理的反馈会显示状态与官方回复。
              <br />· 快速反馈:任意页面点顶栏「反馈」即可弹窗提交,不打断当前操作。
            </CSBox>
          </CSContainer>
        </CSColumnLayout>
      </CSSpaceBetween>
