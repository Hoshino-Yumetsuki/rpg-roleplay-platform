    ...
    return (
      <label className="wz-field">
        ...
        <select value={value} onChange={(e) => onChange(e.target.value)} disabled={locked}>
          {worldlines.map((w) => (
            <option key={w.wl_key} value={w.wl_key}>
              {w.label}
            </option>
          ))}
        </select>
    ...
    getSettings(saveId).then((d) => {
      if (d.ok) {
        ...
        (d.schema.fields || []).forEach((f) => {
          init[f.key] = (d.settings && d.settings[f.key]) ?? f.default;
        });
        setVals(init);
  ...
  return (
    <div className="new-game-wizard" style={{ padding: 16, maxWidth: 560 }}>
      <div className="wz-steps" style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        {STEPS.map((s) => (
          <span key={s.n} className={s.n === step ? 'wz-step active' : 'wz-step'}>
            {s.n}. {s.title}
          </span>
        ))}
      </div>
        ...
        <div className="wz-confirm">
          ...
          <ul>
            {Object.entries(vals).map(([k, v]) => (
              <li key={k}>
                {fieldsByKey[k]?.label || k}:<b>{String(v)}</b>
              </li>
            ))}
          </ul>
        </div>
