...
(function () {
  ...
  try {
    storedFont = localStorage.getItem('rpg.narrativeFont') || 'serif';
  } catch (_) {}
                ...
                <button
                  ...
                  style={{
                    ...
                    background: isCurrent
                      ? 'var(--accent-soft, rgba(212,164,94,0.12))'
                      : 'transparent',
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: 18 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 4 }}>
            自带密钥(BYOK)· 平台不代为提供 API 服务
          </div>
          <div style={{ fontSize: 13, lineHeight: 1.65, color: 'var(--text-quiet, #9a9590)' }}>
            GM 主对话与 RAG Embedder 均采用自带密钥（BYOK）模式,需你自行准备并配置 AI API
            Key（Anthropic / OpenAI / Vertex / DeepSeek 等）。 平台不代为提供 AI API 调用服务。
          </div>
        </div>
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          <button
            onClick={() => {
              onClose();
              window.open('/platform/settings/models', '_blank');
            }}
            style={{
              padding: '6px 14px',
              borderRadius: 6,
...
function mapAgentPhase(phase) {
  ...
  if (
    ...
    phase === 'manifest' ||
    phase === 'assembly' ||
    phase === 'context_retrieve' ||
    phase === 'context_agent' ||
    phase === 'world_check' ||
    phase === 'prompt_assemble' ||
    phase === 'aborted' ||
    (typeof phase === 'string' && phase.startsWith('provider:'))
  )
...
function App() {
  ...
  const [activeTab, setActiveTab] = useState(() =>
    getRightTabForLocation(t.defaultRightTab || 'status'),
  );
  // 侧栏折叠状态持久化(localStorage,刷新后保留)。来自 PR #14。
  const [railCollapsed, setRailCollapsed] = useState(() => {
    try {
      return localStorage.getItem('gc.rail.collapsed') === 'true';
    } catch {
      return false;
    }
  });
  const [panelCollapsed, setPanelCollapsed] = useState(() => {
    try {
      return localStorage.getItem('gc.panel.collapsed') === 'true';
    } catch {
      return false;
    }
  });
  useEffect(() => {
    try {
      localStorage.setItem('gc.rail.collapsed', railCollapsed ? 'true' : 'false');
    } catch {}
  }, [railCollapsed]);
  useEffect(() => {
    try {
      localStorage.setItem('gc.panel.collapsed', panelCollapsed ? 'true' : 'false');
    } catch {}
  }, [panelCollapsed]);
  ...
  useEffect(() => {
    ...
    return () => {
      delete window.__openWelcome;
    };
  }, []);
  const _railResize = useResizable({
    storageKey: 'gc.rail.w',
    defaultSize: 240,
    min: 180,
    max: 360,
    side: 'left',
  ...
  const [showUsage, setShowUsage] = useState(() => {
    try {
      return localStorage.getItem('rpg.showTokenUsage') === 'on';
    } catch (_) {
      return false;
    }
  });
  ...
  const reloadState = useCallback(async () => {
    try {
          ...
          if (isFresh && !alreadyOpened) {
            ...
            requestAnimationFrame(() => {
              try {
                      ...
                      on_stage: (d) => {
                        ...
                        setHistory((h) => {
                          ...
                          if (arr.length && arr[arr.length - 1]._opening && !openingText) {
                            arr[arr.length - 1] = {
                              ...arr[arr.length - 1],
                              content: label || arr[arr.length - 1].content,
                              _thinking: phase,
                            };
                          }
                          return arr;
                        ...
                        if (!openingText && !openingRetried) {
                          ...
                          setTimeout(() => {
                            try {
                              ...
                              console.warn('[opening] retry error', e);
                            }
                          }, 1000);
                        } else {
                          setRunState((r) => ({
                        ...
                        if (!openingText && !openingRetried) {
                          ...
                          setTimeout(() => {
                            try {
                              ...
                              console.warn('[opening] retry (onClose) error', e);
                            }
                          }, 1000);
                        } else {
                          setRunState((r) => ({
                ...
                console.warn('[opening] trigger error', e);
              }
            });
          }
        } catch (_) {}
      ...
      if (data && data.save_id != null) {
        setActiveSave({
          id: data.save_id,
          title: data.save_title || `存档 #${data.save_id}`,
          updated_at: data.save_updated_at || '',
        });
      }
  ...
  useEffect(() => {
    setActiveTab(getRightTabForLocation(t.defaultRightTab || 'status'));
  }, [t.defaultRightTab]);
  useEffect(() => {
  ...
  useEffect(
    () => () => {
      ...
      rc.timers = [];
      if (rc.doneTimer) {
        clearTimeout(rc.doneTimer);
        rc.doneTimer = null;
      }
      if (rc.inactivityTimer) {
        clearTimeout(rc.inactivityTimer);
        rc.inactivityTimer = null;
      }
      if (rc.sse) {
        try {
          rc.sse.stop();
  ...
  const stopRun = useCallback(() => {
    ...
    runRef.current.timers.forEach((t) => {
      try {
        clearInterval(t);
      } catch (_) {}
    });
    runRef.current.timers = [];
    if (runRef.current.doneTimer) {
      clearTimeout(runRef.current.doneTimer);
      runRef.current.doneTimer = null;
    }
    if (runRef.current.inactivityTimer) {
      clearTimeout(runRef.current.inactivityTimer);
      runRef.current.inactivityTimer = null;
    }
    if (runRef.current.sse) {
      try {
        runRef.current.sse.stop();
      } catch (_) {}
      runRef.current.sse = null;
    }
    try {
      window.api.game.stop();
    } catch (_) {}
    setRunState((r) => ({
      ...r,
      running: false,
      ...
      publicStage: null,
      completedAt: 0,
      completedElapsed: r.totalElapsed,
    }));
  }, []);

  const startRunReal = useCallback(
    async (playerText) => {
      // task #61: 存档冲突时拒绝发送（用户点"继续"后 banner 清空才恢复）
      if (tabConflictBanner) {
        window.__apiToast?.('存档冲突', {
          kind: 'warn',
          detail: '已在另一窗口打开此存档，请关闭另一窗口或点"继续"后重试',
          duration: 4000,
        });
        return;
      }
      ...
      setHistory((h) => [
        ...h,
        { role: 'user', content: playerText, ts, attachments: sentAttachments },
      ]);
      setLastPlayerText(playerText);
      setSseLog([
        { t: Date.now(), kind: 'send', payload: { message: playerText, model: model && model.id } },
      ]);
      setText('');
      setAttachments([]);
      setShowSlash(false);
      ...
      setRunState({
        running: true,
        publicStage: 'context',
        label: PUBLIC_STAGES.context.label,
        detail: '',
        totalElapsed: 0,
        completedAt: 0,
        completedElapsed: 0,
      ...
      const logEvent = (kind, payload) =>
        setSseLog((l) => (l.length >= 500 ? l : [...l, { t: Date.now(), kind, payload }]));
      ...
      const resetInactivityTimer = () => {
        ...
        runRef.current.inactivityTimer = setTimeout(() => {
          try {
            runRef.current.sse && runRef.current.sse.stop && runRef.current.sse.stop();
          } catch (_) {}
          restoreFailedDraft();
          setRunState((r) => {
            if (!r.running) return r;
            setHasError('超过 120 秒没有新输出, 已主动断开。可能是模型卡死或网络慢, 请重试。');
            window.__apiToast?.('生成停滞', {
              kind: 'warn',
              detail: '120 秒无响应, 已中断',
              duration: 4000,
            });
        ...
        },
        {
          ...
          on_status: (data) => {
            ...
            if (data && data.player)
              setGame((g) => {
                ...
                n._raw = {
                  save_id: data.save_id ?? n._raw?.save_id,
                  save_title: data.save_title ?? n._raw?.save_title,
                  turn: data.turn ?? n._raw?.turn,
                };
                return n;
              });
            ...
            setRunState((r) =>
              r.running
                ? {
                    ...r,
                    label: '思考中',
                    detail: '💭 ' + reasoningBuf.slice(-90).replace(/\s+/g, ' ').trim(),
                  }
                : r,
            );
          },
          on_token: (data) => {
            ...
            setHistory((h) => {
              if (!openedAssistant) {
                ...
                return [...h, { role: 'assistant', content: piece, ts, streaming: true }];
              }
              const last = h[h.length - 1];
          ...
          on_agent: (data) => {
            ...
            setRunState((r) => {
              ...
              const merged = {
                ...
                elapsed_ms: data.elapsed_ms ?? (idx >= 0 ? rawSteps[idx].elapsed_ms : 0),
                detail: data.detail || (idx >= 0 ? rawSteps[idx].detail : undefined),
              };
              if (idx >= 0) rawSteps[idx] = { ...rawSteps[idx], ...merged };
              else rawSteps.push(merged);
              const nextStageId = advancePublicStage(r.publicStage, mapped);
              const nextLabel =
                nextStageId && PUBLIC_STAGES[nextStageId]
                  ? PUBLIC_STAGES[nextStageId].label
                  : r.label;
              if (data.status === 'stopped')
                return { ...r, rawSteps, publicStage: null, label: '已停止', detail: '' };
          ...
          on_updates: (data) => {
            ...
            if (stage === 'pre_llm') {
              ...
              if (items.length)
                window.__apiToast?.(`设定已更新（${items.length} 项）`, {
                  kind: 'ok',
                  detail: items.join('\n'),
                  duration: 4000,
                });
            ...
            setRunState((r) => {
              ...
              return {
                ...r,
                publicStage: nextStageId,
                label: PUBLIC_STAGES[nextStageId].label,
                detail: '',
              };
            });
          },
          ...
          on_done: (data) => {
            ...
            logEvent('done', {
              status: !!data && data.status ? 'ok' : 'noop',
            ...
            setHistory((h) => {
              ...
              return [
                ...h.slice(0, -1),
                { ...last, content: cleaned, streaming: false, streaming_done: true },
              ];
            });
            setRunState((r) => {
            ...
            runRef.current.doneTimer = setTimeout(() => {
              ...
              // 重新填 rawSteps。
              setRunState((r) =>
                r.running ? r : { ...r, publicStage: null, completedAt: 0, label: '' },
              );
            }, 1800);
            ...
            if (payload && payload.player)
              setGame((g) => {
                ...
                n._raw = {
                  save_id: payload.save_id ?? n._raw?.save_id,
                  save_title: payload.save_title ?? n._raw?.save_title,
                  turn: payload.turn ?? n._raw?.turn,
                };
                return n;
              });
            ...
            if (
              payload &&
              payload.save_id != null &&
              (!activeSave || activeSave.id !== payload.save_id)
            ) {
              setActiveSave({
                id: payload.save_id,
                title: payload.save_title || `存档 #${payload.save_id}`,
                updated_at: payload.save_updated_at || '',
              });
            }
            runRef.current.sse = null;
          ...
          onClose: () => {
            ...
            setRunState((r) => {
              ...
              window.__apiToast?.('生成中断', {
                kind: 'warn',
                detail: '流式连接关闭但没收到完成事件,可能是模型 safety filter 截断',
                duration: 4000,
              });
              restoreFailedDraft();
              return {
      ...
      const runStep = (i) => {
        ...
        if (i >= MOCK_PHASES.length) {
          ...
          setRunState((r) => ({
            ...
            detail: '',
            completedAt: Date.now(),
            completedElapsed: Date.now() - startedAt,
          }));
          if (runRef.current.doneTimer) clearTimeout(runRef.current.doneTimer);
          runRef.current.doneTimer = setTimeout(() => {
            runRef.current.doneTimer = null;
        ...
        setRunState((r) => {
          const rawSteps = [
            ...r.rawSteps,
            { phase: step.phase, message: step.message, status: 'running', elapsed_ms: 0 },
          ];
          const nextStageId = advancePublicStage(r.publicStage, mapped);
          return {
            ...r,
        ...
        if (step.phase === 'main_gm') {
          ...
          const chunkInterval = setInterval(
            () => {
              ...
              if (chunkIdx >= STREAM_CHUNKS.length) {
                clearInterval(chunkInterval);
                return;
              }
        ...
        const timerB = setTimeout(() => {
          if (runRef.current.stopped) return;
          setRunState((r) => {
            const rawSteps = r.rawSteps.slice();
            const idx = rawSteps.findIndex((s) => s.phase === step.phase);
            if (idx >= 0)
              rawSteps[idx] = { ...rawSteps[idx], status: 'done', elapsed_ms: step.duration };
            return { ...r, rawSteps };
          });
          if (step.phase === 'main_gm') {
            setHistory((h) => {
  ...
  const onSend = () => {
    ...
    startRun(
      text.trim() ||
        (pickedCommand ? pickedCommand.trigger.trim() : '（仅附件，请基于本轮上下文推进。）'),
    );
  };
  ...
  const onRetry = useCallback(() => {
    ...
    if (!t2) {
      window.__apiToast?.('没有可重试的输入', { kind: 'warn', duration: 2000 });
      return;
    }
    setHasError(false);
    setHistory((h) => {
      ...
      if (
        ...
        (out[out.length - 1].content || '').trim() === t2
      )
        out.pop();
      return out;
    });
  ...
  const onRegenerate = useCallback(
    async (messageIndex) => {
      if (runState.running) {
        window.__apiToast?.('正在生成中,请先停止或稍候', { kind: 'warn', duration: 2000 });
        return;
      }
      const h = Array.isArray(history) ? history : [];
      ...
      if (pIdx < 0) {
        window.__apiToast?.('这一轮没有玩家输入,无法重新生成', { kind: 'warn', duration: 2400 });
        return;
      }
      const playerText = String(h[pIdx]?.content || '').trim();
      if (!playerText) {
        window.__apiToast?.('玩家输入为空,无法重新生成', { kind: 'warn', duration: 2400 });
        return;
      }
      const saveId = activeSave?.id ?? null;
      if (saveId == null) {
        window.__apiToast?.('缺存档上下文,无法重新生成', { kind: 'warn', duration: 2400 });
        return;
      }
      try {
        setHasError(false);
        const r = await window.api.branches.continueFrom({
          save_id: saveId,
          message_index: pIdx,
          label: '重新生成',
        });
  ...
  const onAttachPick = (item) => {
    const fixtures = {
      file: { name: '南陵卷宗.md', kind: 'file' },
      image: { name: '雾港地图.png', kind: 'image' },
      chapter: { name: '第 314 章 · 北港', kind: 'chapter' },
      card: { name: '角色卡 · 沈知微', kind: 'card' },
      world: { name: '世界书 · 残页', kind: 'world' },
      mcp: { name: 'MCP · 文件检索', kind: 'mcp' },
      skill: { name: 'Skill · 角色一致性', kind: 'skill' },
      plan: { name: '计划模式', kind: 'skill' },
    };
  ...
  const onApprove = async (target) => {
    ...
    try {
      await window.api.game.pendingWrite({ id: target.id, index: target.index, action: 'approve' });
    } catch (e) {
      window.__apiToast?.('审批失败', { kind: 'danger', detail: e?.message });
    }
    try {
      const d = await window.api.game.state();
      if (d && d.permissions) {
        setPendingWrites(d.permissions.pending_writes || []);
        setPendingQuestions(d.permissions.pending_questions || []);
      }
    } catch (_) {}
  };
  const onReject = async (target) => {
    setPendingWrites((arr) => arr.filter(_matchPending(target)));
    try {
      await window.api.game.pendingWrite({ id: target.id, index: target.index, action: 'reject' });
    } catch (e) {
      window.__apiToast?.('拒绝失败', { kind: 'danger', detail: e?.message });
    }
    try {
      const d = await window.api.game.state();
      if (d && d.permissions) {
  ...
  const onAnswerQuestion = async (target, choice) => {
    ...
    try {
      await window.api.game.clearQuestions({ id: target.id, index: target.index, choice });
    } catch (e) {
      window.__apiToast?.('回答失败', { kind: 'danger', detail: e?.message });
    }
    try {
      const d = await window.api.game.state();
      if (d && d.permissions) {
  ...
  const rootStyle = useMemo(() => {
    ...
    return {
      '--density': densityMap[t.density] || 1,
      '--ui-size': t.uiSize + 'px',
      '--narrative-size': t.narrativeSize + 'px',
    };
  ...
  useEffect(() => {
    ...
    const raf = requestAnimationFrame(() => {
      requestAnimationFrame(() => setMountStage((s) => Math.min(2, s + 1)));
    });
    ...
    <div className="gc-shell" style={{ ...rootStyle, '--gc-rail-w': gcRailW + 'px' }}>
      ...
      {tabConflictBanner && (
        <div
          style={{
            ...
            right: 0,
            zIndex: 10000,
            background: 'rgba(200,130,0,0.93)',
            color: '#fff',
            padding: '8px 16px',
            display: 'flex',
            alignItems: 'center',
      ...
      {mountStage >= 1 ? (
        <LeftRail
          resizeHandle={
            <div
              className="gc-rail-resize-handle"
              title="拖动调整宽度 · 双击恢复默认"
              {...gcRailDragProps}
            />
          }
          ...
          onNew={() => {
            if (
              !confirm(
                '新建存档需要选择剧本与角色,将跳到平台『存档目录』走正规创建流。\n\n确认跳转?',
              )
            )
              return;
      ...
      <main className="gc-main">
            ...
            const _handleModelChange = async (modelId, _provider) => {
              try {
                if (window.api && window.api.models && window.api.models.select) {
                  ...
                  const cat = await (window.api.models.catalog
                    ? window.api.models.catalog()
                    : Promise.resolve({ models: [] }));
                  const info =
                    cat && Array.isArray(cat.models)
                      ? cat.models.find((m) => m.id === modelId)
                      : null;
                ...
                window.__apiToast?.('切换失败', { kind: 'danger', detail: e && e.message });
              }
              setShowModel(false);
            };
            return (
                ...
                >
                  <div
                    style={{
                      ...
                      borderBottom: '1px solid var(--line-soft,#2a2724)',
                      padding: '12px 16px',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'space-between',
        ...
        {mountStage >= 1 ? (
          <ChatArea
            ...
            runState={runState}
            runStyle={t.runStyle}
            narrativeFont={t.narrativeFont}
            narrativeSize={t.narrativeSize}
            hasError={hasError}
            saveId={
              (activeSave && activeSave.id) || (game && game._raw && game._raw.save_id) || null
            }
            onRetry={onRetry}
            onShowSse={onShowSse}
          />
        ...
        <div className="gc-foot-wrap">
          ...
          <Composer
            ...
            model={model}
            setModel={setModel}
            composerMode={t.composerMode}
            suggestions={game.suggestions}
            gameState={game}
            attachments={attachments}
            removeAttachment={(i) => setAttachments((a) => a.filter((_, j) => j !== i))}
          ...
          {showUsage && lastUsage && (
            ...
            >
              <span title="本轮输入 tokens">
                ↑ {Number(lastUsage.input_tokens || 0).toLocaleString()}
                {lastUsage.cached_input_tokens
                  ? ` · 缓存 ${Number(lastUsage.cached_input_tokens).toLocaleString()}`
                  : ''}
              </span>
              <span title="本轮输出 tokens">
                ↓ {Number(lastUsage.output_tokens || 0).toLocaleString()}
                {lastUsage.reasoning_tokens
                  ? ` · 思考 ${Number(lastUsage.reasoning_tokens).toLocaleString()}`
                  : ''}
              </span>
              {lastUsage.context_max ? (
                <span title="上下文占用">
                  上下文 {Number(lastUsage.context_used || 0).toLocaleString()}/
                  {Number(lastUsage.context_max).toLocaleString()} ·{' '}
                  {Math.round(lastUsage.context_pct || 0)}%
                </span>
              ) : null}
              {lastUsage.cost_usd ? (
                <span title="本轮费用">${Number(lastUsage.cost_usd).toFixed(4)}</span>
              ) : null}
      ...
      {sseLogOpen && (
        ...
        >
          ...
          >
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button
                  ...
                  onClick={async () => {
                    try {
                      await navigator.clipboard.writeText(JSON.stringify(sseLog, null, 2));
                      window.__apiToast?.('已复制全部事件', { kind: 'ok', duration: 1500 });
                    } catch {
                      window.__apiToast?.('复制失败', { kind: 'danger' });
                    }
                  }}
                >
                  复制 JSON
                </button>
                <button className="btn ghost" onClick={() => setSseLogOpen(false)}>
                  关闭
                </button>
              </div>
            </div>
            ...
            >
              {sseLog.length === 0 && (
                <div style={{ padding: '24px 0', color: 'var(--muted, #888)' }}>
                  暂无事件（本轮未开始或已被清空）
                </div>
              )}
              {sseLog.map((ev, i) => (
                <div
                  key={i}
                  style={{ padding: '4px 0', borderBottom: '1px dashed var(--line-soft, #2a2d33)' }}
                >
                  <span style={{ color: 'var(--muted-2, #777)' }}>
      ...
      {mountStage >= 2 && (
        <RightPanel
          ...
          sidebarWidth={gcPanelW}
          density={t.density}
          collapsed={panelCollapsed}
          onToggle={() => setPanelCollapsed((c) => !c)}
          resizeHandle={
            <div
              className="gp-panel-resize-handle"
              title="拖动调整宽度 · 双击恢复默认"
              {...gcPanelDragProps}
            />
          }
      ...
      <button
        className="gc-float-panel-btn"
        onClick={() => {
          setPanelCollapsed(false);
          _panelResize.setSize(320);
        }}
        title="打开状态面板"
      >
        ⌖
