...
function SetGroup({ title, description, actions, children }) {
  ...
  return (
    <CSContainer
      header={
        <CSHeader variant="h2" actions={actions} description={description || undefined}>
          {title}
        </CSHeader>
      }
    >
...
function SettingsPage({ section: sectionProp } = {}) {
  ...
  return (
    <CSSpaceBetween size="l">
      {!external && <CSHeader variant="h1">{t('settings.title')}</CSHeader>}
      {!external && (
        <CSSpaceBetween direction="horizontal" size="xs">
          {SECTIONS.map((s) => (
            <CSButton
              key={s.id}
              variant={section === s.id ? 'primary' : 'normal'}
      ...
      {section === 'preferences' && [
        <PrefSection key="pref" />,
        <CSContainer key="gmstyle">
          <GmStyleEditor scope="user" />
        </CSContainer>,
        <BlackSwanSection key="bs" />,
        <ExtractorSection key="ext" />,
        <ClarifySection key="clar" />,
      ]}
...
function PrefSection() {
  ...
  return (
    <SetGroup title={t('settings.preferences.title')}>
      <SetRow
        label={t('settings.preferences.interface_lang')}
        description={t('settings.preferences.interface_lang_desc')}
      >
        <SetSelect
          value={interfaceLang}
          options={[
          ...
          onChange={(v) => {
            setInterfaceLang(v);
            save('ui_language', v);
            import('../../i18n/index').then((m) => m.changeLanguage(v));
          }}
        />
      </SetRow>
      <SetRow
        label={t('settings.preferences.serif_font')}
        description={t('settings.preferences.serif_font_desc')}
      >
        <CSToggle
          checked={serif}
          onChange={({ detail }) => {
            setSerif(detail.checked);
            save('serif', detail.checked);
          }}
        >
      ...
      <SetRow
        ...
        description={t('settings.preferences.autosave_desc')}
      >
        <CSToggle
          checked={auto}
          onChange={({ detail }) => {
            setAuto(detail.checked);
            save('autosave', detail.checked);
          }}
        >
...
function ExtractorSection() {
  ...
  useEffectPL(() => {
    ...
    (async () => {
      try {
        ...
        const rawApis =
          models?.models?.apis ?? (Array.isArray(models?.apis) ? models.apis : null) ?? [];
  ...
  for (const preferred of ['agent_platform', 'anthropic']) {
    apiOptions.push({
      id: preferred,
      name:
        preferred === 'agent_platform'
          ? 'Agent Platform（JSON mode）'
  ...
  return (
    <SetGroup title={t('settings.extractor.title')}>
      <SetRow
        label={t('settings.extractor.enable')}
        description={t('settings.extractor.enable_desc')}
      >
        <CSToggle
          checked={enabled}
          onChange={({ detail }) => {
            setEnabled(detail.checked);
            save('enabled', detail.checked);
          }}
        >
...
function BlackSwanSection() {
  ...
  return (
    <SetGroup title={t('settings.black_swan.title')}>
      <SetRow
        label={t('settings.black_swan.enable')}
        description={t('settings.black_swan.enable_desc')}
      >
        <CSToggle
          checked={enabled}
          onChange={({ detail }) => {
...
function ClarifySection() {
  ...
  return (
    <SetGroup title={t('settings.clarify.title')}>
      <SetRow
        label={t('settings.clarify.threshold')}
        description={t('settings.clarify.threshold_desc')}
      >
...
function ModelsSection() {
  ...
  const mapModel = React.useCallback(
    (m) => ({
      id: m.real_name || m.id,
  ...
  const loadConfiguredApis = useCallbackPL(async () => {
    ...
    const catalogIds = new Set(
      (Array.isArray(list) ? list : []).map((a) =>
        normalizeApiId(catalogApiIdForCredential(a.api_id || a.id)),
      ),
    );
    const customRows = Object.entries(credMap)
      .filter(
        ([cid, c]) => c.has_key && c.base_url_override && !catalogIds.has(normalizeApiId(cid)),
      )
      .map(([cid, c]) => ({
        id: cid,
        credential_id: cid,
  ...
  const syncRemoteModels = useCallbackPL(
    async (api, opts = {}) => {
      if (!api) return null;
      ...
      try {
        ...
        if (!opts.silent) {
          window.__apiToast?.(t('settings.models.sync_ok', { count: models.length }), {
            kind: 'ok',
            duration: 2200,
          });
        }
        ...
        if (!opts.silent) {
          window.__apiToast?.(t('settings.models.sync_fail'), {
            kind: 'danger',
            detail: e?.message,
          });
        }
    ...
    setApis((arr) =>
      arr.map((a) =>
        a.id === apiId
          ? {
              ...a,
              models: a.models.map((m) => (m.id === mId ? { ...m, enabled: !m.enabled } : m)),
            }
          : a,
      ),
    );
  ...
  const renameModel = async (apiId, mId, display) => {
    ...
    try {
      await window.api.models.upsertModel({ api_id: apiId, real_name: mId, display_name: display });
    } catch (_) {}
  };
  const setModelVisibility = async (apiId, ids) => {
    ...
    if (api) {
      await Promise.all(
        api.models.map((m) =>
          window.api.models
            .visibility({ api_id: apiId, model: m.id, visible: ids.includes(m.id) })
            .catch(() => {}),
        ),
      );
    }
  ...
  const removeModels = async (apiId, ids) => {
    setApis((arr) =>
      arr.map((a) =>
        a.id === apiId ? { ...a, models: a.models.filter((m) => !ids.includes(m.id)) } : a,
      ),
    );
    await Promise.all(
      ids.map((id) =>
      ...
      onDeleteKey={async () => {
        if (
          !(await window.__confirm({
            title: t('settings.models.delete_key_title'),
            message: t('settings.models.delete_key_confirm', { name: selectedApi.name }),
            danger: true,
            confirmText: t('settings.models.delete_key_btn'),
          }))
        )
          return;
        try {
          await window.api.credentials.set({
            api_id: credentialApiIdForCatalog(selectedApi.id),
            api_key: '',
          });
          window.__apiToast?.(t('settings.models.delete_key_ok'), { kind: 'ok' });
          setSelectedApiId(null);
          setApis((arr) =>
            arr.map((a) => (a.id === selectedApi.id ? { ...a, key_set: false, key_hint: '—' } : a)),
          );
          ...
          window.__apiToast?.(t('settings.models.delete_key_fail'), {
            kind: 'danger',
            detail: e?.message,
          });
        }
  ...
  if (apisLoading) {
    return (
      <CSSpaceBetween size="l">
        <CSHeader variant="h1" description={t('settings.models.description')}>
              ...
              {[1, 2].map((j) => (
                <div
                  ...
                  style={{
                    height: 18,
                    borderRadius: 4,
                    background: 'var(--color-background-control-disabled, #3a3a3a)',
                    opacity: 0.5 + j * 0.15,
                    width: j === 1 ? '40%' : '70%',
                  }}
                />
  ...
  return (
    <CSSpaceBetween size="l">
      <CSHeader
        ...
        actions={
          <CSButton variant="primary" iconName="add-plus" onClick={() => setAddingApi(true)}>
            {t('settings.models.add_key')}
          </CSButton>
        }
      >
        {t('settings.models.title')}
          ...
          const apiTableEl = (
            <CSTable
              ...
              onSelectionChange={({ detail }) => {
                const x = detail.selectedItems[0];
                if (x) setSelectedApiId(x.id);
              }}
              onRowClick={({ detail }) => setSelectedApiId(detail.item.id)}
              columnDefinitions={[
                {
                  id: 'name',
                  header: t('settings.models.col_provider'),
                  cell: (a) => (
                    <div>
                      <CSBox fontWeight="bold">{a.name}</CSBox>
                ...
                },
                {
                  ...
                  cell: (a) => (
                    <span className="mono">
                      •••• {a.key_hint || t('settings.models.key_set_hint')}
                    </span>
                  ),
                },
                {
                  id: 'models',
                  header: t('settings.models.col_models'),
                  cell: (a) => `${a.models.filter((m) => m.enabled).length} / ${a.models.length}`,
                },
                {
                  id: 'connectivity',
                  header: t('settings.models.col_connectivity'),
                  cell: (a) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {c.latency_ms ? (
                          <span className="mono muted-2">{c.latency_ms}ms</span>
                        ) : null}
                      </button>
                    );
                  },
      ...
      <EditApiModal
        ...
        onConfirm={async (payload) => {
          ...
          const cfg = PROVIDERS_CONFIG.find(
            (p) =>
              catalogApiIdForCredential(p.id) === catalogId ||
              normalizeApiId(p.id) === credentialId,
          );
          const kind =
            catalogId === 'vertex_ai'
              ? 'vertex_ai'
          ...
          try {
            ...
            if (needsCatalogWrite) {
              try {
                ...
                if (e?.status === 403) {
                  ...
                  window.__apiToast?.(
                    '提供商配置(base_url/proxy)需管理员修改,你的 API 密钥仍会保存',
            ...
            if (payload.api_key && payload.api_key.trim()) {
              try {
                ...
                window.__apiToast?.(t('settings.edit_api.key_save_fail'), {
                  kind: 'warn',
                  detail: e?.message,
                  duration: 4000,
                });
            ...
            window.__apiToast?.(
              addingApi ? t('settings.edit_api.add_ok') : t('settings.edit_api.save_ok'),
              { kind: 'ok' },
            );
            ...
            window.__apiToast?.(t('settings.edit_api.save_fail'), {
              kind: 'danger',
              detail: e?.message,
            });
          }
      ...
      <VisibilityModal
        ...
        onConfirm={(visibleIds) => {
          setModelVisibility(visibilityApi, visibleIds);
          setVisibilityApi(null);
        }}
      />
      <ValidateModal
        open={!!validateApi}
        ...
        onConfirm={(toRemove) => {
          removeModels(validateApi, toRemove);
          setValidateApi(null);
        }}
      />
    </CSSpaceBetween>
    ...
    <CSContainer
      header={
        <CSHeader
          ...
          description={
            <span
              style={{ display: 'inline-flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}
            >
              <span className="mono">{api.id}</span>
              <span style={{ color: 'var(--muted)' }}>
                {t('settings.models.base_url_label')}:{' '}
                <span className="mono">{api.base_url || '—'}</span>
              </span>
              <span style={{ color: 'var(--muted)' }}>
                {t('settings.models.key_label')}:{' '}
                <span className="mono">
                  •••• {api.key_hint || t('settings.models.key_set_hint')}
                </span>
              </span>
            </span>
          }
          actions={
            <CSSpaceBetween direction="horizontal" size="xs">
              <CSButton iconName="edit" onClick={onEdit}>
                {t('settings.models.detail_edit')}
              </CSButton>
              <CSButton iconName="view-full" onClick={onVisibility}>
                {t('settings.models.detail_manage')}
              </CSButton>
              <CSButton iconName="refresh" onClick={onValidate}>
                {t('settings.models.detail_validate')}
              </CSButton>
              <CSButton iconName="remove" onClick={onDeleteKey}>
                {t('settings.models.detail_delete_key')}
              </CSButton>
            </CSSpaceBetween>
          }
        >
    ...
    >
      <CSTabs
        ...
        tabs={[
          {
            id: 'models',
            label: t('settings.models.tab_models', { count: api.models.length }),
            content: (
              <ApiModelsList
                api={api}
          ...
          {
            id: 'usage',
            label: t('settings.models.tab_usage'),
            content:
              usage == null ? (
                ...
                <CSSpaceBetween size="m">
                  <CSKeyValuePairs
                    ...
                    items={[
                      {
                        label: t('settings.models.usage_requests'),
                        value:
                          usage.requests != null ? Number(usage.requests).toLocaleString() : '—',
                      },
                      {
                        label: t('settings.models.usage_input_tokens'),
                        value:
                          usage.input_tokens != null
                            ? Number(usage.input_tokens).toLocaleString()
                            : '—',
                      },
                      {
                        label: t('settings.models.usage_output_tokens'),
                        value:
                          usage.output_tokens != null
                            ? Number(usage.output_tokens).toLocaleString()
                            : '—',
                      },
                      {
                        label: t('settings.models.usage_cost'),
                        value:
                          usage.cost_usd != null ? `$${Number(usage.cost_usd).toFixed(2)}` : '—',
                      },
                    ]}
                  />
                  <CSBox fontSize="body-s" color="text-body-secondary">
                    {t('settings.models.usage_detail')}{' '}
                    <a
                      href="/usage"
                      onClick={(e) => {
                        e.preventDefault();
      ...
      >
        <header className="pl-modal-head">
          <div>
            <div className="pl-modal-eyebrow">
              {t('settings.add_model.eyebrow', { api: api.name })}
            </div>
            <h2 className="pl-modal-title">{t('settings.add_model.title')}</h2>
          </div>
          <button className="iconbtn" onClick={onClose} data-tip={t('common.close')}>
        ...
        <div className="pl-modal-form">
          <div className="pl-field">
            <label>
              {t('settings.add_model.real_name')}{' '}
              <span
                className="muted-2"
                style={{ textTransform: 'none', letterSpacing: 0, marginLeft: 6 }}
              >
                {t('settings.add_model.real_name_hint', { api: api.name })}
              </span>
            </label>
            <input
              className="mono"
          ...
          <div className="pl-field">
            <label>
              {t('settings.add_model.display')}{' '}
              <span
                className="muted-2"
                style={{ textTransform: 'none', letterSpacing: 0, marginLeft: 6 }}
              >
                {t('settings.add_model.display_hint')}
              </span>
            </label>
            <input
              value={form.display}
              onChange={(e) => setForm((f) => ({ ...f, display: e.target.value }))}
              placeholder="GPT-4o · RPG"
            />
          </div>
          <div className="pl-field">
            <label>
              {t('settings.add_model.caps')}{' '}
              <span
                className="muted-2"
                style={{ textTransform: 'none', letterSpacing: 0, marginLeft: 6 }}
              >
                {t('settings.add_model.caps_hint')}
              </span>
            </label>
            <div className="pl-rules">
              {Object.keys(CAP_LABEL).map((c) => (
                <button
                  key={c}
                  className={`pl-rule-chip ${form.capabilities.includes(c) ? 'active' : ''}`}
                  onClick={() => toggleCap(c)}
                >
                  {CAP_LABEL[c]}
                </button>
        ...
        <footer className="pl-modal-foot">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button
              ...
              disabled={!form.real_name || !form.display}
...
function EditApiModal({ open, api, isNew, isAdminUser = false, onClose, onConfirm }) {
  ...
  React.useEffect(() => {
    ...
    if (isNew) {
      setProvider('');
      setForm({ id: '', name: '', base_url: '', api_key: '', proxy: 'direct' });
    } else if (api) {
      setProvider(api.id);
      setForm({
        id: api.id,
  ...
  const provOptions = [
    ...PROVIDERS_CONFIG.filter((p) => !p.hidden_in_edit_modal).map((p) => ({
      value: p.id,
      label: p.name,
      description: p.defaultBase || undefined,
    })),
    {
      value: CUSTOM,
      label: t('settings.edit_api.custom_provider'),
      description: t('settings.edit_api.custom_provider_desc'),
    },
  ];
  const onPickProvider = (val) => {
    setProvider(val);
    if (val === CUSTOM) {
      setForm((f) => ({ ...f, id: '', name: '', base_url: '' }));
      return;
    }
  ...
  const isAgentPlatform =
    selectedProviderCfg?.special === 'agent_platform' || api?.kind === 'vertex_ai';
  ...
  return (
    <CSModal
      ...
      header={
        isNew
          ? t('settings.edit_api.add_title')
          : t('settings.edit_api.edit_title', { name: api?.name || '' })
      }
      footer={
        <CSBox float="right">
          <CSSpaceBetween direction="horizontal" size="xs">
            ...
            <CSButton variant="primary" disabled={!canSubmit} onClick={() => onConfirm(form)}>
              {isNew ? t('settings.edit_api.add_btn') : t('settings.edit_api.save_btn')}
            </CSButton>
          </CSSpaceBetween>
        </CSBox>
      }
    >
      <CSSpaceBetween size="l">
        {isNew && (
          <CSFormField
            label={t('settings.edit_api.provider')}
            description={t('settings.edit_api.provider_desc')}
          >
            <CSSelect
          ...
          <CSColumnLayout columns={2}>
            <CSFormField label={t('settings.edit_api.id_field')}>
              <CSInput
                value={form.id}
                disabled={!isNew}
                onChange={({ detail }) => setForm((f) => ({ ...f, id: detail.value }))}
                placeholder="openai"
              />
            </CSFormField>
          ...
          <>
            ...
            {isAgentPlatform ? (
              <CSFormField
                ...
                description={
                  api?.key_set
                    ? `已配置 SA (${api.key_hint || '已加密'})，留空保持不变`
                    : '请粘贴 Google Cloud Service Account JSON 文件内容'
                }
              >
                <textarea
                  rows={6}
                  ...
                  placeholder={
                    '{"type": "service_account", "project_id": "...", "client_email": "...", "private_key": "<PRIVATE_KEY_PEM_WITH_NEWLINES>"}'
                  }
                  style={{
                    width: '100%',
                ...
                {_saJsonValid && (
                  <div
                    style={{
                      color: 'var(--color-text-status-success, #1a7e3c)',
                      fontSize: '12px',
                      marginTop: '4px',
                    }}
                  >
                    ...
                    {(() => {
                      try {
                        return JSON.parse(form.api_key).project_id;
                      } catch {
                        return '';
                      }
                    })()}
              ...
              <>
                <CSFormField label={t('settings.edit_api.base_url')}>
                  <CSInput
                    value={form.base_url}
                    onChange={({ detail }) => setForm((f) => ({ ...f, base_url: detail.value }))}
                    placeholder="https://your-relay.example.com/v1"
                  />
                </CSFormField>
                <CSFormField
                  label={t('settings.edit_api.api_key')}
                  description={
                    api?.key_set
                      ? t('settings.edit_api.api_key_desc_set', {
                          hint: api.key_hint || t('settings.models.key_set_hint'),
                        })
                      : t('settings.edit_api.api_key_desc_new')
                  }
                >
                  <CSInput
                    type="password"
            ...
            <CSFormField label={t('settings.edit_api.connection')}>
              <CSSelect
                ...
                options={[
                  { value: 'direct', label: t('settings.edit_api.direct') },
                  { value: 'http_proxy', label: t('settings.edit_api.http_proxy') },
                  { value: 'lan', label: t('settings.edit_api.lan') },
                ]}
                onChange={({ detail }) =>
                  setForm((f) => ({ ...f, proxy: detail.selectedOption.value }))
                }
              />
            </CSFormField>
          </>
  ...
  return (
    <div className="pl-modal-backdrop" onClick={onClose}>
      <div
        className="pl-modal"
        onClick={(e) => e.stopPropagation()}
        style={{ width: 'min(640px, 100%)', maxHeight: '88vh' }}
      >
        <header className="pl-modal-head">
          <div>
            <div className="pl-modal-eyebrow">
              {t('settings.visibility.eyebrow', { name: api.name })}
            </div>
            <h2 className="pl-modal-title">
              {t('settings.visibility.title', {
                selected: selected.size,
                total: api.models.length,
              })}
            </h2>
          </div>
          <button className="iconbtn" onClick={onClose} data-tip={t('common.close')}>
        ...
        <div className="pl-model-search" style={{ flex: '0 0 auto' }}>
          ...
          <input
            ...
            placeholder={t('settings.visibility.search_placeholder', { count: api.models.length })}
            autoFocus
          />
          {q && (
            <button className="iconbtn" onClick={() => setQ('')} style={{ width: 18, height: 18 }}>
        ...
        <div className="pl-vis-toolbar">
          <button className="btn ghost" onClick={toggleAll}>
            {allVisible ? (
              <>
                <Icon name="eye_off" size={12} /> {t('settings.visibility.hide_all')}
              </>
            ) : (
              <>
                <Icon name="eye" size={12} /> {t('settings.visibility.show_all')}
              </>
            )}
          </button>
          <span className="muted-2 mono" style={{ marginLeft: 'auto', fontSize: 11 }}>
            {t('settings.visibility.matched', {
              count: filtered.length,
              selected: filtered.filter((m) => selected.has(m.id)).length,
            })}
          </span>
        </div>
                      ...
                      return (
                        <>
                          {caps.slice(0, 2).map((c) => (
                            <span key={c} className="pl-cap-tag">
...
function ValidateModal({ open, api, onClose, onConfirm }) {
  ...
  const wrap = (arr) =>
    (arr || []).map((s) => (typeof s === 'string' ? { real_name: s, display: s } : s));
  ...
  const toRemoveList = [
    ...localOnly,
    ...unreachable.filter((u) => !localOnly.some((r) => r.real_name === u.real_name)),
  ];
  ...
  const addAll = async () => {
    ...
    window.__apiToast?.(
      fail
        ? t('settings.validate.add_ok_fail', { ok, fail })
        : t('settings.validate.add_ok', { ok }),
      { kind: ok ? 'ok' : 'danger', duration: 3000 },
    );
    if (typeof window.__refreshPlatform === 'function') {
      try {
        await window.__refreshPlatform();
    ...
    <div className="pl-modal-backdrop" onClick={onClose}>
      ...
      >
        <header className="pl-modal-head">
          <div>
            <div className="pl-modal-eyebrow">
              {t('settings.validate.eyebrow', { name: api.name })}
            </div>
            <h2 className="pl-modal-title">
        ...
        {phase === 'sniffing' ? (
          <div className="pl-validate-progress">
            <div className="pl-validate-step done">
              <span className="dot ok" /> {t('settings.validate.step1')}
            </div>
            <div className="pl-validate-step running">
              <Icon name="spinner" size={12} className="spin" /> {t('settings.validate.step2')}
            </div>
          </div>
        ) : err ? (
          ...
          <div className="pl-validate-result">
            <div className="pl-validate-stat-row">
              <div className="pl-validate-stat">
                ...
                <span className="pl-stat-value" style={{ fontSize: 20 }}>
                  {api.models.length}
                </span>
              </div>
              <div className="pl-validate-stat">
                ...
                <span className="pl-stat-value" style={{ fontSize: 20 }}>
                  {remoteOnly.length + kept.length}
                </span>
              </div>
              <div className="pl-validate-stat">
                <span className="pl-stat-label accent">{t('settings.validate.stat_new')}</span>
                <span className="pl-stat-value accent" style={{ fontSize: 20 }}>
                  {remoteOnly.length}
                </span>
              </div>
              <div className="pl-validate-stat">
                <span className="pl-stat-label danger">
                  {t('settings.validate.stat_local_extra')}
                </span>
                <span className="pl-stat-value danger" style={{ fontSize: 20 }}>
                  {localOnly.length}
                </span>
              </div>
            </div>
            ...
            {remoteOnly.length > 0 && (
              <div className="pl-validate-section">
                <div className="pl-validate-section-head">
                  ...
                  {t('settings.validate.new_models', { count: remoteOnly.length })}
                  <button
                    className="btn ghost"
                  ...
                  >
                    {adding ? (
                      <>
                        ...
                        {t('settings.validate.adding')}
                      </>
                    ) : (
                      <>
                        <Icon name="plus" size={11} /> {t('settings.validate.add_all')}
                      </>
                    )}
                  </button>
                </div>
            ...
            {toRemoveList.length > 0 && (
              <div className="pl-validate-section">
                <div className="pl-validate-section-head">
                  ...
                  {t('settings.validate.local_extra', { count: toRemoveList.length })}
                  <span className="muted-2" style={{ marginLeft: 6, fontSize: 11 }}>
                    {t('settings.validate.local_extra_hint')}
                  </span>
                </div>
                <ul className="pl-validate-list">
                  {toRemoveList.map((m) => (
                    <li
                      key={m.id || m.real_name}
                      className={removeIds.has(m.id || m.real_name) ? 'marked' : ''}
                    >
                      <input
                        type="checkbox"
                        checked={removeIds.has(m.id || m.real_name)}
                        onChange={() => toggleRemove(m.id || m.real_name)}
                      />
                      <HealthDot health={m.health} statusDetail={m.health_status_detail} />
                      ...
                      <span className="pill danger" style={{ fontSize: 10.5 }}>
                        {m.health === 'err'
                          ? t('settings.validate.unreachable')
                          : t('settings.validate.remote_missing')}
                      </span>
                    </li>
                  ))}
                </ul>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {phase === 'done' && removeIds.size > 0 && (
              <button className="btn danger" onClick={() => onConfirm([...removeIds])}>
                ...
                {t('settings.validate.delete_btn', { count: removeIds.size })}
              </button>
            )}
          </div>
        </footer>
...
function ApiModelsList({ api, onToggleModel, onRenameModel }) {
  ...
  const filtered = visibleModels.filter((m) => {
    if (q) {
      ...
      if (!m.display.toLowerCase().includes(s) && !m.real_name.toLowerCase().includes(s))
        return false;
    }
    ...
    <>
        ...
        <div className="pl-model-toolbar">
          <div className="pl-model-search">
            ...
            <input
              ...
              placeholder={t('settings.model_list.search_placeholder', {
          ...
          <div className="seg" style={{ flexShrink: 0 }}>
            ...
            >
              {t('settings.model_list.filter_all')}{' '}
              <span className="muted-2" style={{ marginLeft: 4, fontSize: 10.5 }}>
            ...
            >
              {t('settings.model_list.filter_enabled')}{' '}
              <span className="muted-2" style={{ marginLeft: 4, fontSize: 10.5 }}>
                {visibleModels.filter((m) => m.enabled).length}
              </span>
            </button>
            <button
              className={statusFilter === 'err' ? 'active' : ''}
              onClick={() => setStatusFilter('err')}
            >
              {t('settings.model_list.filter_err')}{' '}
              <span className="muted-2" style={{ marginLeft: 4, fontSize: 10.5 }}>
                {visibleModels.filter((m) => m.health === 'err').length}
              </span>
            </button>
          </div>
          <select
      ...
      {showSearch && allCaps.length > 0 && (
        <div className="pl-model-caps-row">
          ...
          >
            {t('settings.model_list.caps_label')}
          </span>
          {allCaps.map((c) => (
            <button
              key={c}
      ...
      {sorted.length === 0 ? (
        <div className="pl-model-empty">
          ...
          {filtersActive && (
            ...
            >
              {t('settings.model_list.clear_filter')}
            </button>
          )}
        </div>
      ) : (
          ...
          columnDefinitions={[
            ...
            },
            {
              ...
              cell: (m) => (
                <ModelNameCell
                  m={m}
                  onRename={(v) => onRenameModel?.(m.id, v)}
                ...
                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  {getCaps(m).map((c) => (
                    <span
                      ...
                      data-tip={t('settings.capabilities.' + c, {
                        defaultValue: CAP_LABEL[c] || c,
                      })}
                    >
                      {t('settings.capabilities.' + c, { defaultValue: CAP_LABEL[c] || c })}
                    </span>
              ...
              cell: (m) => (
                <span className="mono muted">
                  ...
                  {m.input_cost_per_million != null ? (
                    <span
                      data-tip={`输入 $${m.input_cost_per_million}/M · 输出 $${m.output_cost_per_million ?? '?'}/M`}
                    >
                ...
                <span className="mono muted">
                  ...
                  {m.max_output_tokens != null && (
                    <div
                      className="muted-2"
                      style={{ fontSize: 10 }}
                      data-tip={`最大输出 ${fmtCtx(m.max_output_tokens)} tokens`}
                    >
                ...
                return (
                  <span style={{ fontSize: 11 }} className="muted-2">
                    ...
                    {m.source ? (
                      <span
                        className="pl-cap-tag"
                        data-tip={`数据来源: ${sourceLabel(m.source)}`}
                        style={{ fontSize: 10 }}
                      >
                    ...
                    {isDeprecated && (
                      <span
                        className="pl-cap-tag"
                        data-tip={`deprecated: ${m.deprecated_at}`}
                        style={{
                          marginLeft: 2,
                          color: 'var(--warn)',
                          fontSize: 10,
    ...
    <div className="pl-title-cell">
      <strong
        ...
        data-tip={
          deprecated
            ? `deprecated · ${m.deprecated_at || ''}`
            : t('settings.model_list.tip_double_click')
        }
      >
        {m.display}
        {deprecated && (
          <span style={{ marginLeft: 4, fontSize: 11, color: 'var(--warn)' }}>
      ...
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
        <span
          ...
          style={{
            ...
            color: 'var(--warn,#d4823c)',
            borderColor: 'var(--warn,#d4823c)',
            cursor: 'pointer',
            fontSize: 10.5,
          }}
...
const MODELS_DATA = [
    ...
    models: [
      {
        ...
        real_name: 'gpt-5.5',
        display: 'GPT-5.5 · 标准',
        capabilities: ['text', 'vision', 'tool-use', 'rpg'],
        enabled: true,
        price: '$2.50 / $10.00',
        context: '400K',
        health: 'ok',
      ...
      {
        id: 'gpt-5.5-instant',
        real_name: 'gpt-5.5-instant',
        display: 'GPT-5.5 Instant · 低延迟',
    ...
    models: [
      {
        ...
        display: 'Claude Opus 4.7 · 长文',
        capabilities: ['long', 'tool-use', 'rpg'],
        enabled: true,
        price: '$15 / $75',
        context: '200K',
        health: 'ok',
  ...
  },
  {
    ...
    models: [
      {
        id: 'gemini-3.5-flash',
        real_name: 'gemini-3.5-flash',
        display: 'Gemini 3.5 Flash · 当前默认',
      ...
      {
        id: 'gemini-3.1-pro',
        real_name: 'gemini-3.1-pro',
        display: 'Gemini 3.1 Pro',
        capabilities: ['long', 'vision', 'tool-use'],
        enabled: false,
        price: '$2.00 / $12.00',
        context: '1M',
        health: 'ok',
  ...
  },
  {
    ...
    models: [
      {
        id: 'qwen3.7-max',
        real_name: 'qwen3.7-max',
        display: 'Qwen 3.7-Max · 旗舰',
        capabilities: ['cn', 'rpg', 'text', 'reasoning'],
        enabled: true,
        price: '$2.50 / $7.50',
        context: '1M',
        health: 'ok',
      ...
      {
        id: 'qwen3.6-flash',
        real_name: 'qwen3.6-flash',
        display: 'Qwen 3.6 Flash',
        capabilities: ['cn', 'fast'],
        enabled: true,
        price: '$0.19 / $1.13',
        context: '131K',
        health: 'ok',
  ...
  },
  {
    ...
    models: [
      {
        id: 'deepseek-v4-pro',
        real_name: 'deepseek-ai/DeepSeek-V4-Pro',
        display: 'DeepSeek V4-Pro · 旗舰',
        capabilities: ['reasoning', 'cn', 'tool-use'],
        enabled: true,
        price: '$1.74 / $3.48',
        context: '1M',
        health: 'ok',
      ...
      },
      {
        ...
        real_name: 'deepseek-ai/DeepSeek-V4-Flash',
        display: 'DeepSeek V4-Flash · 快速',
        capabilities: ['cn', 'fast'],
        enabled: true,
        price: '$0.30 / $1.20',
        context: '1M',
  ...
  },
  {
    ...
    models: [
      {
        id: 'qwen-72b',
        real_name: 'Qwen2.5-72B-Instruct',
        display: 'Qwen2.5-72B · 本地',
        capabilities: ['cn', 'long'],
        enabled: false,
        price: '本地',
        context: '128K',
        health: 'ok',
...
const PROVIDERS_CONFIG = [
  ...
  {
    id: 'AgentPlatform',
    name: 'Agent Platform (Service Account)',
    kind: 'native',
    defaultBase: '',
    keyEnv: '',
  ...
  const saveKey = async (providerId, apiKey, baseUrl) => {
    ...
    try {
        ...
        const kind =
          providerId === 'AgentPlatform'
            ...
            : providerId === 'anthropic'
              ...
              : 'openai_compat';
        await window.api.models.upsertApi({
          api_id: catalogApiIdForCredential(providerId),
          base_url: baseUrl,
          kind,
          display_name: cfg?.name || providerId,
        });
      }
      ...
      setCreds((s) => ({
        ...
        [providerId]: {
          ...s[providerId],
          has_key: !!(apiKey?.trim() || s[providerId]?.has_key),
          base_url: baseUrl ?? s[providerId]?.base_url,
        },
      }));
    } catch (e) {
      window.__apiToast?.(t('settings.providers.save_fail'), {
        kind: 'danger',
        detail: e?.message,
      });
    } finally {
...
function ProviderCard({
  ...
  useEffectPL(() => {
    setBaseVal(cred.base_url || p.defaultBase || '');
  }, [cred.base_url, p.defaultBase]);
    ...
    return (
      <CSContainer>
        <CSSpaceBetween size="s">
          ...
          {agentPlatformError && <CSAlert type="error">{agentPlatformError}</CSAlert>}
        ...
        <CSSpaceBetween size="s">
          ...
          <CSSpaceBetween direction="horizontal" size="xs" alignItems="center">
            <div className="seg" style={{ display: 'flex' }}>
              <button
                className={alibabaMode === 'openai_compat' ? 'active' : ''}
                onClick={() => onAlibabaMode('openai_compat')}
              >
                OpenAI-compat
              </button>
              <button
                className={alibabaMode === 'native' ? 'active' : ''}
                onClick={() => onAlibabaMode('native')}
              >
                Native DashScope
              </button>
            </div>
        ...
        <CSSpaceBetween key="form" direction="horizontal" size="xs" alignItems="flex-end">
          <CSFormField label={t('settings.edit_api.api_key')} stretch>
            <CSInput
              ...
              onChange={({ detail }) => setKeyVal(detail.value)}
              placeholder={
                cred.has_key ? t('settings.providers.keep_key') : p.keyEnv ? p.keyEnv : 'sk-…'
              }
              autoComplete="new-password"
            />
          </CSFormField>
          <CSFormField
            label={
              p.special === 'openrouter'
                ? t('settings.providers.base_url_relay')
          ...
          <CSButton
            ...
            loading={isSaving}
            disabled={
              isSaving || (!keyVal.trim() && baseVal === (cred.base_url || p.defaultBase || ''))
            }
            onClick={() => onSaveKey(p.id, keyVal, baseVal)}
          >
            {t('common.save')}
...
function readScopedPref(prefs, key, fallback) {
  if (prefs && Object.prototype.hasOwnProperty.call(prefs, `settings.${key}`))
    return prefs[`settings.${key}`];
...
function ModelParamsSection() {
  ...
  useEffectPL(() => {
    ...
    (async () => {
      try {
        ...
        const nsfwMode = String(
          readScopedPref(prefs, 'nsfw_mode', legacyNsfw.mode || 'soft') || 'soft',
        );
        const nsfwIntensity = Number(
          readScopedPref(prefs, 'nsfw_intensity', legacyNsfw.intensity ?? 0.5),
        );
        setNsfw({
          ...
          extra_prompt: String(
            readScopedPref(
              prefs,
              'nsfw_extra_prompt',
              legacyNsfw.extra_prompt || legacyNsfw.extra || '',
            ) || '',
          ),
        });
  ...
  const updateNsfw = (patch) => {
    ...
    if (Object.prototype.hasOwnProperty.call(patch, 'intensity'))
      save('nsfw_intensity', patch.intensity);
    if (Object.prototype.hasOwnProperty.call(patch, 'extra_prompt'))
      save('nsfw_extra_prompt', patch.extra_prompt);
  };

  return (
    <SetGroup
      title={t('settings.modelparams.title')}
      description={t('settings.modelparams.description')}
    >
      <SetRow
        label={t('settings.modelparams.preset')}
        description={t('settings.modelparams.preset_desc')}
      >
        <CSSpaceBetween direction="horizontal" size="xs">
          {PRESETS.map((p) => (
            <CSButton
      ...
      <ParamSlider
        label="Temperature"
        desc="Higher = more random; 0 = most deterministic; recommended 0.4–1.0"
      ...
      {showReasoningEffort && (
        <SetRow
          label={t('settings.modelparams.reasoning_effort')}
          description={t('settings.modelparams.reasoning_desc')}
        >
          <CSSpaceBetween direction="horizontal" size="xs">
            {['low', 'medium', 'high'].map((lv) => (
              <CSButton
                key={lv}
                variant={reasoningEffort === lv ? 'primary' : 'normal'}
                onClick={() => {
                  setReasoningEffort(lv);
                  save('reasoning_effort', lv);
                }}
              >
                {lv === 'low'
                  ? t('settings.modelparams.effort_low')
                  : lv === 'medium'
                    ? t('settings.modelparams.effort_medium')
                    : t('settings.modelparams.effort_high')}
              </CSButton>
            ))}
          </CSSpaceBetween>
        </SetRow>
      ...
      <ParamSlider
        label="Top-p"
        desc="Cumulative probability cutoff; 0.9–0.95 is typical"
        value={params.top_p}
        min={0}
        max={1}
        step={0.01}
      ...
      <ParamSlider
        label="Top-k"
        desc="Sample only from the top K tokens; 0 = disabled"
        value={params.top_k}
        min={0}
        max={200}
        step={1}
      ...
      <ParamSlider
        ...
        desc="Suppresses recently used tokens; 1.0 = no effect; 1.15–1.2 typical"
        ...
        onChange={(v) => {
          setPreset('custom');
          u('repetition_penalty', v);
        }}
      />

      <ParamSlider
        label="Frequency Penalty"
        desc="OpenAI-style: adjusts based on token frequency so far"
        ...
        onChange={(v) => {
          setPreset('custom');
          u('frequency_penalty', v);
        }}
      />

      <ParamSlider
        label="Presence Penalty"
        desc="OpenAI-style: adjusts based on whether token has appeared"
        ...
        onChange={(v) => {
          setPreset('custom');
          u('presence_penalty', v);
        }}
      />

      <SetRow
        label={t('settings.modelparams.max_tokens')}
        description={t('settings.modelparams.max_tokens_desc')}
      >
        <CSInput
          type="number"
          ...
          onChange={({ detail }) => {
            setPreset('custom');
            u('max_tokens', Number(detail.value));
          }}
        />
      </SetRow>

      <SetRow
        label={t('settings.modelparams.context_size')}
        description={t('settings.modelparams.context_size_desc')}
      >
        <SetSelect
      ...
      <SetRow
        label={t('settings.modelparams.seed')}
        description={t('settings.modelparams.seed_desc')}
      >
        <CSInput
          type="number"
      ...
      <SetRow
        label={t('settings.modelparams.stop')}
        description={t('settings.modelparams.stop_desc')}
      >
        <CSInput
          value={params.stop}
          onChange={({ detail }) => u('stop', detail.value)}
          placeholder="player:|system:"
        />
      </SetRow>

      <SetRow
        label={t('settings.modelparams.nsfw')}
        description={t('settings.modelparams.nsfw_desc')}
      >
        <CSSpaceBetween direction="horizontal" size="xs">
          <CSButton
            variant={nsfw.mode === 'block' ? 'primary' : 'normal'}
            onClick={() => updateNsfw({ mode: 'block' })}
          >
            {t('settings.modelparams.nsfw_block')}
          </CSButton>
          <CSButton
            variant={nsfw.mode === 'soft' ? 'primary' : 'normal'}
            onClick={() => updateNsfw({ mode: 'soft' })}
          >
            {t('settings.modelparams.nsfw_soft')}
          </CSButton>
          <CSButton
            variant={nsfw.mode === 'open' ? 'primary' : 'normal'}
            onClick={() => updateNsfw({ mode: 'open' })}
          >
            {t('settings.modelparams.nsfw_open')}
          </CSButton>
          <CSButton
            variant={nsfw.mode === 'explicit' ? 'primary' : 'normal'}
            onClick={() => updateNsfw({ mode: 'explicit' })}
          >
            {t('settings.modelparams.nsfw_explicit')}
          </CSButton>
        </CSSpaceBetween>
      </SetRow>
      ...
      {nsfw.mode !== 'block' && (
        <ParamSlider
          label={t('settings.modelparams.nsfw_intensity')}
          desc={t('settings.modelparams.nsfw_intensity_desc')}
      ...
      <SetRow
        label={t('settings.modelparams.nsfw_extra')}
        description={t('settings.modelparams.nsfw_extra_desc')}
      >
        <CSInput
      ...
      <SetRow
        label={t('settings.modelparams.mirostat')}
        description={t('settings.modelparams.mirostat_desc')}
      >
        <CSToggle checked={advanced} onChange={({ detail }) => setAdvanced(detail.checked)}>
          {advanced
            ? t('settings.modelparams.mirostat_on')
            : t('settings.modelparams.mirostat_off')}
        </CSToggle>
      </SetRow>
      ...
      {advanced && (
        <>
          <SetRow
            label={t('settings.modelparams.mirostat_mode')}
            description={t('settings.modelparams.mirostat_mode_desc')}
          >
            <CSSpaceBetween direction="horizontal" size="xs">
              {['off', 'v1', 'v2'].map((m) => (
                <CSButton
                  key={m}
                  variant={params.mirostat_mode === m ? 'primary' : 'normal'}
                  onClick={() => u('mirostat_mode', m)}
                >
                  {m === 'off' ? t('settings.modelparams.mirostat_off_btn') : m}
                </CSButton>
              ))}
            </CSSpaceBetween>
          </SetRow>
          <ParamSlider
            ...
            desc="Target perplexity; 5 is a common value"
      ...
      <SetRow
        label={t('settings.modelparams.preview_json')}
        description={t('settings.modelparams.preview_json_desc')}
      >
        <pre
          className="mono"
          style={{
          ...
          {JSON.stringify(
            {
              ...
              nsfw:
                nsfw.mode === 'block'
                  ? null
                  : { mode: nsfw.mode, intensity: nsfw.intensity, extra: nsfw.extra_prompt },
              ...(advanced
                ? {
...
function ModuleModelsSection() {
  ...
  const MODULES = [
    {
      ...
      shape: 'flat',
      apiKey: 'gm.api_id',
      modelKey: 'gm.model_real_name',
      tip: '玩家对话默认使用的主模型。这里选择后会写入个人默认模型,新开局和未单独切模型的存档会优先使用它。',
    },
    {
      id: 'sub_agent',
      label: '上下文子代理',
      shape: 'dict',
      overrideKey: 'sub_agent_model_override',
      tip: '整理玩家意图 + 检索计划的子代理;空 = 跟主 GM 共享实例。',
    },
    {
      id: 'set_parser',
      label: '指令解析代理',
      shape: 'flat',
      apiKey: 'set_parser.api_id',
      modelKey: 'set_parser.model_real_name',
      tip: '/set 命令自然语言解析子代理。',
    },
    {
      id: 'console',
      label: '控制台助手',
      shape: 'dict',
      overrideKey: 'console_assistant_model_override',
      tip: '侧栏控制台助手专用模型;空 = 跟主 GM。',
    },
    {
      id: 'extractor',
      label: '叙事提取器',
      shape: 'flat',
      apiKey: 'extractor.api_id',
      modelKey: 'extractor.model_real_name',
      tip: 'GM 叙事二次解析抽 ops (两步式 GM 第二步)。',
    },
    {
      id: 'card_gen',
      label: '角色卡生成器',
      shape: 'flat',
      apiKey: 'character_card_generator.api_id',
      modelKey: 'character_card_generator.model_real_name',
      tip: '侧栏创意工具:生成 / 微调角色卡。',
    },
    {
      id: 'card_import',
      label: 'AI 整理卡字段',
      shape: 'flat',
      apiKey: 'card_import.api_id',
      modelKey: 'card_import.model_real_name',
      tip: '导入酒馆卡时,用 LLM 把一整段自由文本档案整理成结构化字段(身份/背景/外貌/性格等)。仅在导入勾选「用 AI 整理字段」时调用;空 = 跟主 GM。',
    },
    {
      id: 'critic',
      label: '一致性评分',
      shape: 'flat',
      apiKey: 'critic.api_id',
      modelKey: 'critic.model_real_name',
      tip: '角色卡生成的一致性评分子代理 (0-1 阈值 0.6)。',
    },
    {
      id: 'verifier',
      label: '接受条件验证',
      shape: 'flat',
      apiKey: 'acceptance_verifier.api_id',
      modelKey: 'acceptance_verifier.model_real_name',
      tip: 'GM 输出是否满足 curator 设置的 acceptance 条件。',
    },
    {
      id: 'phase_digest',
      label: '阶段浓缩 (compact)',
      shape: 'flat',
      apiKey: 'phase_digest.api_id',
      modelKey: 'phase_digest.model_real_name',
      tip: '长局历史按阶段浓缩成摘要(compact),供 GM 记忆远期剧情;空 = 系统默认。',
    },
    {
      id: 'black_swan',
      label: '黑天鹅事件代理',
      shape: 'flat',
      apiKey: 'black_swan_agent.api_id',
      modelKey: 'black_swan_agent.model_real_name',
      tip: '主动触发世界突发事件的子代理;空 = 系统默认。',
    },
    {
      id: 'agent',
      label: '通用子代理兜底',
      shape: 'flat',
      apiKey: 'agent.api_id',
      modelKey: 'agent.model_real_name',
      tip: '未单独配置模型的其它子代理统一兜底用它;空 = 跟主 GM / 系统默认。',
    },
    {
      id: 'embedder',
      label: '向量嵌入 (RAG)',
      shape: 'flat',
      apiKey: 'embed.api_id',
      modelKey: 'embed.model_real_name',
      capsFilter: ['embedding'],
      allowInherit: false,
      defaultApiId: 'vertex_ai',
      defaultModelId: 'text-embedding-004',
      credentialApiId: 'AgentPlatform',
      tip: '向量嵌入模型，用于 RAG 召回 + 拆书后的语义检索。系统默认 Vertex text-embedding-004，需要在「API 密钥」配 Vertex SA JSON 才能用。可改成其他 embedding 模型。',
    },
  ];
  ...
  const mainCurrent = useMemoPL(() => {
    ...
    if (
      a &&
      m &&
      flatModels.some((x) => x.api_id === catalogApiIdForCredential(a) && x.real_name === m)
    ) {
      return { api_id: catalogApiIdForCredential(a), real_name: m };
    }
    if (flatModels.length)
      return { api_id: flatModels[0].api_id, real_name: flatModels[0].real_name };
    return null;
  }, [prefs, flatModels]);
  ...
  const handleChange = async (mod, value) => {
    ...
    try {
      ...
      if (value === '__inherit__') {
        ...
        if (mod.shape === 'dict') {
          calls.push(
            window.api.account.preferences({ [mod.overrideKey]: { api_id, model: real_name } }),
          );
        } else {
      ...
      await reload();
      window.toast?.(t('settings.modules.save_ok', { label: mod.label }), {
        kind: 'ok',
        duration: 1800,
      });
    } catch (e) {
      window.toast?.(t('settings.modules.save_fail', { label: mod.label }), {
        kind: 'danger',
        detail: e?.message,
        duration: 3200,
      });
  ...
  const resetAll = async () => {
    ...
    try {
      ...
      window.toast?.(t('settings.modules.reset_fail'), {
        kind: 'danger',
        detail: e?.message,
        duration: 3000,
      });
          ...
          <thead>
            <tr>
              <th style={{ textAlign: 'left', padding: '6px 8px' }}>
                {t('settings.modules.col_module')}
              </th>
              <th style={{ textAlign: 'left', padding: '6px 8px' }}>
                {t('settings.modules.col_current')}
              </th>
              <th style={{ textAlign: 'left', padding: '6px 8px' }}>
                {t('settings.modules.col_override')}
              </th>
            </tr>
          </thead>
          <tbody>
            {MODULES.map((mod) => {
              ...
              const value =
                mod.shape === 'dict'
                  ? (() => {
                      const v = prefs[mod.overrideKey];
                      return v && (v.api_id || v.model)
                        ? `${catalogApiIdForCredential(v.api_id || '')}/${v.model || ''}`
                        : '__inherit__';
                    })()
                  : mod.id === 'gm'
                    ? cur
                      ? `${cur.api_id}/${cur.real_name}`
                      : ''
              ...
              return (
                <tr key={mod.id} style={{ borderTop: '1px solid var(--pl-line, #eee)' }}>
                  ...
                  <td style={{ padding: '8px 8px', verticalAlign: 'top' }} className="mono">
                    {isInherit ? (
                      <span className="muted-2" data-tip={t('settings.modules.inherit_tip')}>
                        {t('settings.modules.follow_main')}
                      </span>
                    ) : cur ? (
                      <div>
                        <span>
                        ...
                        {cur.is_default && (
                          <span className="muted-2" style={{ fontSize: 11, marginLeft: 6 }}>
                            (系统默认)
                          </span>
                        )}
                        {!hasCred && (
                          <div
                            style={{
                              marginTop: 4,
                              fontSize: 11,
                              color: 'var(--color-text-status-warning, #d18a00)',
                            }}
                          >
                        ...
                        {mod.id === 'embedder' &&
                          ...
                          (embedderStatus.fallback_active ? (
                            ...
                            <div style={{ marginTop: 4, fontSize: 11, color: '#d18a00' }}>
                              ⚠ 普通用户不享受平台兜底。请配自己的 embedder key(Gemini 免费 1500 RPM
                              / OpenAI $0.02/M / Cohere),否则 RAG 召回降级。
                            </div>
                          ) : null)}
                      </div>
                    ) : (
                  ...
                  <td style={{ padding: '8px 8px', verticalAlign: 'top' }}>
                    ...
                    {(() => {
                      ...
                      if (mod.id !== 'gm' && mod.allowInherit !== false) {
                        opts.push({
                          value: '__inherit__',
                          label: t('settings.modules.follow_main'),
                        });
                      }
                      ...
                      if (
                        value !== '__inherit__' &&
                        value &&
                        !visibleModels.some((m) => `${m.api_id}/${m.real_name}` === value)
                      ) {
                        opts.push({
                          value,
                          label: `${value} ${t('settings.modules.not_in_catalog')}`,
                        });
                      }
                      ...
                      return (
                        <CSSelect
                          ...
                          options={opts}
                          placeholder={
                            flatModels.length ? undefined : '请先在 API 设置添加并同步模型'
                          }
                          disabled={
                            savingId === mod.id || savingId === '__all__' || flatModels.length === 0
                          }
                    ...
                    {mod.id === 'embedder' && (
                      <div style={{ marginTop: 6, fontSize: 11, color: 'var(--muted)' }}>
                        ...
                        <strong> 切换 embedder 后,已嵌过的剧本需重新嵌入才会用新模型</strong>
                        (旧向量与新模型不互通)。
                      </div>
                    )}
                  </td>
  ...
  useEffectPL(() => {
    ...
    (async () => {
      try {
        ...
        // 新字段 — 无旧 key
        if (p['memory.token_budget'] !== undefined)
          setTokenBudget(Number(p['memory.token_budget']));
        if (p['memory.auto_archive_after_turns'] !== undefined)
          setAutoArchiveAfter(Number(p['memory.auto_archive_after_turns']));
        if (typeof p['memory.bucket_pinned_enabled'] === 'boolean')
          setBucketPinnedEnabled(p['memory.bucket_pinned_enabled']);
        if (typeof p['memory.bucket_world_enabled'] === 'boolean')
          setBucketWorldEnabled(p['memory.bucket_world_enabled']);
        if (typeof p['memory.bucket_character_enabled'] === 'boolean')
          setBucketCharacterEnabled(p['memory.bucket_character_enabled']);
      } catch (_) {}
    })();
    return () => {
      cancelled = true;
  ...
  return (
    <CSSpaceBetween size="l">
      ...
      <SetGroup title={t('settings.memory.title_recall')}>
        <SetRow
          label={t('settings.memory.recall_depth')}
          description={t('settings.memory.recall_depth_desc')}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <input
              type="range"
              ...
              onMouseUp={(e) => {
                ...
                if (n >= 2 && n <= 20) save('recall_depth', n);
              }}
              onTouchEnd={(e) => {
                const n = Number(e.target.value);
                if (n >= 2 && n <= 20) save('recall_depth', n);
        ...
        <SetRow
          label={t('settings.memory.summary_window')}
          description={t('settings.memory.summary_window_desc')}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <input
              type="range"
              ...
              onMouseUp={(e) => {
                const n = Number(e.target.value);
                if (n >= 3 && n <= 20) save('summary_window', n);
              }}
              onTouchEnd={(e) => {
                const n = Number(e.target.value);
                if (n >= 3 && n <= 20) save('summary_window', n);
        ...
        <SetRow
          label={t('settings.memory.token_budget')}
          description={t('settings.memory.token_budget_desc')}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <input
              type="range"
              ...
              onMouseUp={(e) => {
                const n = Number(e.target.value);
                if (n >= 200 && n <= 2000) save('token_budget', n);
              }}
              onTouchEnd={(e) => {
                const n = Number(e.target.value);
                if (n >= 200 && n <= 2000) save('token_budget', n);
        ...
        <SetRow
          label={t('settings.memory.auto_archive')}
          description={t('settings.memory.auto_archive_desc')}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <input
              type="range"
              ...
              onMouseUp={(e) => {
                const n = Number(e.target.value);
                if (n >= 10 && n <= 200) save('auto_archive_after_turns', n);
              }}
              onTouchEnd={(e) => {
                const n = Number(e.target.value);
                if (n >= 10 && n <= 200) save('auto_archive_after_turns', n);
      ...
      <SetGroup title={t('settings.memory.title_buckets')}>
        <SetRow
          label={t('settings.memory.pinned_max')}
          description={t('settings.memory.pinned_max_desc')}
        >
          <CSInput
            type="number"
            value={String(pinnedMax)}
            onChange={({ detail }) => {
        ...
        <SetRow
          label={t('settings.memory.bucket_pinned')}
          description={t('settings.memory.bucket_pinned_desc')}
        >
          <CSToggle
            checked={bucketPinnedEnabled}
            onChange={({ detail }) => {
              setBucketPinnedEnabled(detail.checked);
              save('bucket_pinned_enabled', detail.checked);
            }}
          >
        ...
        <SetRow
          label={t('settings.memory.bucket_world')}
          description={t('settings.memory.bucket_world_desc')}
        >
          <CSToggle
            checked={bucketWorldEnabled}
            onChange={({ detail }) => {
              setBucketWorldEnabled(detail.checked);
              save('bucket_world_enabled', detail.checked);
            }}
          >
        ...
        <SetRow
          label={t('settings.memory.bucket_character')}
          description={t('settings.memory.bucket_character_desc')}
        >
          <CSToggle
            checked={bucketCharacterEnabled}
            onChange={({ detail }) => {
              setBucketCharacterEnabled(detail.checked);
              save('bucket_character_enabled', detail.checked);
            }}
          >
...
function PermSection() {
  ...
  const saveCustomWhitelist = async (next) => {
    ...
    try {
      localStorage.setItem('perm.custom_whitelist', JSON.stringify(next));
    } catch (_) {}
  };
  ...
  const addCustomEntry = () => {
    ...
    if (!val) {
      setCustomInputError(t('settings.permissions.err_empty'));
      return;
    }
    if (val.length > 80) {
      setCustomInputError(t('settings.permissions.err_too_long'));
      return;
    }
    if (!_CUSTOM_WL_RE.test(val)) {
      setCustomInputError(t('settings.permissions.err_invalid'));
      return;
    }
    if (_HIGH_RISK_ALL.includes(val)) {
      setCustomInputError(t('settings.permissions.err_in_builtin'));
      return;
    }
    if (customWhitelist.includes(val)) {
      setCustomInputError(t('settings.permissions.err_duplicate'));
      return;
    }
  ...
  return (
    <SetGroup title={t('settings.permissions.title')}>
      <SetRow
        label={t('settings.permissions.default_mode')}
        description={t('settings.permissions.default_mode_desc')}
      >
        <SetSelect
          value={defaultMode}
          options={[
      ...
      <SetRow
        label={t('settings.permissions.high_risk')}
        description={t('settings.permissions.high_risk_desc')}
      >
        <CSSpaceBetween direction="horizontal" size="xs">
      ...
      {/* B1: 自定义高风险白名单 */}
      <SetRow
        label={t('settings.permissions.custom_whitelist')}
        description={t('settings.permissions.custom_whitelist_desc')}
      >
        <CSSpaceBetween size="s">
          <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
            <div style={{ flex: 1 }}>
              <CSInput
                ...
                onChange={({ detail }) => {
                  setCustomInput(detail.value);
                  if (customInputError) setCustomInputError('');
                }}
                onKeyDown={(e) => {
                  if (e.detail?.key === 'Enter' || e.key === 'Enter') addCustomEntry();
                }}
                invalid={!!customInputError}
              />
              {customInputError && (
                <div style={{ color: 'var(--danger, #c8675d)', fontSize: 12, marginTop: 4 }}>
                  {customInputError}
                </div>
              )}
            </div>
            <CSButton variant="primary" onClick={addCustomEntry}>
              {t('settings.permissions.add_entry')}
            </CSButton>
          </div>
          ...
          {customWhitelist.length === 0 && (
            <span className="muted" style={{ fontSize: 12 }}>
              {t('settings.permissions.no_entries')}
            </span>
          )}
        </CSSpaceBetween>
      </SetRow>
...
function AuditLogView() {
  ...
  const KIND_META = {
    ...
    parse_error: {
      label: t('settings.permissions.kind_parse_error'),
      color: 'var(--warning, #d4a857)',
      desc: '',
    },
    rejected: {
      label: t('settings.permissions.kind_rejected'),
      color: 'var(--danger, #c8675d)',
      desc: '',
    },
    hard_forbidden: {
      label: t('settings.permissions.kind_hard_forbidden'),
      color: 'var(--danger, #c8675d)',
      desc: '',
    },
    extractor_error: {
      label: t('settings.permissions.kind_extractor_error'),
      color: 'var(--warning, #d4a857)',
      desc: '',
    },
    set_parser_error: {
      label: t('settings.permissions.kind_set_parser_error'),
      color: 'var(--warning, #d4a857)',
      desc: '',
    },
    clarify_yield: {
      label: t('settings.permissions.kind_clarify_yield'),
      color: 'var(--ok, #7eb88e)',
      desc: '',
    },
    acceptance_unmet: {
      label: t('settings.permissions.kind_acceptance_unmet'),
      color: 'var(--warning, #d4a857)',
      desc: '',
    },
    question_skip: {
      label: t('settings.permissions.kind_question_skip'),
      color: 'var(--muted, #888)',
      desc: '',
    },
  };
  ...
  return (
    <>
      ...
      >
        <CSSpaceBetween direction="horizontal" size="s">
          <CSButton variant="normal" onClick={refresh} disabled={loading}>
            {loading
              ? t('settings.permissions.audit_loading')
              : t('settings.permissions.audit_refresh')}
          </CSButton>
      ...
      {!hasState ? (
        ...
        <CSAlert type="info">
          {entries.length === 0
            ? t('settings.permissions.audit_empty')
            : t('settings.permissions.audit_empty_filter', { kind: kindFilter })}
        </CSAlert>
      ) : (
        <div
          style={{
            maxHeight: 360,
            overflowY: 'auto',
            border: '1px solid var(--pl-line, #eee)',
            borderRadius: 6,
          }}
        >
          <table
          ...
          >
            <thead>
              <tr style={{ background: 'var(--pl-bg-soft, #f7f7f9)' }}>
                <th style={{ textAlign: 'left', padding: '6px 8px', width: 130 }}>
                  {t('settings.permissions.audit_col_time')}
                </th>
                <th style={{ textAlign: 'left', padding: '6px 8px', width: 90 }}>
                  {t('settings.permissions.audit_col_type')}
                </th>
                <th style={{ textAlign: 'left', padding: '6px 8px', width: 80 }}>
                  {t('settings.permissions.audit_col_source')}
                </th>
                <th style={{ textAlign: 'left', padding: '6px 8px' }}>
                  {t('settings.permissions.audit_col_detail')}
                </th>
                <th style={{ textAlign: 'right', padding: '6px 8px', width: 50 }}>
                  {t('settings.permissions.audit_col_turn')}
                </th>
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <tr key={idx} style={{ borderTop: '1px solid var(--pl-line, #eee)' }}>
                    ...
                    <td style={{ padding: '4px 8px' }}>
                      <span
                        className="pl-rule-chip"
                        style={{ fontSize: 11, color: meta.color, borderColor: meta.color }}
                      >
                        {meta.label}
                      </span>
                    </td>
...
function DeploySection() {
  ...
  const saveDeployConfig = React.useCallback((patch) => {
    ...
    timerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        window.toast?.(t('settings.deploy.save_fail'), {
          kind: 'danger',
          detail: e?.message || '',
          duration: 3000,
        });
  ...
  const [captchaTriggers, setCaptchaTriggers] = useStatePL([
    'register',
    'password_reset',
    'login_retry',
  ]);
  ...
  return (
    <SetGroup title={t('settings.deploy.title')}>
      ...
      <SetRow
        label={t('settings.deploy.listen_addr')}
        description={t('settings.deploy.listen_addr_desc')}
      >
        <CSInput
          value={listenAddr}
          onChange={({ detail }) => {
            setListenAddr(detail.value);
            saveDeployConfig({ listen_address: detail.value });
          }}
        />
      </SetRow>
      <SetRow label={t('settings.deploy.cors')} description={t('settings.deploy.cors_desc')}>
        <CSInput
          value={corsOrigins}
          onChange={({ detail }) => {
            setCorsOrigins(detail.value);
            saveDeployConfig({ cors_origins: detail.value });
          }}
        />
      </SetRow>
      <SetRow
        label={t('settings.deploy.upload_limit')}
        description={t('settings.deploy.upload_limit_desc')}
      >
        <div>
          <CSInput
            value={uploadLimit}
      ...
      <SetRow label={t('settings.deploy.smtp')} description={t('settings.deploy.smtp_desc')}>
        <CSToggle
          checked={smtpEnabled}
          onChange={({ detail }) => {
            setSmtpEnabled(detail.checked);
            saveDeployConfig({ smtp_enabled: detail.checked });
          }}
        >
      ...
      {smtpEnabled && (
        <>
          <SetRow
            label={t('settings.deploy.smtp_preset')}
            description={t('settings.deploy.smtp_preset_desc')}
          >
            <SetSelect
              value="custom"
              options={[
          ...
          <SetRow
            label={t('settings.deploy.smtp_host_port')}
            description={t('settings.deploy.smtp_host_port_desc')}
          >
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 90px 110px', gap: 6 }}>
              <CSInput
                value={smtpHost}
                placeholder={t('settings.deploy.smtp_host_placeholder')}
                onChange={({ detail }) => {
                  setSmtpHost(detail.value);
                  saveDeployConfig({ smtp_host: detail.value });
                }}
              />
              <CSInput
                value={smtpPort}
                placeholder={t('settings.deploy.smtp_port_placeholder')}
                onChange={({ detail }) => {
                  setSmtpPort(detail.value);
                  saveDeployConfig({ smtp_port: detail.value });
                }}
              />
              <SetSelect
                value={smtpTls}
                ...
                onChange={(val) => {
                  setSmtpTls(val);
                  saveDeployConfig({ smtp_tls: val });
                }}
              />
            </div>
          </SetRow>
          <SetRow
            label={t('settings.deploy.smtp_auth')}
            description={t('settings.deploy.smtp_auth_desc')}
          >
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
              <CSInput
                value={smtpUser}
                placeholder={t('settings.deploy.smtp_user_placeholder')}
                onChange={({ detail }) => {
                  setSmtpUser(detail.value);
                  saveDeployConfig({ smtp_user: detail.value });
                }}
              />
              <CSInput
                type="password"
                value={smtpPass}
                placeholder={t('settings.deploy.smtp_pass_placeholder')}
                onChange={({ detail }) => {
                  setSmtpPass(detail.value);
                  saveDeployConfig({ smtp_pass: detail.value });
                }}
              />
            </div>
          </SetRow>
          <SetRow
            label={t('settings.deploy.smtp_from')}
            description={t('settings.deploy.smtp_from_desc')}
          >
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
              <CSInput
                value={smtpFromName}
                placeholder={t('settings.deploy.smtp_from_name_placeholder')}
                onChange={({ detail }) => {
                  setSmtpFromName(detail.value);
                  saveDeployConfig({ smtp_from_name: detail.value });
                }}
              />
              <CSInput
                value={smtpFromEmail}
                placeholder={t('settings.deploy.smtp_from_email_placeholder')}
                onChange={({ detail }) => {
                  setSmtpFromEmail(detail.value);
                  saveDeployConfig({ smtp_from_email: detail.value });
                }}
              />
            </div>
          </SetRow>
          <SetRow
            label={t('settings.deploy.smtp_test')}
            description={t('settings.deploy.smtp_test_desc')}
          >
            <CSSpaceBetween direction="horizontal" size="s">
              <CSButton
                variant="normal"
                ...
                onClick={async () => {
                  setSmtpTesting(true);
                  window.toast?.(t('settings.deploy.smtp_testing_toast'), {
                    kind: 'info',
                    duration: 1200,
                  });
                  let ok = false;
                  ...
                  window.toast?.(
                    ok ? t('settings.deploy.smtp_test_ok') : t('settings.deploy.smtp_test_fail'),
                    { kind: ok ? 'ok' : 'danger', duration: 3000 },
                  );
                }}
              >
                {smtpTesting
                  ? t('settings.deploy.smtp_testing')
                  : t('settings.deploy.smtp_test_btn')}
              </CSButton>
              <span className="muted-2" style={{ fontSize: 11 }}>
                {smtpLastTestAt
                  ? smtpLastTestOk
                    ? t('settings.deploy.smtp_last_ok', {
                        time: window.__fmt?.ago(smtpLastTestAt) || smtpLastTestAt,
                      })
                    : t('settings.deploy.smtp_last_fail', {
                        time: window.__fmt?.ago(smtpLastTestAt) || smtpLastTestAt,
                      })
      ...
      <SetRow label={t('settings.deploy.captcha')} description={t('settings.deploy.captcha_desc')}>
        <CSSpaceBetween direction="horizontal" size="xs">
          <CSButton
            variant={captchaProvider === 'off' ? 'primary' : 'normal'}
            onClick={() => {
              setCaptchaProvider('off');
              saveDeployConfig({ captcha_provider: 'off' });
            }}
          >
            {t('settings.deploy.captcha_off')}
          </CSButton>
          <CSButton
            variant={captchaProvider === 'recaptcha' ? 'primary' : 'normal'}
            onClick={() => {
              setCaptchaProvider('recaptcha');
              saveDeployConfig({ captcha_provider: 'recaptcha' });
            }}
          >
            Google reCAPTCHA
          </CSButton>
          <CSButton
            variant={captchaProvider === 'turnstile' ? 'primary' : 'normal'}
            onClick={() => {
              setCaptchaProvider('turnstile');
              saveDeployConfig({ captcha_provider: 'turnstile' });
            }}
          >
            Cloudflare Turnstile
          </CSButton>
          <CSButton
            variant={captchaProvider === 'hcaptcha' ? 'primary' : 'normal'}
            onClick={() => {
              setCaptchaProvider('hcaptcha');
              saveDeployConfig({ captcha_provider: 'hcaptcha' });
            }}
          >
            hCaptcha
          </CSButton>
      ...
      {captchaProvider === 'recaptcha' && (
        <>
          <SetRow
            label={t('settings.deploy.captcha_recaptcha_ver')}
            description={t('settings.deploy.captcha_recaptcha_ver_desc')}
          >
            <CSSpaceBetween direction="horizontal" size="xs">
              <CSButton
                variant={recaptchaVer === 'v3' ? 'primary' : 'normal'}
                onClick={() => {
                  setRecaptchaVer('v3');
                  saveDeployConfig({ recaptcha_ver: 'v3' });
                }}
              >
                {t('settings.deploy.captcha_recaptcha_v3')}
              </CSButton>
              <CSButton
                variant={recaptchaVer === 'v2c' ? 'primary' : 'normal'}
                onClick={() => {
                  setRecaptchaVer('v2c');
                  saveDeployConfig({ recaptcha_ver: 'v2c' });
                }}
              >
                {t('settings.deploy.captcha_recaptcha_v2c')}
              </CSButton>
              <CSButton
                variant={recaptchaVer === 'v2i' ? 'primary' : 'normal'}
                onClick={() => {
                  setRecaptchaVer('v2i');
                  saveDeployConfig({ recaptcha_ver: 'v2i' });
                }}
              >
                {t('settings.deploy.captcha_recaptcha_v2i')}
              </CSButton>
            </CSSpaceBetween>
          </SetRow>
          <SetRow label="Site Key" description={t('settings.deploy.captcha_site_key_desc')}>
            <CSInput
              ...
              placeholder="6L···Y9"
              onChange={({ detail }) => {
                setRecaptchaSiteKey(detail.value);
                saveDeployConfig({ recaptcha_site_key: detail.value });
              }}
            />
          </SetRow>
          <SetRow label="Secret Key" description={t('settings.deploy.captcha_secret_key_desc')}>
            <CSInput
              ...
              placeholder="6L···Z3"
              onChange={({ detail }) => {
                setRecaptchaSecretKey(detail.value);
                saveDeployConfig({ recaptcha_secret_key: detail.value });
              }}
            />
          </SetRow>
          <SetRow
            label={t('settings.deploy.captcha_score')}
            description={t('settings.deploy.captcha_score_desc')}
          >
            <CSInput
              type="number"
              value={String(recaptchaScore)}
              onChange={({ detail }) => {
                setRecaptchaScore(Number(detail.value));
                saveDeployConfig({ recaptcha_score: Number(detail.value) });
              }}
            />
          </SetRow>
        </>
        ...
        <>
          <SetRow label="Site Key" description={t('settings.deploy.captcha_turnstile_site_desc')}>
            <CSInput
              ...
              placeholder="0x4A···AAAA"
              onChange={({ detail }) => {
                setTurnstileSiteKey(detail.value);
                saveDeployConfig({ turnstile_site_key: detail.value });
              }}
            />
          </SetRow>
          <SetRow
            label="Secret Key"
            description={t('settings.deploy.captcha_turnstile_secret_desc')}
          >
            <CSInput
              type="password"
              ...
              placeholder="0x4A···AAAA"
              onChange={({ detail }) => {
                setTurnstileSecretKey(detail.value);
                saveDeployConfig({ turnstile_secret_key: detail.value });
              }}
            />
          </SetRow>
          <SetRow
            label={t('settings.deploy.captcha_widget_mode')}
            description={t('settings.deploy.captcha_widget_desc')}
          >
            <CSSpaceBetween direction="horizontal" size="xs">
              <CSButton
                variant={turnstileMode === 'non_interactive' ? 'primary' : 'normal'}
                onClick={() => {
                  setTurnstileMode('non_interactive');
                  saveDeployConfig({ turnstile_mode: 'non_interactive' });
                }}
              >
                {t('settings.deploy.captcha_non_interactive')}
              </CSButton>
              <CSButton
                variant={turnstileMode === 'interactive' ? 'primary' : 'normal'}
                onClick={() => {
                  setTurnstileMode('interactive');
                  saveDeployConfig({ turnstile_mode: 'interactive' });
                }}
              >
                {t('settings.deploy.captcha_interactive')}
              </CSButton>
              <CSButton
                variant={turnstileMode === 'invisible' ? 'primary' : 'normal'}
                onClick={() => {
                  setTurnstileMode('invisible');
                  saveDeployConfig({ turnstile_mode: 'invisible' });
                }}
              >
                {t('settings.deploy.captcha_invisible')}
              </CSButton>
            </CSSpaceBetween>
          </SetRow>
        </>
        ...
        <>
          <SetRow label="Site Key">
            <CSInput
              ...
              onChange={({ detail }) => {
                setHcaptchaSiteKey(detail.value);
                saveDeployConfig({ hcaptcha_site_key: detail.value });
              }}
            />
          </SetRow>
          <SetRow label="Secret Key">
            <CSInput
              ...
              placeholder="0x···"
              onChange={({ detail }) => {
                setHcaptchaSecretKey(detail.value);
                saveDeployConfig({ hcaptcha_secret_key: detail.value });
              }}
            />
          </SetRow>
        </>
      ...
      {captchaProvider !== 'off' && (
        <SetRow
          label={t('settings.deploy.captcha_triggers')}
          description={t('settings.deploy.captcha_triggers_desc')}
        >
          <CSSpaceBetween direction="horizontal" size="xs">
            {[
...
function AccountSection() {
  ...
  const handleToggle = async (newChecked) => {
    ...
    try {
      ...
      window.__apiToast?.(t('settings.account.co_builder_save_fail'), {
        kind: 'danger',
        detail: e?.message,
      });
      // 回滚
        ...
        {isCoBuilder ? (
          ...
          <SetRow label={t('settings.account.co_builder_label')} description="">
            <span style={{ fontSize: 13, color: 'var(--text-quiet)' }}>
              {t('settings.account.co_builder_na')}
            </span>
          </SetRow>
        )}
      </SetGroup>
...
function DataMigrationSection() {
  ...
  useEffectPL(() => {
    ...
    window.api?.account
      ...
      .catch((e) => {
        if (alive) setEstErr(e?.message || String(e));
      });
    return () => {
      alive = false;
    };
  ...
  const doExport = () => {
    ...
    try {
      ...
      a.remove();
      window.__apiToast?.(
        t('settings.migrate.export_started', { defaultValue: '已开始下载数据包…' }),
        { kind: 'ok', duration: 2200 },
      );
    } catch (e) {
      window.__apiToast?.(t('settings.migrate.export_fail', { defaultValue: '导出失败' }), {
        kind: 'danger',
        detail: e?.message,
      });
    } finally {
  ...
  const finishJob = async (jobId) => {
    ...
    try {
      ...
      window.__apiToast?.(t('settings.migrate.import_done', { defaultValue: '导入完成' }), {
        kind: 'ok',
        duration: 2600,
        detail: `剧本 ${summary.scripts ?? 0} · 存档 ${summary.saves ?? 0} · 角色卡 ${summary.cards ?? 0}`,
      });
    } catch (e) {
      window.__apiToast?.(t('settings.migrate.import_fail', { defaultValue: '导入失败' }), {
        kind: 'danger',
        detail: e?.message,
      });
    } finally {
  ...
  const doImport = async () => {
    ...
    setImportJob({ stage: 'scripts', stage_progress: 0, stage_total: 0 });
    let jobId = null;
    try {
      ...
      window.__apiToast?.(t('settings.migrate.import_fail', { defaultValue: '导入失败' }), {
        kind: 'danger',
        detail: e?.payload?.error || e?.message,
      });
      return;
    }
    ...
    esRef.current = window.api.scripts.streamImport(jobId, {
      on_update: (jb) => {
        setImportJob(jb);
        if (isTerminal(jb.status)) finishJob(jobId);
      },
  ...
  return (
    <SetGroup
      ...
      description={t('settings.migrate.desc', {
        defaultValue: '把你的全部个人数据打包,迁移到本地自部署实例;或从数据包恢复。不含 API 密钥。',
      })}
    >
      <CSAlert type="info">
        {t('settings.migrate.note_keys', {
          defaultValue:
            '出于安全,导出不含 API 密钥(在服务端加密存储,跨实例无法解密)。迁移到本地后请在「设置 → 模型」重新填写各 provider 的 API key。',
        })}
      </CSAlert>
      ...
      <SetRow
        ...
        description={
          est
            ? t('settings.migrate.export_counts', {
                defaultValue: '剧本 {{s}} · 存档 {{v}} · 角色卡 {{c}} · 模型条目 {{m}}',
                s: est.scripts ?? 0,
                v: est.saves ?? 0,
                c: est.cards ?? 0,
                m: est.model_entries ?? 0,
              })
            : estErr
              ? t('settings.migrate.est_fail', { defaultValue: '统计失败:' }) + estErr
              : t('settings.migrate.estimating', { defaultValue: '正在统计…' })
        }
      >
        <CSSpaceBetween size="xs">
          <CSToggle
            checked={includeChunks}
            onChange={({ detail }) => setIncludeChunks(detail.checked)}
          >
            {t('settings.migrate.include_chunks', {
              defaultValue: '包含原文切片(体积更大,用于本地继续做向量检索)',
            })}
          </CSToggle>
      ...
      <SetRow
        ...
        description={t('settings.migrate.import_help', {
          defaultValue:
            '选择从在线服务导出的 account-*.zip。导入会在当前账号下新建剧本/存档/角色卡,不覆盖现有数据。',
        })}
      >
        <CSSpaceBetween size="xs">
          <input
      ...
      {importJob && (
        <CSBox>
          <div style={{ fontSize: 13, marginBottom: 4 }}>
            ...
            {importJob.stage_total
              ? ` ${importJob.stage_progress || 0}/${importJob.stage_total}`
              : '…'}
          </div>
          <div
            style={{
              height: 6,
              background: 'var(--line,#36322d)',
              borderRadius: 3,
              overflow: 'hidden',
            }}
          >
      ...
      {importResult && (
        <CSAlert
          type={(importResult.warnings || []).length ? 'warning' : 'success'}
          header={t('settings.migrate.import_result', { defaultValue: '导入结果' })}
        >
          <div>
            {t('settings.migrate.import_summary', {
              defaultValue: '剧本 {{s}} · 存档 {{v}} · 角色卡 {{c}}',
              s: importResult.scripts ?? 0,
              v: importResult.saves ?? 0,
              c: importResult.cards ?? 0,
            })}
          </div>
          {(importResult.warnings || []).length > 0 && (
            <ul style={{ margin: '6px 0 0', paddingLeft: 18, fontSize: 12 }}>
              ...
              {importResult.warnings.length > 20 && (
                <li>… 其余 {importResult.warnings.length - 20} 条已省略</li>
              )}
            </ul>
          )}
        </CSAlert>
...
function OnlineLibrarySection() {
  ...
  const reload = useCallbackPL(async () => {
    try {
      setConn(await window.api.federation.connectorGet());
    } catch {
      setConn({ connected: false, base_url: DEFAULT_ONLINE_BASE });
    }
  }, []);
  useEffectPL(() => {
    reload();
  ...
  useEffectPL(() => {
    window.api?.federation
      ?.providerInfo?.()
      .then((r) => setIsProvider(!!r?.provider_enabled))
...
function ConnectorConnect({ conn, onChange }) {
  ...
  const pollRef = React.useRef(null);
  useEffectPL(() => {
    setBase(conn?.base_url || DEFAULT_ONLINE_BASE);
  }, [conn?.base_url]);
  useEffectPL(
    () => () => {
      if (pollRef.current) clearInterval(pollRef.current);
  ...
  const savePat = async () => {
    if (!token.trim()) {
      window.__apiToast?.('请粘贴访问令牌', { kind: 'warning' });
      return;
    }
    setBusy(true);
    try {
      ...
      window.__apiToast?.('连接失败', { kind: 'danger', detail: e?.payload?.error || e?.message });
    } finally {
      setBusy(false);
    }
  };
  ...
  const disconnect = async () => {
    setBusy(true);
    try {
      await window.api.federation.connectorSet(base.trim(), '');
      window.__apiToast?.('已断开', { kind: 'ok' });
      onChange?.();
    } catch (e) {
  ...
  const startDevice = async () => {
    setBusy(true);
    try {
      const d = await window.api.federation.deviceStart(base.trim(), [
        'library:read',
        'library:publish',
      ]);
      setDevice(d);
      ...
      pollRef.current = setInterval(async () => {
        try {
          const r = await window.api.federation.devicePoll(
            d.base_url || base.trim(),
            d.device_code,
          );
          if (r.connected) {
            clearInterval(pollRef.current);
            ...
            pollRef.current = null;
            setDevice(null);
            window.__apiToast?.('授权未完成:' + r.status, { kind: 'warning' });
          }
        } catch {
          /* 继续轮询 */
        }
  ...
  if (conn?.connected) {
    return (
      <CSSpaceBetween size="s">
        <CSBox>
          已连接:<strong>{conn.base_url}</strong>
        </CSBox>
        <CSButton iconName="unlocked" loading={busy} onClick={disconnect}>
          断开连接
        </CSButton>
      </CSSpaceBetween>
    );
  ...
  return (
    <CSSpaceBetween size="m">
      <SetRow
        label="在线服务地址"
        description="默认官方;可改为你自建的在线节点(强制 https,禁私网地址)。"
      >
        <CSInput
          value={base}
          onChange={({ detail }) => setBase(detail.value)}
      ...
      <SetRow
        label="方式一 · 设备码连接(推荐)"
        description="点连接 → 在浏览器登录在线服务并输入下面的配对码授权,无需手动复制令牌。"
      >
        {device ? (
          <CSAlert type="info" header="在浏览器完成授权">
            ...
            <div>
              2. 配对码:
              <strong style={{ fontSize: 18, letterSpacing: 2 }}>{device.user_code}</strong>
              (如未自动填入则手动输入)
            </div>
            <div style={{ marginTop: 6, color: 'var(--text-quiet)' }}>批准后本页自动连接…</div>
          </CSAlert>
        ) : (
          <CSButton variant="primary" iconName="external" loading={busy} onClick={startDevice}>
            用设备码连接
          </CSButton>
        )}
      </SetRow>
      ...
      <SetRow
        label="方式二 · 粘贴个人访问令牌(PAT)"
        description="在线服务「个人访问令牌」里生成一个,粘贴到此。"
      >
        <CSSpaceBetween size="xs">
          <CSInput
            value={token}
            ...
            placeholder="rpgpat_…"
          />
          <CSButton loading={busy} onClick={savePat}>
            保存并连接
          </CSButton>
        </CSSpaceBetween>
      </SetRow>
...
function OnlineBrowse() {
  ...
  const load = useCallbackPL(async (query) => {
    setLoading(true);
    try {
      const r = await window.api.federation.connectorScripts(query);
      setItems(r?.items || []);
    } catch (e) {
      window.__apiToast?.('加载在线库失败', {
        kind: 'danger',
        detail: e?.payload?.error || e?.message,
      });
      setItems([]);
  ...
  const doImport = async (it) => {
    ...
    try {
      ...
      window.__apiToast?.('已完整导入到本地', {
        kind: 'ok',
        detail: `「${it.title}」→ 本地剧本 #${r.script_id}`,
      });
    } catch (e) {
      window.__apiToast?.('导入失败', { kind: 'danger', detail: e?.payload?.error || e?.message });
    } finally {
      setImporting((p) => ({ ...p, [it.id]: false }));
    }
  };
  return (
    <CSExpandableSection
      headerText="浏览在线剧本库 → 完整导入到本地"
      defaultExpanded
      onChange={({ detail }) => {
        if (detail.expanded && items == null) load('');
      }}
    >
      <CSSpaceBetween size="s">
        <div style={{ display: 'flex', gap: 8, maxWidth: 460 }}>
          <div style={{ flex: 1 }}>
            <CSInput
              ...
              onKeyDown={(e) => {
                if (e.detail.key === 'Enter') load(q);
              }}
            />
          </div>
          <CSButton loading={loading} onClick={() => load(q)}>
            搜索
          </CSButton>
        </div>
        {items && items.length === 0 && (
          <CSBox color="text-body-secondary">在线库暂无公开剧本。</CSBox>
        )}
        {(items || []).map((it) => (
          <div
            key={it.id}
          ...
          >
            <div style={{ minWidth: 0 }}>
              ...
              >
                {it.owner_name ? `by ${it.owner_name} · ` : ''}♥ {it.clone_count || 0}
                {it.description ? ' · ' + String(it.description).slice(0, 50) : ''}
              </div>
            </div>
            <CSButton variant="primary" loading={!!importing[it.id]} onClick={() => doImport(it)}>
              导入
            </CSButton>
          </div>
        ))}
...
function OnlinePublish() {
      ...
      .then((r) => {
        ...
        setScripts(
          list
            ...
            .map((s) => ({ value: String(s.id), label: s.title || `剧本 #${s.id}` })),
        );
      })
      .catch(() => {});
  }, []);
  const publish = async () => {
    ...
    try {
      ...
      window.__apiToast?.('发布失败', { kind: 'danger', detail: e?.payload?.error || e?.message });
    } finally {
      setBusy(false);
    }
  };
  return (
    <SetRow
      label="发布自有剧本到在线库"
      description="把本地一个自有剧本完整上传到在线库并公开(需令牌含发布权限)。"
    >
      <div style={{ display: 'flex', gap: 8, maxWidth: 460 }}>
        <div style={{ flex: 1 }}>
          <CSSelect
        ...
        <CSButton iconName="upload" loading={busy} disabled={!sel} onClick={publish}>
          发布
        </CSButton>
      </div>
    </SetRow>
...
function DeviceApprove() {
  ...
  const lookup = async () => {
    ...
    setInfo(null);
    try {
      const r = await window.api.federation.deviceLookup(code.trim().toUpperCase());
      setInfo(r.device);
    } catch (e) {
      window.__apiToast?.('未找到配对码', {
        kind: 'warning',
        detail: e?.payload?.error || e?.message,
      });
    } finally {
  ...
  const decide = async (deny) => {
    ...
    try {
      ...
      window.__apiToast?.('操作失败', { kind: 'danger', detail: e?.payload?.error || e?.message });
    } finally {
      setBusy(false);
    }
  };
  return (
    <CSSpaceBetween size="s">
      <div style={{ display: 'flex', gap: 8, maxWidth: 360 }}>
        ...
        <CSButton loading={busy} disabled={!code.trim()} onClick={lookup}>
          查询
        </CSButton>
      </div>
      {info && (
        <CSAlert type="info" header="确认授权">
          <div>
            客户端:{info.client_name || '未命名'} · 权限:{(info.scopes || []).join(', ')}
          </div>
          <CSSpaceBetween direction="horizontal" size="xs">
...
function PatManager() {
  ...
  const reload = useCallbackPL(async () => {
    try {
      const r = await window.api.federation.patList();
      setItems(r?.items || []);
    } catch {
      setItems([]);
    }
  ...
  const create = async () => {
    ...
    if (!sc.length) {
      window.__apiToast?.('至少选一个权限', { kind: 'warning' });
      return;
    }
    setBusy(true);
    try {
      ...
      window.__apiToast?.('生成失败', { kind: 'danger', detail: e?.payload?.error || e?.message });
    } finally {
      setBusy(false);
    }
  };
  const revoke = async (id) => {
    try {
      await window.api.federation.patRevoke(id);
      reload();
      window.__apiToast?.('已吊销', { kind: 'ok' });
    } catch (e) {
      window.__apiToast?.('操作失败', { kind: 'danger', detail: e?.message });
    }
  };
    ...
    <CSSpaceBetween size="s">
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ width: 180 }}>
          <CSInput
            value={name}
            placeholder="令牌名称(如:我的本地实例)"
            onChange={({ detail }) => setName(detail.value)}
          />
        </div>
        <CSToggle
          checked={scopes.read}
          onChange={({ detail }) => setScopes((s) => ({ ...s, read: detail.checked }))}
        >
          读取
        </CSToggle>
        <CSToggle
          checked={scopes.publish}
          onChange={({ detail }) => setScopes((s) => ({ ...s, publish: detail.checked }))}
        >
          发布
        </CSToggle>
        <CSButton loading={busy} onClick={create}>
          生成令牌
        </CSButton>
      </div>
      {items.length === 0 && (
        <CSBox color="text-body-secondary" fontSize="body-s">
          还没有令牌。生成一个供本地实例/CLI 连接,或在另一台设备上用设备码连接(会自动出现在这里)。
        </CSBox>
      )}
      {items.map((p) => (
        <div
        ...
        >
          <span style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            <CSBadge color={p.source === 'device' ? 'green' : 'grey'}>
              {p.source === 'device' ? '设备授权' : '手动令牌'}
            </CSBadge>
            <strong>{p.name || '(未命名)'}</strong>
            <span style={{ color: 'var(--text-quiet)' }}>{(p.scopes || []).join(', ')}</span>
            <span style={{ color: 'var(--text-quiet)', fontSize: 12 }}>
              {p.last_used_at
                ? '· 最近使用 ' + (window.__fmt?.ago(p.last_used_at) || p.last_used_at)
                : '· 从未使用'}
              {p.revoked_at ? ' · 已吊销' : ''}
            </span>
          </span>
          {!p.revoked_at && (
            <CSButton variant="inline-link" onClick={() => revoke(p.id)}>
              吊销
            </CSButton>
          )}
        </div>
...
function DangerSection() {
  ...
  return (
    <SetGroup title={t('settings.danger.title')}>
      <SetRow
        label={t('settings.danger.clear_saves')}
        description={t('settings.danger.clear_saves_desc')}
      >
        <CSButton variant="normal" onClick={() => openConfirm('clear')}>
          {t('settings.danger.clear_saves_btn')}
        </CSButton>
      </SetRow>
      <SetRow
        label={t('settings.danger.reset_platform')}
        description={t('settings.danger.reset_platform_desc')}
      >
        <CSSpaceBetween direction="horizontal" size="s">
          <CSButton variant="normal" disabled>
            {t('settings.danger.reset_cli_btn')}
          </CSButton>
          <span className="muted-2" style={{ fontSize: 11 }}>
            {t('settings.danger.reset_cli_hint')}
      ...
      {confirm === 'clear' && (
        <div className="pl-modal-backdrop" onClick={closeConfirm}>
          ...
          >
            <header className="pl-modal-head">
              <div>
                <div className="pl-modal-eyebrow" style={{ color: 'var(--danger)' }}>
                  {t('settings.danger.clear_modal_eyebrow')}
                </div>
                <h2 className="pl-modal-title">{t('settings.danger.clear_modal_title')}</h2>
              </div>
              <button className="iconbtn" onClick={closeConfirm} data-tip={t('common.close')}>
                <Icon name="close" size={14} />
              </button>
            </header>
            ...
            <div style={{ marginTop: 14 }}>
              ...
              >
                {t('settings.danger.clear_confirm_label')}{' '}
                <strong style={{ color: 'var(--danger)' }}>
                  {t('settings.danger.clear_confirm_word')}
                </strong>{' '}
                {t('settings.danger.clear_confirm_suffix')}
              </label>
              <input
                className="pl-input"
                value={confirmText}
            ...
            {clearProgress && (
              <div style={{ marginTop: 10, fontSize: 12.5, color: 'var(--text-quiet)' }}>
                {t('settings.danger.clear_progress', {
                  done: clearProgress.done,
                  total: clearProgress.total,
                })}
                <div
                  style={{ height: 4, background: 'var(--bg-deep)', borderRadius: 2, marginTop: 6 }}
                >
                  <div
                    style={{
                      height: '100%',
            ...
            <footer className="pl-modal-foot">
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button
                  ...
                  disabled={
                    confirmText !== t('settings.danger.clear_confirm_word') || !!clearProgress
                  }
                  onClick={async () => {
                    if (nSaves === 0) {
                      window.__apiToast?.(t('settings.danger.clear_empty'), {
                        kind: 'info',
                        duration: 1600,
                      });
                      closeConfirm();
                    ...
                    window.__apiToast?.(
                      fail
                        ? t('settings.danger.clear_ok_fail', { count: done - fail, fail })
                        : t('settings.danger.clear_ok', { count: done - fail }),
                      { kind: fail ? 'warn' : 'ok', duration: 3000 },
                    );
                    try {
