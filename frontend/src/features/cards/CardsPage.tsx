...
const _asLines = (v) =>
  Array.isArray(v)
    ? v
        .map((x) => (typeof x === 'string' ? x : (x && (x.content || x.text)) || ''))
        .filter(Boolean)
        .join('\n')
    : v || '';
...
function cardFormPayload(form, card) {
  ...
  return {
    ...
    sample_dialogue: trim(form.sample_dialogue)
      .split('\n')
      .map((s) => s.trim())
      .filter(Boolean),
    ...
    <CSSpaceBetween size="l">
      ...
      >
        <CSColumnLayout columns={2}>
          <CSFormField
            label={t('cards.editor.name')}
            constraintText={t('cards.editor.name_required')}
          >
            <CSInput
              value={form.name}
              onChange={({ detail }) => u('name', detail.value)}
              autoFocus
            />
          </CSFormField>
          <CSFormField
            label={t('cards.editor.full_name')}
            description={t('cards.editor.full_name_desc')}
          >
            <CSInput
              value={form.full_name}
              onChange={({ detail }) => u('full_name', detail.value)}
            />
          </CSFormField>
          ...
          <CSFormField
            label={t('cards.editor.aliases')}
            description={t('cards.editor.aliases_desc')}
          >
      ...
      >
        <CSSpaceBetween size="l">
          <CSFormField
            label={t('cards.editor.background')}
            description={t('cards.editor.background_desc')}
          >
            <CSTextarea
              rows={3}
              value={form.background}
              onChange={({ detail }) => u('background', detail.value)}
            />
          </CSFormField>
          <CSFormField label={t('cards.editor.appearance')}>
            <CSTextarea
              rows={2}
              value={form.appearance}
              onChange={({ detail }) => u('appearance', detail.value)}
            />
          </CSFormField>
          <CSFormField label={t('cards.editor.personality')}>
            <CSTextarea
              rows={3}
              value={form.personality}
              onChange={({ detail }) => u('personality', detail.value)}
            />
          </CSFormField>
          <CSFormField label={t('cards.editor.speech_style')}>
            <CSTextarea
              rows={2}
              value={form.speech_style}
              onChange={({ detail }) => u('speech_style', detail.value)}
            />
          </CSFormField>
          <CSFormField
            label={t('cards.editor.current_status')}
            description={t('cards.editor.current_status_desc')}
          >
            <CSTextarea
              rows={2}
              value={form.current_status}
              onChange={({ detail }) => u('current_status', detail.value)}
            />
          </CSFormField>
        </CSSpaceBetween>
      </CSExpandableSection>
      ...
      >
        <CSSpaceBetween size="l">
          <CSFormField
            label={t('cards.editor.secrets')}
            description={t('cards.editor.secrets_desc')}
          >
            <CSTextarea
              rows={3}
              value={form.secrets}
              onChange={({ detail }) => u('secrets', detail.value)}
            />
          </CSFormField>
          <CSFormField
            label={t('cards.editor.sample_dialogue')}
            description={t('cards.editor.sample_dialogue_desc')}
          >
            <CSTextarea
              rows={4}
              value={form.sample_dialogue}
              onChange={({ detail }) => u('sample_dialogue', detail.value)}
            />
          </CSFormField>
        </CSSpaceBetween>
      </CSExpandableSection>
      ...
      >
        <CSColumnLayout columns={2}>
          <CSFormField
            label={t('cards.editor.importance')}
            description={t('cards.editor.importance_desc')}
          >
            <CSInput
              type="number"
              value={String(form.importance)}
              onChange={({ detail }) => u('importance', detail.value)}
            />
          </CSFormField>
          {isNpc && (
            <CSFormField
              label={t('cards.editor.first_revealed_chapter')}
              description={t('cards.editor.first_revealed_chapter_desc')}
            >
              <CSInput
                type="number"
                value={String(form.first_revealed_chapter)}
                onChange={({ detail }) => u('first_revealed_chapter', detail.value)}
              />
            </CSFormField>
          )}
          <CSFormField
            label={t('cards.editor.token_budget')}
            description={t('cards.editor.token_budget_desc')}
          >
            <CSInput
              type="number"
              value={String(form.token_budget)}
              onChange={({ detail }) => u('token_budget', detail.value)}
            />
          </CSFormField>
          <CSFormField
            label={t('cards.editor.priority')}
            description={t('cards.editor.priority_desc')}
          >
            <CSInput
              type="number"
              value={String(form.priority)}
              onChange={({ detail }) => u('priority', detail.value)}
            />
          </CSFormField>
          <CSFormField label={t('cards.editor.scope')}>
            <CSSelect
              selectedOption={scopeOpts.find((o) => o.value === form.scope) || scopeOpts[0]}
              options={scopeOpts}
              onChange={({ detail }) => u('scope', detail.selectedOption.value)}
            />
          </CSFormField>
          <CSFormField label={t('cards.editor.enabled')}>
            <CSToggle
              checked={!!form.enabled}
              onChange={({ detail }) => u('enabled', detail.checked)}
            >
...
function CardSheet({ card, kind = 'user' }) {
  ...
  const chapterGate =
    kind === 'npc' && raw.first_revealed_chapter > 1 ? raw.first_revealed_chapter : null;
  ...
  const attrs = [
    {
      label: t('cards.detail.type'),
      value:
        cardTypeLabel[raw.card_type] ||
        (kind === 'npc' ? t('cards.detail.type_npc') : t('cards.detail.type_user')),
    },
    {
      label: t('cards.detail.source'),
      value: sourceLabel[raw.source] || t('cards.detail.source_generic'),
    },
    {
      label: t('cards.detail.importance'),
      value: raw.importance != null ? String(raw.importance) : '—',
    },
    ...(chapterGate
      ? [
          {
            label: t('cards.detail.first_revealed'),
            value: t('cards.detail.chapter_n', { n: chapterGate }),
          },
        ]
      : []),
    {
      label: t('cards.detail.scope'),
      value: scopeLabel[raw.scope] || t('cards.detail.scope_private'),
    },
    {
      label: t('cards.detail.status'),
      value:
        raw.enabled === false ? (
          <CSStatusIndicator type="stopped">{t('cards.detail.status_disabled')}</CSStatusIndicator>
        ) : (
          <CSStatusIndicator type="success">{t('cards.detail.status_enabled')}</CSStatusIndicator>
        ),
    },
      ...
      <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
        <div
          style={{
            ...
            background: 'var(--accent-soft, rgba(201,100,66,.16))',
            color: 'var(--accent, #c96442)',
          ...
          {(aliases.length > 0 || tags.length > 0) && (
            <div style={{ marginTop: 9 }}>
              <CSSpaceBetween direction="horizontal" size="xxs">
                {aliases.map((a) => (
                  <CSBadge key={'a' + a}>{a}</CSBadge>
                ))}
                {tags.map((tg) => (
                  <CSBadge key={'t' + tg} color="green">
                    {tg}
                  </CSBadge>
                ))}
              </CSSpaceBetween>
        ...
        <CSSpaceBetween size="s">
          ...
          {dialogues.length > 0 && (
            ...
            >
              ...
              >
                {t('cards.detail.sample_dialogue')}
              </div>
              <CSSpaceBetween size="xs">
                {dialogues.map((d, i) => (
                  <div
                    key={i}
                    style={{
                      borderLeft: '2px solid var(--accent-soft, rgba(201,100,66,.4))',
                      paddingLeft: 10,
                      color: 'var(--text-quiet, #c8c2b7)',
                      fontSize: 13,
                      lineHeight: 1.6,
...
function OnlineCardsView() {
  ...
  const doImport = async (c) => {
    ...
    try {
      ...
      window.__apiToast?.('已导入到「我的角色卡」', {
        kind: 'ok',
        duration: 2200,
        detail: `「${c.name}」已复制为你自己的卡,可在「我的角色卡」编辑使用。`,
      });
      load(q); // 刷新热度
    } catch (e) {
  ...
  return (
    <CSSpaceBetween size="l">
      <CSHeader
        ...
        actions={
          <CSButton iconName="refresh" loading={loading} onClick={() => load(q)}>
            刷新
          </CSButton>
        }
      >
      ...
      {err && (
        <CSAlert type="error" header="加载失败">
          {err}
        </CSAlert>
      )}
      {loading && items == null ? (
        <CSBox color="text-body-secondary" padding="m">
          正在加载在线角色卡…
        </CSBox>
      ) : items && items.length === 0 ? (
        <CSBox textAlign="center" color="text-body-secondary" padding={{ vertical: 'xl' }}>
          暂无公开角色卡。你可以在「我的角色卡」里把卡设为公开,分享给大家。
        </CSBox>
      ) : (
        <div
          style={{
          ...
          {(items || []).map((c) => (
            ...
            >
              ...
              >
                ...
                <span style={{ fontSize: 11, color: 'var(--text-quiet, #9a948c)' }}>
                  ♥ {c.clone_count || 0}
                </span>
              </div>
              {c.identity && (
                <div style={{ fontSize: 12, color: 'var(--accent, #c96442)' }}>
                  {String(c.identity).slice(0, 40)}
                </div>
              )}
              <div
                style={{
                  fontSize: 12,
                  color: 'var(--text-quiet, #9a948c)',
                  lineHeight: 1.5,
                  minHeight: 36,
                  display: '-webkit-box',
                  WebkitLineClamp: 2,
              ...
              >
                <span style={{ fontSize: 11, color: 'var(--muted, #b8b2a8)' }}>
                  by {c.owner_name || '匿名'}
                </span>
                <CSButton variant="primary" loading={!!importing[c.id]} onClick={() => doImport(c)}>
                  导入
                </CSButton>
              </div>
            </div>
...
function CardGrid({ cards, onEdit, kind, filter, empty, onDeleted, onDuplicate, onPromoteToUser }) {
  ...
  const promoteNpcToUserCard = async (c) => {
    ...
    const body = {
      ...
      tags:
        Array.isArray(c.tags) && c.tags.length
          ? [...c.tags, t('cards.list.tag_from_npc')]
          : [t('cards.list.tag_from_npc')],
      metadata: {
        source: 'npc_promote',
    ...
    try {
      ...
      if (r && r.ok === false)
        throw new Error(r.error || r.detail || t('cards.toast.promote_fail'));
      window.__apiToast?.(t('cards.toast.promoted', { name: body.name }), {
        kind: 'ok',
        duration: 2200,
        detail: t('cards.toast.promoted_detail'),
      });
      if (onPromoteToUser) onPromoteToUser(r?.card || body);
    } catch (e) {
      window.__apiToast?.(t('cards.toast.promote_fail'), {
        kind: 'danger',
  ...
  const menuItems = (c) => {
    ...
    return [
      {
        id: 'png',
        text: t('cards.list.menu_export_png'),
        href: window.api.cards.exportPng(c.id),
        external: true,
        iconName: 'file',
      },
      {
        id: 'tavern',
        text: t('cards.list.menu_export_tavern'),
        href: window.api.cards.exportTavern(c.id),
        external: true,
        iconName: 'download',
      },
      { id: 'copyid', text: t('cards.list.menu_copy_id'), iconName: 'copy' },
      ...(onDuplicate
        ? [{ id: 'dup', text: t('cards.list.menu_duplicate'), iconName: 'copy' }]
        : []),
      isPub
        ? {
            id: 'unpublish',
            text: t('cards.list.menu_unpublish', { defaultValue: '取消公开' }),
            iconName: 'lock-private',
          }
        : {
            id: 'publish',
            text: t('cards.list.menu_publish', { defaultValue: '发布到在线库' }),
            iconName: 'share',
          },
      { id: 'delete', text: t('cards.list.menu_delete'), iconName: 'remove' },
    ];
    ...
    try {
      ...
      window.__apiToast?.(
        pub
          ...
          : t('cards.toast.unpublished', { defaultValue: '已取消公开', name: c.name }),
        { kind: 'ok' },
      );
      onDeleted && onDeleted(c); // 复用 reload 信号刷新列表
    } catch (e) {
      window.__apiToast?.(t('cards.toast.publish_fail', { defaultValue: '操作失败' }), {
        kind: 'danger',
        detail: e?.message || String(e),
      });
    }
      ...
      cardDefinition={{
        header: (c) => (
          <CSSpaceBetween direction="horizontal" size="xs" alignItems="center">
            <CSBox key="name" variant="h3" padding="n">
              {c.name}
            </CSBox>
            {c.pinned && (
              <CSBadge key="pin" color="blue">
                {t('cards.list.pinned')}
              </CSBadge>
            )}
            {(c._raw?.is_public ?? c.is_public) && kind !== 'npc' && (
              <CSBadge key="pub" color="green">
                {t('cards.list.published', { defaultValue: '已公开' })}
              </CSBadge>
            )}
          </CSSpaceBetween>
        ),
        sections: [
          ...
          },
          {
            id: 'bio',
            content: (c) => (
              <div style={{ ...clampLines(3), fontSize: 13, color: 'var(--text-quiet, #968f85)' }}>
                {c.bio || '—'}
              </div>
            ),
          },
          ...
          },
          {
            ...
            content: (c) => (
              <CSBox fontSize="body-s" color="text-status-inactive">
                {kind === 'npc' ? c.save : c.origin} ·{' '}
                {t('cards.list.uses_count', { count: c.uses })} · {c.updated}
              </CSBox>
            ),
          },
          {
            ...
            content: (c) => (
              <CSSpaceBetween direction="horizontal" size="xs">
                <CSButton variant="inline-link" iconName="edit" onClick={() => onEdit(c)}>
                  {t('cards.list.btn_edit')}
                </CSButton>
                <CSButtonDropdown
                  variant="inline-icon"
                  ariaLabel={t('cards.list.more_actions')}
                  expandToViewport
                  items={menuItems(c)}
                  onItemClick={({ detail }) => onMenu(c, detail.id)}
                />
              </CSSpaceBetween>
            ),
          },
...
function UserCardsView() {
  ...
  const onImport = async (payload) => {
    try {
      if (payload?.type === 'card' && payload.file) {
        ...
        await window.api.cards.importJson({
          json_string: payload.json_string,
          ai_split: payload.aiSplit,
        });
  ...
  const onDuplicate = async (c) => {
    try {
      ...
      const body = {
        ...
        id: undefined,
        slug: undefined,
        name: (src.name || c.name) + t('cards.list.duplicate_suffix'),
      };
      await window.api.cards.myUpsert(body);
      window.__apiToast?.(t('cards.toast.duplicated'), { kind: 'ok' });
      reload();
    } catch (e) {
      window.__apiToast?.(t('cards.toast.duplicate_fail'), { kind: 'danger', detail: e?.message });
    }
  };
  ...
  const detailEl = selected ? (
    <CardDetailPanel
      ...
      onSave={async (vals) => {
        await onSaveCard({
          ...(selected._raw?.id ? { id: selected._raw.id } : { id: selected.id }),
          ...vals,
        });
      }}
    ...
    <CSTable
      ...
      empty={
        <CSBox textAlign="center" color="inherit" padding={{ vertical: 'l' }}>
          {q ? t('cards.empty.no_match') : t('cards.empty.no_user_cards')}
        </CSBox>
      }
      columnDefinitions={[
        {
          id: 'name',
          header: t('cards.list.col_card'),
          cell: (c) => (
        ...
        {
          id: 'uses',
          header: t('cards.list.col_uses'),
          cell: (c) => t('cards.list.uses_count', { count: c.uses }),
        },
    ...
    <>
      <CSSpaceBetween size="l">
        <CSHeader
          ...
          actions={
            <CSSpaceBetween direction="horizontal" size="xs">
              <CSButton iconName="download" onClick={() => setImporting(true)}>
                {t('cards.import.btn_import')}
              </CSButton>
              <CSButton variant="primary" iconName="add-plus" onClick={() => setAdding(true)}>
                {t('cards.list.btn_add')}
              </CSButton>
            </CSSpaceBetween>
          }
        >
          {t('cards.list.user_cards_title')}
        </CSHeader>
        ...
        <CSSpaceBetween direction="horizontal" size="xs">
          <div style={{ minWidth: 260 }}>
            <CSTextFilter
              filteringText={q}
              filteringPlaceholder={t('cards.list.search_placeholder')}
          ...
          <CSSegmentedControl
            selectedId={filter}
            options={[
              { id: 'all', text: t('cards.list.filter_all') },
              { id: 'pinned', text: t('cards.list.filter_pinned') },
            ]}
            onChange={({ detail }) => setFilter(detail.selectedId)}
          />
        </CSSpaceBetween>

        {selected ? <ResizableSplit storageKey="cards" top={tableEl} bottom={detailEl} /> : tableEl}
      </CSSpaceBetween>
      {adding && (
        <CardEditModal
          card={null}
...
function CardDetailPanel({ card, kind, onSave, onDuplicate, onDelete }) {
  ...
  const doSave = async () => {
    if (!form?.name?.trim()) {
      window.__apiToast?.(t('cards.toast.name_required'), { kind: 'warn' });
      return;
    }
    setSaving(true);
    try {
    ...
    <CSContainer
      header={
        <CSHeader
          ...
          actions={
            <CSSpaceBetween direction="horizontal" size="xs">
              <CSButton variant="primary" iconName="check" loading={saving} onClick={doSave}>
                {t('cards.detail.btn_save')}
              </CSButton>
              <CSButton iconName="copy" onClick={onDuplicate}>
                {t('cards.detail.btn_duplicate')}
              </CSButton>
              {kind === 'user' && (
                <CSButton
                  href={window.api.cards.exportTavern(card.id)}
                  target="_blank"
                  iconName="download"
                >
                  {t('cards.detail.btn_export')}
                </CSButton>
              )}
              <CSButton iconName="remove" onClick={onDelete}>
    ...
    >
      <CSTabs
        ...
        tabs={[
          {
            id: 'info',
            label: t('cards.detail.tab_info'),
            content: (
              <CSKeyValuePairs
                columns={4}
                items={[
                  {
                    label: t('cards.detail.identity'),
                    value:
                      raw.identity || raw.role ? (
                        <div style={{ ...clampLines(2) }}>
                          {_oneLine(raw.identity || raw.role, 140)}
                        </div>
                      ) : (
                        '—'
                      ),
                  ...
                  },
                  {
                    label: t('cards.detail.source'),
                    value:
                      sourceLabel[raw.source] || card.origin || t('cards.detail.source_generic'),
                  },
                  {
                    label: t('cards.detail.importance'),
                    value: raw.importance != null ? String(raw.importance) : '—',
                  ...
                  ...(chapterGate
                    ? [
                        {
                          label: t('cards.detail.chapter_gate'),
                          value: (
                            <CSStatusIndicator type="info">
                              📖 {t('cards.detail.chapter_n', { n: chapterGate })}
                            </CSStatusIndicator>
                          ),
                        },
                      ]
                    ...
                    value:
                      raw.enabled === false ? (
                        <CSStatusIndicator type="stopped">
                          {t('cards.detail.status_disabled')}
                        </CSStatusIndicator>
                      ) : (
                        <CSStatusIndicator type="success">
                          {t('cards.detail.status_enabled')}
                        </CSStatusIndicator>
                      ),
                  },
                  {
                    label: t('cards.detail.tags_label'),
                    value: Array.isArray(raw.tags) && raw.tags.length ? raw.tags.join(' · ') : '—',
                  },
                  { label: t('cards.detail.updated'), value: card.updated || '—' },
                  {
                    label: t('cards.detail.card_id'),
                    value: <span className="mono">{card.id}</span>,
                  },
                ]}
              />
            ),
          ...
          {
            id: 'setting',
            label: t('cards.detail.tab_setting'),
            content: <CardSheet card={card} kind={kind} />,
          },
          {
            id: 'edit',
            label: t('cards.detail.tab_edit'),
            content: form && (
              <CSSpaceBetween size="l">
...
function TavernImportModal({ open, onClose, onConfirm }) {
  ...
  const handleFiles = (list) => {
    ...
    if (list.length > MAX_FILES) {
      window.__apiToast?.(`一次最多 ${MAX_FILES} 张角色卡,已截取前 ${MAX_FILES} 张`, {
        kind: 'warning',
        duration: 2400,
      });
    }
    const valid = arr.filter((f) => {
      ...
      if (f.size > MAX_BYTES) {
        window.__apiToast?.(`文件过大: ${f.name} (>${MAX_BYTES / 1024 / 1024}MB)`, {
          kind: 'danger',
          duration: 2400,
        });
        return false;
  ...
  const tryParseJson = () => {
    ...
    try {
      ...
      setParsed({
        ...
        example_count: (obj.mes_example || obj.data?.mes_example || '')
          .split(/<START>/)
    ...
    <div className="pl-modal-backdrop" onClick={onClose}>
      ...
      >
        <header className="pl-modal-head">
          ...
          <button className="iconbtn" onClick={onClose} data-tip={t('cards.import.btn_close')}>
        ...
        <div className="pl-modal-form">
          ...
          <div className="seg" style={{ display: 'flex' }}>
            <button
              className={importType === 'card' ? 'active' : ''}
              onClick={() => setImportType('card')}
            >
              <Icon name="user" size={12} /> {t('cards.import.type_card')}
            </button>
            <button
              className={importType === 'chat' ? 'active' : ''}
              onClick={() => setImportType('chat')}
            >
            ...
            <>
                    ...
                    <div style={{ display: 'grid', gap: 4 }}>
                      {files.map((f, i) => (
                        ...
                        >
                          <Icon
                            name={
                              f.name.endsWith('.png') || f.name.endsWith('.webp') ? 'image' : 'file'
                            }
                            size={12}
              ...
              {mode === 'paste' && (
                <>
                  ...
                  {parseError && (
                    <div
                      className="pl-validate-step"
                      style={{
                        color: 'var(--danger)',
                        borderColor: 'rgba(200, 103, 93, 0.32)',
              ...
              {parsed && (
                <div
                  className="pl-import"
                  style={{ borderStyle: 'solid', gap: 8, padding: '12px 14px' }}
                >
                  <div
                    className="muted-2"
                    style={{ fontSize: 10.5, textTransform: 'uppercase', letterSpacing: '0.14em' }}
                  >
                    {t('cards.import.preview_label')} · {parsed.format}
                  </div>
                  <div className="pl-card-head" style={{ margin: 0 }}>
                    ...
                    <div className="pl-card-id" style={{ flex: 1 }}>
                      ...
                      <span className="muted-2" style={{ fontSize: 11.5 }}>
                        {t('cards.import.preview_stats', {
                          dialogues: parsed.example_count,
                          tags: parsed.tags?.length || 0,
                        })}
                      </span>
                    </div>
                  </div>
                  <p className="pl-card-bio serif" style={{ margin: 0, WebkitLineClamp: 2 }}>
                    {parsed.description}
                  </p>
                  <div
                    style={{
                  ...
                  >
                    ...
                    >
                      {t('cards.import.first_mes_label')}
                    </span>
                    {parsed.first_mes}
                  </div>
                  {parsed.tags?.length > 0 && (
                    <div className="pl-card-tags">
                      {parsed.tags.map((tg) => (
                        <span key={tg} className="pl-cap-tag">
                          {tg}
                        </span>
                      ))}
                    </div>
              ...
              <label
                style={{
                  ...
                  background: aiSplit
                    ? 'var(--accent-soft, rgba(201,100,66,.1))'
                    : 'var(--bg-deep)',
                }}
              >
                <input
                  type="checkbox"
                  checked={aiSplit}
                  onChange={(e) => setAiSplit(e.target.checked)}
                  style={{ marginTop: 2, accentColor: 'var(--accent, #c96442)' }}
                />
                <span style={{ fontSize: 12.5, lineHeight: 1.5 }}>
                  <strong style={{ color: 'var(--text)' }}>
                    {t('cards.import.ai_split_label')}
                  </strong>
                  <span className="muted-2" style={{ display: 'block', fontSize: 11.5 }}>
                    {t('cards.import.ai_split_hint')}
                  </span>
                </span>
              </label>
              {aiSplit && (
              ...
              {chatParsed && (
                ...
                >
                  <div
                    ...
                    style={{ fontSize: 10.5, textTransform: 'uppercase', letterSpacing: '0.14em' }}
                  >
                    {t('cards.import.chat_preview_label')}
                  </div>
                  <div className="pl-card-head" style={{ margin: 0 }}>
                    ...
                    <div className="pl-card-id" style={{ flex: 1 }}>
                      ...
                      <span className="muted-2" style={{ fontSize: 11.5 }}>
                        {t('cards.import.chat_preview_stats', {
                          msgs: chatParsed.msgCount,
                          user: chatParsed.userName,
                        })}
                      </span>
                    </div>
                  </div>
        ...
        <footer className="pl-modal-foot">
          <span className="muted-2" style={{ fontSize: 11.5 }}>
            ...
            {importType === 'chat'
              ? t('cards.import.chat_footer_hint')
              : t('cards.import.footer_hint')}
          </span>
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn ghost" onClick={onClose}>
              {t('cards.import.btn_cancel')}
            </button>
            {importType === 'card' ? (
              <button className="btn primary" onClick={doConfirmCard} disabled={!canSubmitCard}>
                <Icon name="check" size={12} />{' '}
                {t('cards.import.btn_confirm', { count: files.length > 1 ? files.length : 0 })}
              </button>
            ) : (
...
function NpcCardsView() {
  ...
  const scriptKeys = [...new Set(cards.map((c) => String(c.script_id)))].filter(
    (k) => k && k !== 'null' && k !== 'undefined',
  );
  const titleOfScript = (sid) => {
    const s = scripts.find((x) => String(x.id) === String(sid));
    return (
      (s && s.title) ||
      cards.find((c) => String(c.script_id) === String(sid))?.save ||
      t('cards.list.script_n', { id: sid })
    );
  };
  let filtered = cards;
  ...
  const saveOpts = [
    { value: 'all', label: t('cards.list.all_scripts') },
    ...scriptKeys.map((k) => ({ value: k, label: titleOfScript(k) })),
  ];
  ...
  const onSaveNpc = async (payload) => {
    ...
    try {
      ...
      window.__apiToast?.(t('cards.toast.save_fail'), {
        kind: 'danger',
        detail: e?.message || String(e),
      });
      throw e;
      ...
      <CSSpaceBetween size="l">
        <CSHeader
          ...
          actions={
            ...
            >
              {t('cards.list.btn_add_npc')}
            </CSButton>
          }
        >
          {t('cards.list.npc_cards_title')}
        </CSHeader>
        {error && (
          <CSAlert type="error" header={t('cards.toast.load_fail_header')}>
            {error}
          </CSAlert>
        )}
        <CardGrid
          ...
          empty={
            <CSBox textAlign="center" color="inherit" padding={{ vertical: 'l' }}>
              {loading ? (
                t('cards.list.loading')
              ) : (
                <>
                  {t('cards.empty.no_npc_cards')}
                  <br />
                  {t('cards.empty.no_npc_hint')}
                </>
              )}
            </CSBox>
          }
          filter={
            <CSSpaceBetween direction="horizontal" size="xs">
              <div style={{ minWidth: 240 }}>
                <CSTextFilter
                  filteringText={q}
                  filteringPlaceholder={t('cards.list.search_npc_placeholder')}
                  onChange={({ detail }) => setQ(detail.filteringText)}
                />
              </div>
              <CSSelect
                selectedOption={saveOpts.find((o) => o.value === saveFilter)}
                options={saveOpts}
                disabled={loading}
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {isNew ? t('cards.editor.modal_title_new') : t('cards.editor.modal_title_edit')}
            {kind === 'user' ? t('cards.editor.kind_user') : t('cards.editor.kind_npc')}
          </div>
          <CSButton iconName="close" variant="link" onClick={onClose}>
            {t('cards.editor.btn_cancel')}
          </CSButton>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '20px 24px 80px' }}>
        <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
          ...
          <div style={{ width: 300, flexShrink: 0, position: 'sticky', top: 72 }}>
            <CSContainer
              header={<CSHeader variant="h2">{t('cards.editor.summary_title')}</CSHeader>}
            >
              <CSSpaceBetween size="m">
                <CSStatusIndicator type={nameOk ? 'success' : 'pending'}>
                  {t('cards.editor.name_required_status')}
                </CSStatusIndicator>
                {kind === 'npc' && isNew && targetScriptOptions.length > 0 && (
                  <CSFormField
                    label={t('cards.editor.target_script')}
                    description={t('cards.editor.target_script_desc')}
                  >
                    <CSSelect
                      selectedOption={
                        targetScriptOptions.find((o) => o.value === String(targetScriptId)) ||
                        targetScriptOptions[0]
                      }
                ...
                <CSKeyValuePairs
                  ...
                  items={[
                    ...
                    {
                      label: t('cards.editor.scope'),
                      value:
                        form.scope === 'public'
                          ? t('cards.detail.scope_public')
                          : t('cards.detail.scope_private'),
                    },
                    {
                      label: t('cards.editor.enabled'),
                      value: form.enabled
                        ? t('cards.editor.enabled_on')
                        : t('cards.editor.enabled_off'),
                    },
                  ]}
                />
